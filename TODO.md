...
**0 · KÄYTTÄJÄN TOIMET (päivitetty 2026-07-30 illalla):**
- [x] Kasvojen karsinta: käyttäjä hyväksyi kuvat sellaisenaan ("Kuvat ovat hyviä!") → ei karsintakierrosta. Uuden 1024-erän (ml. 35 naiscoachia) voi halutessa katselmoida: `open tools/faces/out/review.html` (caption näyttää nyt genderin).
- [ ] iPad-silmämääräys: tuore buildi asennettu simuun (vanha appi poistettu → ei fi.lproj-jäänteitä). Katso: draft-luokka (PROD/LRN/CMP, A-arvosanat), kehitysraportit, PlayerDetailView, Fixed 2026 -aloitus, kasvot kaikissa näkymissä + UUTTA: naisvalmentajat (~6 % staffista satunnaisliigassa, palkkausmarkkinoilla ja carousel-palkkauksissa; she/her-taustatekstit; coach_f-placeholderit oikein).
- [x] Committi pilkottuna — tehty 2026-07-30 (ks. git log: draft-dev / fi-poisto / template-liiga / kasvot+naisvalmentajat / tracking).

**⭐ SEURAAVAT ISOT AALLOT — käyttäjän kirjaamat 2026-07-30:**

- [ ] **A · Trade-järjestelmän uudistus — SELVITYS VALMIS 2026-07-30 → `docs/TRADE_OVERHAUL_PLAN.md`.** Verdikti: arvostuskerros (JJ-kartta + ikä/positio/sopimuskäyrät) on NFL-tasoa ja säilyy; markkinakerros on kuollut — **simuloitu kausi tuottaa ~0 tradea** (NFL ~45–60), juurisyy: tulevaisuuden draft-pickejä ei ole olemassa olioina → kaikki AI-polut guard-failaavat; deadline-phase on fantomi (set+revert vierekkäisillä riveillä); `Team.players` ei päivity tradeissa (traded pelaaja pelaa vanhassa joukkueessaan!); AI on deterministinen ratio-testi joka on exploitattavissa (4×75-OVR ostaa 85-OVR-tähden); GM-persoonat on koodattu valmiiksi kuolleena koodina. Suunnitelma: 5 aaltoa (W0 instrumentointi → W1 future-pickit+Team.players+deadline+cap-totuus → W2 elävä AI-markkina+persoonat+exploit-fixit → W3 neuvottelu-UI sopimusneuvottelun portilla → W4 draft-huoneen unifiointi ml. käyttäjän trade-up → W5 siivous) + NFL-vertailubandit + portit. **ODOTTAA: raportin §7 avoimet päätökset käyttäjältä ennen toteutusta** (karttafilosofia, AI-tarjousvolyymit, multi-team-rajaus, conditional-pickit, Team.players-korjaustapa) — **§7 LUKITTU 2026-07-30, toteutus käynnissä: W0 (TradeRecord-ledger + SMOKE diag trades) valmis; W1 kohdat 1 (future-pickit N+1…N+3, provisional slot + adoptio standingsista) ja 3 (deadline viikko 9 oikeana `.tradeDeadline`-vaiheena, kuolleet tehtävät/inbox/tile eloon) valmiit — yksityiskohdat + handoffit `docs/TRADE_OVERHAUL_PLAN.md` §6 Wave 1.**
- [ ] **B · Iso visuaalinen + käytettävyys-UI-aalto ("Tehdään polished!").** Koko sovelluksen visuaalinen ja käytettävyysparannus, erityispainona viikkonäkymän parannus. Generoituja kuvia (Replicate FLUX, sama putki kuin kasvokirjasto) saa käyttää vapaasti: taustakuvitus, stadionit, tunnelmakuvat, tyhjien tilojen kuvitus. Tavoite: viimeistelty, julkaisukelpoinen ulkoasu.
...
- [ ] **HOF-bustit + MVP-kisa + virstanpylväsuutiset** voivat nyt lukea oikeat ura-statsit (`HallOfFameEntry` on Codable defaulteilla; `LeagueNarrativeEngine.accumulateMVPRace` ja `MilestoneTracker` ovat yhä heuristiikkoja).
...
**5 · Pelilliset laajennukset (vaihe 2:n puuteanalyysi — poimi kiinnostavin):**
- [ ] Practice squad + poaching
- [ ] Per-joukkue-kausiarkistot (AI-narratiivit, trendit, valmentajien track record; nyt vain lastSeasonWins)
- [ ] Positiokonversiot kehityspolkuina (versatility-koneisto olemassa)
- [ ] Fasiliteetti-investoinnit → kehitys/vammakytkös (omistajatalous olemassa)
- [ ] Sopimusten kannustinlausekkeet + motivaatiokytkös
- [ ] OwnerGoals-proxyjen korjaus (divisioona/putket/konferenssi lasketaan approksimaatioilla)
- [ ] Valmentajan kehittäjämaine FA/draft-vetovoimana
- [ ] Kasvojen ikävariantit (v2)

**5a · ~~Omistajien gender-yhtenäisyys~~ TEHTY 2026-07-30 (erä 1: Owner.gender + gender-tietoinen kasvovalinta + ownerFemaleFirstNames gate E:hen + dev-templaten oikeat sukupuolet).** Omistajakasvon valinta on gender-sokea: satunnaisliigassa ~4/32 omistajaa saa naiskasvon miehisen fiktiivisen nimen alle, ja dev-moodissa oikeat naisomistajat (Ford Hamp, Irsay-Gordon, Benson, Adams Strunk, Allen) voivat saada mieskasvon. Fix: (1) `Owner.gender` (inline default) + gender-tietoinen `ownerFaceID`-valinta (12 naiskasvoa / 84 mieskasvoa); (2) satunnaisliigaan `ownerFemaleFirstNames`-pooli (~12 % osuus, gate E:hen 6. poolipari + ristitulovalidointi); (3) dev-templateen `ownerGender`-kenttä DEV_OWNERS-taulusta (tunnetut naiset listattu). Pieni aalto, sama kaava kuin naisvalmentajissa.
...
## 🧑‍🏫 #96 VALMENTAJAPOPULAATIO — 2026-08-04 (committoimaton)

8 vuoden analyysin ainoa aikapommi korjattu. **Juurisyy: liigalla oli vain sisäänvirtaus.** Kolme irrotuspassia (`checkCoordinatorPoaching` rullaa JOKAISTA ei-HC-penkkiä kaikilla 32 staffilla — myös lääkintähenkilöstöä — plus `checkHCPromotionPoaching` ja Black Mondayn potkut) heittää ~110-240 miestä/kausi tilaan `teamID = nil`, ja `refillAIStaffVacancies` generoi jokaiseen aukkoon tuntemattoman katsomatta työttömien penkkiä kertaakaan. Mitattu ennen: 575 → 1147 elävää valmentajaa (+82/kausi), vapaat kasvot −100/kausi → katalogi tyhjä ~kaudella 9-10.

- **Penkki ensin** (`CoachMarketEngine.hireFromBench` + `CoachRole.family`/`hiringFamilies`): aukko tarjotaan ensin tarkalle roolille, sitten perheladderille (position ↔ koordinaattori, komento ↔ koordinaattori); lääkintä ei koskaan ristiin. Generointi jää fallbackiksi → yksikään staffi ei jää vajaaksi. `claimed`-setti estää saman miehen palkkaamisen kahteen seuraan samalla advancella; roolin vaihtuessa palkka johdetaan uudelleen istuimen mukaan.
- **Ammatista poistuminen** (`CoachMarketEngine.settleUnemployment`, ajetaan offseasonin VIIMEISENÄ palkkauspassina): `Coach.unemployedSeasons` (inline default) + `attritionChance(age:reputation:years:)` — 1 vuosi ulkona 10 %, 2 → 35 %, 3 → 60 %, 4+ → 85 %, ikäkerroin 0.6-1.7 ja mainekerroin 0.45-1.0. Vanha kiertolainen lähtee heti, nuori kuuma nimi jää odottamaan.
- **Kasvojen palautus, sääntö dokumentoitu koodissa:** lähtö vapauttaa varauksen, **rivi ja sen `faceID` jäävät** — sama kauppa jonka 65+-eläköityminen jo teki. Jäljitys: `CoachingTreeData` ja `coachCarouselLog` ovat nimitilannekuvia ilman kasvoja, mutta `LeagueEvent.coachID` → `EventAlertView` hakee rivin id:llä ja lukee `coach.faceID` — rivin poisto tyhjentäisi arkistoidun uutisen kasvon. `isRetired` on edelleen ainoa lippu jota `FaceLibrary.backfill` katsoo; uusi `departureReason` erottaa vain `"retired"` / `"leftLeague"`.
- **Sivulöydös korjattu:** eläköityneet rivit kiersivät joka kausi kehityspassin ja eläkerullan (`teamID == nil` tarkoitti sekä "työtön" että "poissa") — 65-vuotiaana lopettanut oli kaudella 8 jo 71 ja hänen ikäytetty muotokuvansa liukui arkistouutisissa. Molempiin silmukoihin `!isRetired`.
- **Portit:** buildi BUILD SUCCEEDED · `career` 36/36 · `leaguegen` 19/19 · 4 kauden smoke (kertakäyttösimu c96b, poistettu). **livingCoaches 514 · 516 · 513 · 513** — tasanne heti, vertailu +82/kausi. Rivikirjanpito täsmää nyt eksaktisti joka kausi (512 seedattua + `generated` = `livingCoaches` + `retiredCoaches`: 543/543 · 569/569 · 589/589 · 600/600), ja `retiredAge + leftLeague` on tasan `retiredCoaches`-delta (29+0=29 · 20+4=24 · 13+10=23 · 7+4=11). coachChurn: generated 31 → 11/kausi, recycled 63 → 81-93/kausi. `SMOKE: staffing OK` kaikilla neljällä kaudella — **jokaisella AI-seuralla kaikki 16 penkkiä täynnä**. Pyramidi/kauppabändit referenssin sisällä, ei uusia anomaliatyyppejä (jäljellä vain tunnetut #97 `80+`/`yp0to3` ja #88/#100 `deadlineWeek`); avgOVR 71.74 vs vertailun 71.79 samalla kaudella.

### Katselmuskorjaukset (fixup, sama päivä)

Adversariaalinen katselmus: 8 löydöstä, 4 CONFIRMED + 4 PLAUSIBLE — kaikki korjattu tai verifioitu mittaamalla.

- **F1 — portti oli eri buildista.** Ensimmäisen ajon 8 kauden loki oli aritmeettisesti mahdoton omaa koodiaan vasten: `retiredAge` summautui 674:ään 660 rivillä joista vain 154 päätyi `isRetired`-tilaan, ja kumulatiivinen `leftLeague` LASKI 9→7→4→3. Loki oli buildista jossa `!isRetired`-vahdit puuttuivat (sen `SMOKE: staffing`-rivikin on eri, käyttäjän seuran mukaan laskeva versio). Uusi 4 kauden ajo yllä on ainoa validi lukema. Smokeen lisätty **monotonisuustripwire**: kumulatiivinen `retiredCoaches`/`leftLeague` ei voi pienentyä, ja jos pienenee, se on todiste että kuollutta riviä yhä muokataan → `ANOMALY coachExitsWentBackwards`. Ajossa ei laukea.
- **F4 — kierrätetty valmentaja maksoi kehitysrangaistuksen jota generoitu tuntematon ei maksa.** `promotedInSeason` leimattiin JOKAISESTA roolinvaihdosta, myös sivusiirroista ja alennuksista, ja `isInAdjustmentPeriod` maksaa kauden −0.05 HC / −0.03 koordinaattori pelaajakehityksestä. ~90 kierrätystä/kausi olisi tuonut liiganlaajuisen jarrun. Uusi `CoachRole.isPromotion(from:)` (perheiden `seniority`; lääkintä ladderin ulkopuolella) → vain aito ylennys leimaa.
- **F6 — tripwire kattoi 4/16 penkkiä.** Nyt `CoachRole.allCases`: jokainen vaje raportoidaan penkkikohtaisesti (`unfilledSeats=[TD×3 …]`). Tämä oli se portti joka olisi voinut printata OK vaikka seurat olisivat jääneet pysyvästi ilman lääkäriä — lääkintäperheellä on oma ladderinsa, joten se on ainoa perhe jolle penkki voi oikeasti loppua kesken.
- **F7 — käyttäjän palkkaus POISTI edeltäjän rivin** (`HireCoachView.hire`, `CoachingStaffView.hireMedical`), vaikka #96:n oma sääntö on että rivi jää: `LeagueEvent.coachID` → `EventAlertView` hakee id:llä ja lukee `faceID`. Nyt edeltäjä irrotetaan (`teamID = nil`) → arkistouutinen säilyy, kasvovaraus ei orvoksi, ja mies päätyy työttömien penkille kuten jokainen AI:n potkima.
- **F8 — markkina oli näkymätön pelaajalle.** Kaikki 31 AI-seuraa palkkasivat penkiltä; käyttäjä sai aina keksiä tuntemattoman — myös silloin kun uutinen oli juuri kertonut hänen omasta koordinaattoristaan että "hired away by another organization". `HireCoachView`, lääkintäsheetit ja auto-hire listaavat nyt liigan oikeat työttömät (**FREE AGENT** -merkki rivillä). Vain tarkka rooli, ei mutaatiota ennen allekirjoitusta → budjettitarkistus rivillä on rehellinen ilman uudelleenhinnoittelua.
- **F3 (PLAUSIBLE, korjattu) — `releaseFace` vapautti id:n tarkistamatta haltijaa.** Täydellä poolilla `backfill` jättää duplikaatit tahallaan paikoilleen, joten B saattaa käyttää kasvoa jonka rekisteri sanoo olevan A:n. #96 nosti coach-vapautukset ~17 → ~90/kausi. Uusi `releaseFace(_:heldBy:)` vapauttaa vain jos rekisteri nimeää kutsujan haltijaksi; kaikki 4 kutsupaikkaa päivitetty (+ harness-stub).
- **F2 (PLAUSIBLE, mitattu) — ikäjakauma ei ollut mitattu.** Uusi `SMOKE: coachPop` -rivi (ikä ka/mediaani, 60+ osuus, OVR ka, penkin ikä ka). Lisäksi `hireFromBench` ei enää palauta tiukkaa OVR-maksimia: `hiringScore` diskonttaa myöhäistä uraa (50+/55+/60+ → −1/−3/−6) ja valinta arpoo top-3:sta kuten karuselli jo tekee. **Mittaus: coachAgeAvg 50.3 → 50.2 → 50.2 → 50.7, 60+ osuus 20.6 % → 18.6 % → 17.5 % → 18.1 %** — ei geriatrista ajautumista.
- **F5 (PLAUSIBLE, korjattu) — penkki jaettiin fetch-järjestyksessä**, joten aakkosissa ensimmäiset seurat saivat joka kausi ensimmäisen tarjousoikeuden liigan parhaaseen työttömään. `teams.shuffled()`.
...
- **#90 5th-year option**: kelpoisuus johdettu `draftRound == 1`:stä (ei uutta lippua), `fifthYearDecided/Exercised` inline-defaulteilla. Ikkuna = rollover jossa `contractYearsRemaining==1 && yearsPro==3` — kalenterijäljitys todistaa: laukeaa täsmälleen kerran, vuonna 4 (2026-luokka → 2029-rollover). Hinta 0,80 × franchise tag; exercise: +1 vuosi & palkka = optiohinta (flat-malli, dokumentoitu yliarvio vuodelle 4). Käyttäjä: "Fifth-Year Options" -osio Final Pushissa (persistoitu tila, skip = declined self-healissa); AI päättää ennen `resignAIOwnCore`a. `ChurnDiag.option5`-ämpäri: smoke `27/ovr80.2/age24.7/yp3.0` ekana kelvollisena vuonna — oikea kohortti. **Seuranta: exercise-rate ~85 % ekasta kohortista** (floor 74 appeal / 78 ovr voi olla matala vs "MOST below both doors" -intentti; yksi havainto, ei viritetty).
- **#91 stance/persona FA:han**: `TeamStance.incomingAgeMultiplier` (contender ostaa 28+, rebuilder nuoria) shortlist-painoon rajattuna `[0.769, 1.30]`; GM-persoona settlement-eksponenttina (aggressive 1.25 / analytics 0.80). Molemmat cachetetaan per markkina-ajo.
- **#92 tarve suunnitelmaksi**: rank-0-seurat pois shortlistalta (ellei tyhjenisi); starter-laatuerottelu (paras alle 70 premium-positiossa → critical vaikka count täynnä — premium-ryhmä {QB,WR,DE,CB,LT}, EI koko OL kerralla); per-seura hole-reserve **replacement-tason** hinnalla (halvin starttikelpoinen, ei ryhmän paras), vapautuu kierroksella 5+ / mop-upissa — QB:tön seura voi silti ostaa QB:n.
- **#93 interaktiivipolun hygienia**: AI-vs-AI-signeeraukset saman oven läpi kuin bulkki (capMode + reserve); `shoppingAround` kunnioitetaan VAIN kun käyttäjän tarjous elää (all-AI-listalla se vain jäädytti markkinan); mop-up ajaa bulkkimarkkinan ennen washouteja stepistä riippumatta — sub-60-tier saa vihdoin tarjouksia pelatussakin FA:ssa. Idempotenssi persistoidulla `Career.lastBulkMarketSeason`-leimalla (in-memory static petti relaunchissa — katselmuslöydös).
- **#94 rookie-diilit näkyviin**: recap-rivi "$10.9M · 4 yrs · 4.1%" (`DraftPick.playerID`-join), luokan cap-veloitus + tila headeriin varoitustintillä; war roomiin "You are $X over the cap" pick-hetkellä (luokan osuus eriteltynä); reveal-kortit näyttävät diilin; `ContractEngine.veteranMinimum(cap:)` korvasi duplikaatit (ContractEngine+PracticeSquad; DraftEngine-kopio jää harness-syistä); kuollut `processContractYear` poistettu. Stale-recap ei näytä elävää cap-tilaa ("Salaries shown are current, not as signed").
- Jatkot: exercise-rate-seuranta (yllä) · LeagueGenerator ei leimaa `draftRound`ia seed-veteraaneille → optiot alkavat vasta 1. oman draftin luokasta (rakenteellinen, hyväksytty).
...
Kymmenen review-löydöksen triage: **9 korjattu, 1 kirjattu ulkopuoliseksi (monisave/`careerID`).** Portit: buildi exit 0 · `make_templates.py` **19/19 PASS** (out-tiedostot byte-identtiset niputettuihin) · `LEAGUE_TEMPLATE_VALIDATE=publish` determinism+ordering+faceUniqueness **PASS** · `FACE_BUNDLE_AUDIT=1` PASS (0 kuvaa = suunniteltu tila) · `./run.sh draftclass` 32/32, `career` 18/18 · **uusi**: 3 kauden smoke auditoi kasvot joka kauden jälkeen.

- [x] **Poolin loppuminen MITATTU, ei arvattu — ja "ei koskaan duplikaattia" korvattu invariantilla jota koodi todella pitää.** 3 kauden smoke: t0 2208 henkeä / 352 vapaata / 0 duplikaattia → kausi 1 2538/22/**0** → kausi 2 2774/0/399 → kausi 3 2557/3/**0** (iso eläköitymisaalto palauttaa poolin). Liiga persistoi joka offseason 224 draft-pickiä + ~124 AI-UDFA:ta vs. 40-90 eläköitymistä, eikä allekirjoittamaton FA tai työtön valmentaja koskaan poistu storesta → **väkiluku ylittää 2560:n kauden 2 aikana**. Uusi invariantti: *kukaan ei jaa kasvoa niin kauan kuin käyttämätön on olemassa*, ja kun pooli on täynnä, duplikaatti osuu näkymättömään henkilöön.
- [x] **Cooldown-reclaim ENNEN duplikaattia** (`pickLocked` tiers 10-16): jos mikään ei ole "kunnioittavasti" vapaa, jaetaan id jota kukaan ELÄVÄ ei pidä (2 kauden cooldown ohitetaan). Tämä yksin pitää kauden 1 nollassa duplikaatissa — ilman sitä ensimmäinen draft/UDFA-aalto tyhjensi vapaan listan samalla kun 40-90 eläköityneen kasvo makasi cooldownin takana käyttämättömänä. Eläkeläisen kasvon uusiokäyttö on aina parempi kuin joukkuetoverin kloonaus.
- [x] **Valmentajan kasvo vapautuu: uusi `Coach.isRetired`** (inline default → kevyt migraatio). 65+-eläköityminen oli pelkkä `teamID = nil` rivin jäädessä ikuisesti, ja `backfill` varasi JOKAISEN Coach-rivin kasvon uudelleen joka advancella → valmentajapuoli vuoti monotonisesti (smoke: 85 eläköitynyttä valmentajaa 3 kauden jälkeen = 85 vuotanutta id:tä). Nyt eläköityminen kutsuu `releaseFace`n ja lippu estää takaisinvarauksen; carousel ei myöskään enää kierrätä eläköitynyttä koordinaattoria (`age < 64` kattoi HC:t, ei koordinaattoreita).
- [x] **Täyden poolin duplikaatti ohjataan pois näkyvistä.** Viimeinen taso suosii kasvoa jota pitää **allekirjoittamaton FA tai työtön valmentaja** (~550 kpl vs ~214 ylijäämä), ja `backfill` **uudelleenkohdistaa** kaksi rosterilla/staffilla olevaa jakajaa (`re-aimed N off the visible league`, smoke: 1/14/2/73/9/26). Uusi mittari `FACEQA: onTeamDuplicates=` erottaa "kaksi rosteripelaajaa jakaa kasvon" (ainoa jonka pelaaja näkee) kokonaisluvusta.
- [x] **Neljä palkkauspolkua varaa kasvon** (ennen: preview jäi varaamatta → kaksi samasta rekisteritilasta persistoitua henkilöä sai identtisen kasvon, koska vapaa lista ja `stableHash % count` ovat samat): `HireCoachView.hire`, `CoachingStaffView.hireMedical`, `WeekAdvancer`in interview-expiry-HC ja `refillAIStaffVacancies` (~10 palkkausta yhdellä advancella).
- [x] **`backfill` korjaa nyt kolme tilaa, ei yhtä:** `nil`, **katalogille tuntematon id** (QA-cull poistaa id:t manifestista → ennen "on id" = "on kasvo" ja henkilö piirtyi siluettina IKUISESTI 600 vapaan kasvon rinnalla) ja **kaksi elävää samalla id:llä** (smoke: 16 ja 118 korjausta). Korjaus vain kun poolissa on tilaa — täydellä poolilla vakaa duplikaatti on parempi kuin joka viikko vaihtuva kasvo. Iterointi id-järjestyksessä, ei hakujärjestyksessä → determinismiharness ei lepata.
- [x] **Cross-season-mittaus olemassa:** `MultiSeasonSmokeTest` auditoi joka valmistuneen kauden jälkeen (`FACEQA: smoke/season-N` + `SMOKE: ANOMALY`), ja audit **trappaa vain jos `duplicated > 0 && free > 0`** = aito regressio. Kapasiteettiraja raportoidaan (`POOL FULL — reuse unavoidable`) mutta ei kaada ajoa.
- [x] **Placeholderit per henkilö, ei yhtä yhteistä siluettia.** Nollakuvatila on nykytila JA shippaava tila (varatulla alueella ei ole kuvia), joten 53 identtistä kiekkoa rosterissa olisi ollut huonompi kuin aallon ALLA olleet avatarit. `PersonFacePlaceholder`: pelaaja → proseduraalinen `PlayerAvatarView`, valmentaja → 20 nipussa olevaa `coach_m*/coach_f*`-kuvaa (**stabiililla** hajautuksella — vanha `fullName.hashValue` on prosessikohtaisesti siemenetty ja vaihtoi kuvan joka käynnistyksellä), prospekti → nimikirjaimet id-sävytetyllä kiekolla, pelkkä siluetti vain kutsujille joilla on vain face-id (uutiset, HOF).
- [x] **GM+HC-rivin kasvo siirretty johtavaan reunaan** (löydös oli osin väärä: rivillä OLI käyttäjän avatar, mutta 40 pt kuvana rivin OIKEASSA laidassa). Aito vika oli epäjohdonmukaisuus: `HeadCoachCardView`, `CoachRowView` ja `CoachRowWithDescriptionView` aloittavat kaikki kasvolla badgen jälkeen, joten HC-kortti näytti siltä kuin siitä puuttuisi kasvo joka rivillä sen alla on. Nyt yksi `PersonFaceView` (medium + kultarengas) badgen jälkeen, duplikaatti poistettu. Verifioitu simussa (GM+HC-ura, Staff-ruutu).
- [x] **Scoutingin otsikkorivit täsmäävät nyt PIKSELILLEEN** — portrettisarakkeen 36 pt lisäksi otsikko ei varannut tilaa johtavalle tähti-/vertailupainikkeelle (44/32 pt) EIKÄ käyttänyt rivien `listRowInsets`-marginaaleja (leading 8 / trailing 16, otsikossa 20/20). Mitattu simussa accessibility-frameista: POS-otsikon keskipiste 70 pt = position-badgen keskipiste; NAME 130 pt = nimitekstin alku; Big Boardissa #66/POS 98/NAME 158 = rank/badge/nimi. Ennen korjausta ero oli 68 pt (36 portretti + 32 johtava sarake).
- [x] **`FaceImageCache` rajattu tavuissa + dekoodaus pois main threadista:** `totalCostLimit` 16 MB + per-kuva cost (ennen: `countLimit=160` × ~590 kt = ~90 MB täysikokoisia bitmappeja 30 pt kiekkojen piirtoon SceneKit-matsinäkymän rinnalla), ja `CGImageSourceCreateThumbnailAtIndex` 288 px:iin (suurin kutsupiste `.large` 96 pt @3×) detached taskissa — ennen `UIImage(contentsOfFile:)` siirsi dekoodauksen (~3,8 ms) siihen frameen jossa rivi vieritettiin näkyviin.
- [x] **`loadFaceLibrary` ei enää skannaa koko storea joka advancen jälkeen.** `loadShellData()` on myös Advance Week -napin reload, ja kolme predikaatitonta hakua (`Player` = storen suurin taulu, ~13 k riviä 30 kauden uralla) toistivat työn jonka `WeekAdvancer` teki jo omilla hauillaan. Nyt täysi rekonsiliaatio kerran per avattu save, myöhemmin vain kirjaston uudelleensidonta.
- [x] **QA-cull-luuppi toimii** (verifioitu stub-backend-kopiolla): `generate_faces.py` pudottaa cullatut id:t manifestista (artefakti → `*.png.culled`), regeneroi ne samasta `(seed, id)`-bucketista ja poistaa id:n `culled.json`:sta vasta kun korvaava kuva on levyllä. Ennen re-run oli **no-op** (resume ohitti tasan ne id:t jotka sweep pyysi tekemään uudelleen) ja `package_faces.py` pudotti ne → id oli lopullisesti poissa kirjastosta.
- [x] **Silmämääräinen verifiointi simussa** (uusi ura, GM+HC, satunnaisliiga): roster näyttää per-pelaaja-avatarit (eri ihonsävyt/kypärät, ei 53 identtistä kiekkoa), prospektilista per-prospekti-monogrammit, staffin HC-kortti käyttäjän oman avatarin, ja molemmat scouting-otsikkorivit osuvat sarakkeidensa päälle.
...
Kolme aiempien aaltojen väitettä muutettu mitatuiksi. **Kuvia ei vieläkään ole nipussa eikä tarvitse olla** — tyhjä `Faces/`-kansio on suunniteltu tila, ei puute. Buildi exit 0 · `LEAGUE_TEMPLATE_VALIDATE=publish` **determinism+ordering+faceUniqueness PASS** · `FACE_BUNDLE_AUDIT=1` PASS · `./run.sh draftclass` 32/32, `career` 18/18.

- [x] **Nippukytkentä MITATTU, ei oletettu.** Pudotuspolku on `dynasty/dynasty/Resources/Faces/` ja projekti on `PBXFileSystemSynchronizedRootGroup` → ei projektitiedoston muokkausta. Todennettu 12 kuvan probe-paketilla (kopioitu kansioon, buildattu, ajettu simussa, poistettu): tiedostot **LITISTYVÄT nipun juureen** (`dynasty.app/face_00000.heic`, `dynasty.app/faces_manifest.json`) — ei `Faces/`-alihakemistoa. Siksi sekä `FaceLibrary.manifestURL()` että `FaceImageCache.bundleURL` kokeilevat molempia haaroja. `.gitkeep` ei kopioidu nippuun. Mitattu: tyhjä kansio → `catalog=synthesized/2560, images 0/2560, 2560 siluettia, PASS`; probe → `catalog=manifest/12, manifest=bundle-root, images 12/12, PASS`. FACE_GENERATION_PLAN.md §7.
- [x] **Uusi portti `FaceBundleAudit`** (DEBUG, env `FACE_BUNDLE_AUDIT=1`, yksi `FACEBUNDLE:`-lohko + verdikti). Nolla kuvaa = **PASS** (siluetti on suunniteltu tila). Mittaa kuvat `FaceImageCache.bundleURL`-funktiolla eli **piirtäjän omalla hakulogiikalla** → portti ei voi läpäistä hakua, jota UI ei tee. Kolme FAIL-ehtoa, kaikki verifioitu rikkomalla probe-build tarkoituksella: (a) tyhjä katalogi; (b) `faces_manifest.json` nipussa mutta katalogi putosi synteesiin (katkaistu JSON → FAIL) — ilman tätä rikkinäinen niputus on näkymätön, koska fallback tuottaa toimivan sovelluksen; (c) nipussa HEIC-tiedostoja joita katalogi ei tunne (poistin yhden rivin manifestista → `orphans=1` → FAIL) = vanhentunut manifest, kuvia jotka eivät voi koskaan näkyä.
- [x] **Determinismi ajettu, `faceID` nyt näkyvissä mittarissa.** `LEAGUE_TEMPLATE_VALIDATE=publish`: 32 joukkuetta, **1807 pelaajaa × 24 kenttää**, 512 valmentajaslottia, 257 pickiä, 7526 historiariviä, 2 importtia 2,8 s → **0 eroa**. Nostin `faceID`:n nimettyjen kenttien riville (pelaajat) ja lisäsin oman valmentajarivin, koska aiemmin nolla oli luettavissa vain diff-yhteenvedon PUUTTUMISESTA: `player faceID=0`, `coach faceID=0 / 512 slottia`. Tämä on se kenttä joka rikkoutuisi ~1800 kertaa jos esijako häviäisi (importer mintaa UUID:t uudelleen joka kerta).
- [x] **Uniikkius mitattu molemmilla lähteillä.** Templateliiga: `persons=2319 distinctFaces=2319 duplicated=0 unassigned=0` (varattu alue 271, cross-role 46). Satunnaisliiga: `persons=2208 distinctFaces=2208 duplicated=0` — ja tämä tulee KAHDESTA riippumattomasta kutsupisteestä (`LeagueGenerator.generate`in oma DEBUG-portti + harnessin oma). Templaten leipomat id:t erikseen: `baked=1902 distinct=1902 playersMissingFace=0 roleMismatch=0/0`. Aalto 1:n "32 duplikaattia = aritmeettinen minimi" on siis **kumottu** varatun alueen myötä: nolla, ei minimi.
- [x] **Harness-suitet vihreinä** (`draftclass` 32/32, `career` 18/18). Kasvomuutokset eivät voi vaikuttaa niihin: harness ei syncaa kasvolähteitä lainkaan vaan kääntää `GameModels.harness.swift`in `FaceLibrary`-stubia vasten, joka palauttaa aina nil.

### Auki tästä aallosta
- [x] **`FACE_BUNDLE_AUDIT=1` ajettu oikeilla kuvilla** (2026-07-30, 3584-poolilla): `catalog=manifest/3584`, `images resolved=3584/3584 (100.0%)`, `orphans=0`, `femaleImages=35/35`, verdikti PASS.
- [x] **Nipun koko mitattu**: kasvot 36,25 MiB nipussa (Release-appi yht. 123,5 MB) — reilusti alle App Store -huolirajojen, ei ODR-tarvetta.

## 🙂 VAIHE 4 / AALTO 2: template-kasvojen esijako — 2026-07-29 (committoimaton)

Kiinteän 2026-liigan jokainen henkilö kantaa nyt kasvo-id:nsä templatessa. **Ei riipu kuvatiedostoista** (bucket = puhdas funktio siemenestä; puuttuva HEIC = siluetti). Buildi exit 0 · `make_templates.py` **19/19 PASS** · transform-determinismi (2 ajoa byte-identtiset) · muut kentät bittiin asti ennallaan (vain `faceID` lisätty).

- [x] **Miksi esijako:** `LeagueTemplateImporter` luo jokaiselle `Player`/`Coach`-riville TUOREEN `UUID()`:n joka importissa, ja ajonaikainen valitsija hajauttaa juuri sen UUID:n → kiinteä liiga olisi näyttänyt eri kasvot joka luontikerralla. Nyt id on templatessa → kiinteä myös kasvoiltaan (harnessin `PlayerFingerprint`/`CoachFingerprint` sai `faceID`-kentän vartioimaan tätä).
- [x] **Poolipäätös: 2048 → 2560.** Templaten kysyntä on 1807 pelaajaa (+95 nimettyä valmentajaa) vs. 1672 pelaaja-ikäistä kasvoa `--count 2048`-alueella → uniikkius ei mahdu. **Jakamista EI valittu** (duplikaattikasvo SAMASSA liigassa lukee bugina) vaan varattu ylivuotoalue `face_02048..face_02559`, jonka `generate_faces.py --count 2560` tuottaa myöhemmin (id:t stabiileja, manifest append-only). Tulos: **1902 uniikkia kasvoa / 1902 henkilöä, 0 rooliristiriitaa**, ikäkaista osuu 84,3 %:iin ja build 88,6 %:iin (loput ottavat lähimmän naapuribucketin — build löysätään ENNEN ikää, kuten ajonaikainen `FaceLibrary.pickLocked`).
- [x] **Ylivuoto menee näkymättömimmille:** jako etenee starter → rotation → backup → depth ja ottaa aina pienimmän vapaan id:n, joten varatun alueen 135 id:tä osuvat koko `depth`-tasoon (122) + 13 heikoimpaan backupiin, ja generaattorin ensimmäisenä valmistuvat kuvat menevät näkyvimmille. `FaceLibrary` kohtelee varattua aluetta **viimeisenä oljenkortena**: vapaa generoitu → vapaa varattu → rooli-sisäinen uudelleenkäyttö → cross-role. Satunnaisliiga ei siis koskaan poistu generoidulta alueelta.
- [x] **Valmentajan ikä ennustetaan:** templaten valmentajalla ei ole ikää tiedostossa (`generateCoach` arpoo sen) → työkalu toistaa sen YHDEN arvonnan (SplitMix64 + Swiftin 64-bit `Int.random` Lemire-haara + sovelluksen oma `fnv1a`-literaali). Verifioitu Swift-skriptillä: **96/96 ikää täsmää**. Kytkentä kommentoitu molempiin päihin (`makeStaff`).
- [x] **Uusi portti 19 `face-preassignment`:** uniikkius + rooli + ikäkaista/build-osumaosuudet + ylivuoto vain jonon hännälle + bucket-matematiikan ristiintarkistus oikeaa generaattorin manifestia vasten (721 kasvoa, 0 poikkeamaa). TRANSFORM_QA §8 kirjaa poolialiteetin ja päätöksen.

### Auki tästä aallosta
- [x] **Generointi ajettu loppuun asti** — lopulta `--count 3584` (2026-07-30): koko pooli kuvitettu, myös 512 reserve-id:tä ja 1 024 laajennusaluetta.

## 🙂 VAIHE 4 / AALTO 1: FaceLibrary-ydin — 2026-07-29 (committoimaton)

Kasvokirjaston Swift-ydin. **Kuvatiedostoja ei ole vielä olemassa (generointi käynnissä taustalla, ~570/2048) eikä mikään tässä aallossa riipu niistä** — sovellus piirtää siistin siluetin kun kuva puuttuu, ja nippu saa laillisesti sisältää manifestin ennen kuvia. Buildi exit 0.

- [x] **Katalogi ilman kuvia.** `FaceCatalog.swift`: `faces_manifest.json` nipusta jos on, muuten koko 2048 kasvon id+bucket-lista **synteesataan generaattorin siemenmatematiikasta**. Vaatii bit-tarkan CPython `random.Random`-portin (str-siemenen sha512-hajautus → `init_by_array` → MT19937 → `genrand_res53`), koska bucket ratkaisee luetaanko kasvo 22-vuotiaana laituriurheilijana vai 60-vuotiaana koordinaattorina. **Verifioitu: kaikki tähän mennessä generoidut 571 kasvoa synteesautuvat byte-identtiseen buckettiin.** Generaattorin vakiot (painot, tag-merkkijonot, pool size, siemen) yhdessä paikassa: `FaceGeneratorConstants`.
- [x] **`faceID`-kentät** `Player`/`Coach`/`CollegeProspect` (inline `String? = nil`) + `Career.faceRegistryData` (JSON-blob: `inUse` + eläköitymis-cooldown). Kaikki inline-defaultteja → kevyt migraatio.
- [x] **`FaceLibrary`**: siemenetty (FNV-1a UUID:sta, EI `Hasher` joka on prosessikohtaisesti siemenetty) valinta VAPAISTA saman bucketin kasvoista; degradaatio exact bucket → build → ageBand → **rooli-sisäinen uudelleenkäyttö** → (vain jos rooli on tyhjä) mikä tahansa. Rooliraja rikotaan vasta viimeisenä: duplikaattikasvo eri kaupungissa on pieni tahra, 60-vuotiaan kasvo rookiella on heti näkyvä bugi.
- [x] **Poolin aritmetiikka kirjattu:** 2048 kasvoa (1672 pelaaja- + 376 valmentaja-ikäistä) vs. 32×53 + 32×12 = **2080 henkilöä** → "kaksi aktiivista ei koskaan jaa kasvoa" ei ole aritmeettisesti mahdollista. Mitattu täydellä liigalla: kaikki saavat kasvon, **32 duplikaattia (= aritmeettinen minimi), 0 rooliristiriitaa**. Poolin kasvatus (`--count`) poistaa senkin, manifest on append-only.
- [x] **Prospektit ja valmentajakandidaatit eivät varaa kasvoa** (`previewFace`): luokka = 350 prospektia joista ~30 allekirjoittaa, kandidaattilista 25 josta 1 palkataan — varaaminen tyhjentäisi poolin joka kevät. `DraftEngine.copyProspectMetadata` / carousel-palkkaus muuttaa previewin oikeaksi varaukseksi (`claimFace`) ja pelaaja pitää skoutatun kasvonsa kun se on yhä vapaa.
- [x] **Kytkennät:** LeagueGenerator (satunnaispolku), DraftClassBuilder, UDFA (kulkee copyProspectMetadatan kautta), CoachCarouselEngine, eläköityminen+washout (`PlayerRetirementEngine.retire` = molempien polkujen yksi vapautuspiste; `faceID` JÄÄ riville, jotta HOF/historia näyttää kasvon), sekä legacy-backfill `WeekAdvancer.backfillLegacyFaces` + `CareerShellView`-lataus (kattaa myös template-liigat).
- [x] **`PersonFaceView`** (small/medium/large, valinnainen joukkuevärirengas, `NSCache` + miss-välimuisti, SF-Symbol-siluetti kun `faceID` nil TAI kuva puuttuu). Nolla kaatumista ilman kuvia — koko aalto ajettu tyhjällä `Faces/`-kansiolla.
- [x] **Mitattu** (integraatioharness, joka kääntää oikean `FaceLibrary.swift`:n stub-modelleja vasten): kylmä backfill 2080 henkilölle 60 ms, lämmin no-op 0,6 ms/advance, rekisteriblob 104 kt, idempotentti (faceID:t + blob byte-identtiset), deterministinen toisto, cooldown pitää 2 kautta ja vapauttaa sen jälkeen, tyhjä katalogi palauttaa nil ilman trappia.

### Auki tästä aallosta
- [ ] **Kuvien valmistuminen + `package_faces.py` → `Faces/*.heic` + `faces_manifest.json` nippuun.** Manifest voittaa synteesin heti kun se ilmestyy (se tuntee cullatut id:t). Pudotuspolku + niputuksen käyttäytyminen on nyt mitattu (aalto 3 / FACE_GENERATION_PLAN §7): kopioi `dynasty/dynasty/Resources/Faces/` ja verifioi `FACE_BUNDLE_AUDIT=1`.
- [ ] **UI-kytkennät (aalto 2):** `PersonFaceView` PlayerDetailView-headeriin, roster-riveille, prospektikortteihin/Big Boardiin, draft-paneeleihin, valmentajanäkymiin, uutisiin, HOF-seremonioihin.
- [ ] **On-device-silmämäärä** kun kuvat ovat paikallaan (siluetti vs. oikea kasvo, rengasvärit, small-koon luettavuus).

## 🔒 VAIHE 3 REVIEW-KORJAUKSET — anonymisointi + template-suorituskyky — 2026-07-29 (committoimaton)

Kuuden review-löydöksen triage: **5 korjattu, 1 kirjattu hyväksytyksi eroksi.** Portit ajettu uudelleen kaikki: Debug-buildi exit 0 · `make_templates.py` **18/18 PASS** · bundle gate (Release + skannaus) exit 0 · transform-determinismi (2 ajoa + niputettu tiedosto byte-identtiset) · `LeagueTemplateValidation` determinism+ordering **PASS molemmilla profiileilla** · `./run.sh draftclass` 32/32, `career` 18/18, `familiarity` OK.

- [x] **P0 · 93 pelaajan oikea IR-status shipatussa nipussa.** `notes`-kenttä vei publish-templateen merkkijonon `"finished 2025 on injured reserve"` suoraan raakadatan `injuredAtSeasonEnd`-lipusta. Se on yksilötason oikea lääketieteellinen fakta, jota `ANONYMIZATION_SPEC.md` §3 ei salli, ja koska profiili säilyttää tarkoituksella joukkueen + position + depth rankin, "KC:n QB1 joka lopetti 2025:n IR:llä" = yksiselitteinen tunnistus pelkästä nipusta (myös GB/CIN/LV DE1, DEN/IND QB1). Mikään portti ei kattanut kenttää. → publish `notes` on nyt aina `null` (dev pitää 553 merkintää, DEBUG-only), uusi portti 15 `publish-no-notes-payload` + tarkistus `scan_bundle.py`:n D-vaiheeseen. Kenttää ei lue mikään Swift-koodi.
- [x] **P1 · Nimipoolit = oikeita ihmisiä (aiempi avoin P1, nyt tehty).** `RandomNameGenerator` (55×55) ristiin kertoi **68 oikean 2026-rosteripelaajan** täsmällisen koko nimen (Davante Adams, Jalen Carter, Daniel Jones…), `LeagueGenerator.ownerFirst/LastNames` oli lista oikeita NFL-omistajia (Kraft/Jones/Irsay/Khan) ja `coachFirst/LastNames` oikeita päävalmentajia (McVay/Reid/Shanahan/Belichick) — ja **template-importti kutsuu `generateOwner`ia kaikille 32 seuralle**, eli kiinteä 2026-liiga nimesi omistajansa oikeiden mukaan. Kaikki kolme paria korvattu fiktiivisillä, blocklist-tarkistetuilla pooleilla (poimittu transformin `PUB_FIRST`/`PUB_LAST`-pooleista). Release-binäärissä `Kelce/Bosa/Parsons/Diggs/Kraft/Irsay/McVay/Belichick` = 0 osumaa (`Shanahan` 15 = skeeman nimi, jonka spec §4 säilyttää faktana).
- [x] **P1 · Bundle gaten verdikti valehteli + uusi portti E.** Gate tulosti ehdottoman "no real name … in the product" tuotteesta, jonka binäärissä oli oikeita sukunimiä. Nyt: (a) verdikti kertoo kattavuutensa ja mitä se EI kata, (b) uusi **gate E** lukee nimipoolit suoraan Swift-lähteestä ja pitää koko ristitulon (3025+1024+900 yhdistelmää) ≥3 editin päässä blocklististä, ja **exit code heijastaa sitä**. Negatiivitesti: istutettu `Travis`/`Kelce` → 3 violaatiota; poolin uudelleennimeäminen ja puuttuva tiedosto → violaatio (portti ei voi hiljaa lakata kattamasta pintaansa).
- [x] **P2 · Valmentajien tenure-jitter oli no-op 30/95:lle.** `since + choice([-1,1])` ja vasta sitten `min(LEAGUE_YEAR, …)` → jokaisen 2026-palkatun +1 kellistyi takaisin 2026:een ja **oikea `sinceYear` shippasi sellaisenaan**. Nyt suunta valitaan niistä siirroista jotka selviävät [1990, leagueYear]-clampista; uusi portti 17 `publish-coach-tenure-moved` (95/95 liikkui, 0 identtistä).
- [x] **P2 · Support-staffin nimien rekombinaatio ohitti §1-vartijan.** Importer poimii templatesta 241 etunimeä × 385 sukunimeä ja arpoo puolikkaat ERIKSEEN 417 taustavalmentajalle → saavutettava avaruus on koko ristitulo, ei transformin emittoimat parit. Kolme yhdistelmää oli 2 editin päässä oikeasta pelaajasta (`Cedric Skillman`, `Zaire Frankland`, `Broderick Warrington`) — ja ne syntyvät ajonaikana, minne mikään nipun skannaus ei yllä. Korjaus juuressa: `NameFactory` ei enää emittoi tokenia joka rikkoisi rekombinaation (guard 1c), uusi portti 16 `publish-name-recombination>=3` tarkistaa koko 239×385 ristitulon (~3 s, split-dekompositio `d(x+y,r)=min_s d(x,r[:s])+d(y,r[s:])` — naiivi pariskannaus veisi ~4 min). Lisäksi importer ei enää anna taustavalmentajalle nimeä joka on jo templaten henkilöllä, ja `("Alex","Monroe")`-fallback (Monroe = oikea sukunimi) vaihdettu. `ANONYMIZATION_SPEC.md` §1 sai guard 4:n ja §6.2 lähdetason tarkistuksen.
- [x] **P2 · `CareerArcEngine` raahasi 1338 tuotua veteraania turhaan.** Predikaatti oli `draftPickNumber != nil`, joka oli de-facto "tämä ura draftasi hänet" -lippu kunnes template alkoi leimata (sumennetun) oikean pickin jokaiselle tuodulle veteraanille → joka offseason ~4000 ylimääräistä SwiftData-hakua 7526-riviselle historialle + 1338 `CareerArcState`-riviä joita mikään ei lue (kaikki näkyvät tuotokset ovat `if let pickGrade`-vartion takana, eikä tuoduilla ole DraftPickGrade-riviä). Vaihdettu `draftedByTeamID != nil` (jonka kirjoittaa vain `DraftEngine`) → pariteetti satunnaisliigan kanssa.
- [ ] **HYVÄKSYTTY ERO (ei korjata, kirjattu): template-liigan ikäpyramidi on aidosti vanhempi.** Mitattu: template ikäka 26,7 / 33+ 5,4 % vs satunnaisliiga 25,8 / 3,2 % → ensimmäinen offseason eläköittää ~86 vs ~46 ja draftaa 288 vs 250. Syy on `LeagueGenerator.randomAge` = `min(U,U)` (tarkoituksella nuori) vs oikea NFL-pyramidi, jonka `ANONYMIZATION_SPEC.md` §2 vaatii SÄILYTETTÄVÄN (siksi ±1-jitter on tasan balansoitu). Ei kalibroida: (a) se korvaisi oikeat rosterit synteettisellä ikäkäyrällä, (b) molemmat liigat ovat joka tapauksessa `DEVELOPMENT_NFL_REFERENCE.md` §8:n ≤2 %-bandin yläpuolella, ja sen korjaus on erikseen lykätty P1-aalto — pelkän templaten siirtäminen veisi sen KAUEMMAS satunnaisliigasta, jota sen pitää vastata. Liiga toipuu itse (kauden 1 jälkeen 33+ 0,8 %, rosterit 53/53, loppu-OVR 75,68 vs 75,97). Kirjattu `TRANSFORM_QA.md` §7:ään täydellä histogrammilla + uusi portti 18 `age-profile-in-drift-band`, joka kaatuu vain DRIFTISTÄ (ka 25,0–28,0, 33+ ≤ 8 %) eli estää tilanteen hiljaisen huononemisen uudessa raakadatassa.

### Auki tästä aallosta
- [ ] **P3 · Bundle gate release-prosessiin** (ennallaan): `check_bundle.sh` on käsin ajettava, exit 1 osumasta, rakentaa Release-konfiguraation itse (~4 min).
- [ ] **P3 · Gate E:n kattavuus on lista.** `scan_bundle.py: SWIFT_NAME_POOLS` kattaa 3 pooliparia; uusi ajonaikainen nimilähde on katveessa kunnes se lisätään listaan. Portti kaatuu jos listattu pooli katoaa tai nimetään uudelleen, muttei tunne poolia jota ei ole listattu — verdikti sanoo tämän ääneen.

## 🏟️ VAIHE 3 / STAGE 4 VALIDOINTI VIHREÄ — kiinteä 2026-liiga — 2026-07-29

Kaikki viisi porttia mitattu ja vihreä (ei argumentteja, ajettuja tarkistuksia). Uusi DEBUG-harness `LeagueTemplateValidation` (env `LEAGUE_TEMPLATE_VALIDATE=publish|dev`), smoke-testin template-variantti (`PERF_SMOKE_LEAGUE=fixed2026`) ja bundle-gate `tools/league-data/check_bundle.sh`. Committoimatta.

- **Determinismi PASS** (molemmat profiilit): kaksi importtia → kaksi erillistä in-memory-containeria → koko graafi haettu takaisin ja diffattu. 1807 pelaajaa × 23 kenttää (attribuutit/potentiaali/learning/competitiveness/moraali/sopimus/bio/familiarity), 32 joukkuetta, 512 valmentajaa, 257 pickiä, 7526 urariviä → **0 eroa**.
- **Template-smoke PASS**: 3 kautta publish-templatesta, drift −0,83 / −0,37 / **+0,36** (portti ±1,5; satunnaisliigan verrokki −0,60/+0,35/+0,89), rosterit 53/53 joka kausi, 76 advancea (watchdog 240), 0 ANOMALY.
- **Ordering PASS**: Spearman(joukkueen OVR, 2025-tulos) **0,942**; top-5 2025 (SEA/NE/DEN/JAX/LA) OVR-sijoilla 1/3/4/8/6; bottom-5 (NYG/ARI/LV/NYJ/TEN) sijoilla 26/30/31/32/29; hajonta 5,01 OVR (78,77 → 73,75).
- **Bundle gate PASS** Release-tuotteesta (121 MB, 8 tiedostoa): ei yhtään blocklist-nimeä, ei `league_2026_dev.json`:ia, ei raakadataa; publish-templaten 2028 identiteettimerkkijonoa ≥3 editin päässä 2048 oikeasta nimestä. Negatiivitesti (istutettu dev-json + "Patrick Mahomes") → exit 1.
- **Regressio**: `./run.sh draftclass` (32 assertia) / `career` (18 assertia) / `familiarity` / `all` kaikki OK; Debug-buildi exit 0.

**Löydökset (korjattu tässä aallossa):**
- [x] **Determinismibugi `LeagueGenerator.salaryForCoach`**: ±5 %:n kohinatermi käytti `Double.random(in:)` ilman `using:`-generaattoria vaikka kutsuja oli antanut siemenetyn → **505/512 valmentajan palkka erosi kahden identtisen importin välillä**. Kiinteä liiga ei siis ollut kiinteä. Korjattu siemenetyllä ylikuormalla; satunnaispolku käyttäytyy ennallaan (`SystemRandomNumberGenerator`-wrapper).
- [x] **32 oikeaa NFL-pelinrakentajan nimeä Release-nipussa**: `NFLTeamData.TeamPreview.startingQBName` ("P. Mahomes", "C.J. Stroud", "L. Jackson", …) ei ollut pelkkä esikatselu-UI-teksti — `LeagueGenerator.generateNamedQB` rosteroi niistä oikeannimisen QB:n oikealla OVR:llä satunnaisliigaan. Tämä on täsmälleen `ANONYMIZATION_SPEC.md` §0:n vahva identifioija (nimi + joukkue + positio + rating). Korvattu 32 fiktiivisellä nimellä (siemenetty poiminta transformin blocklist-suodatetusta sukunimipoolista, jokainen ≥3 editin päässä koko blocklististä). Vain merkkijonot muuttuivat — generointilogiikka koskematon, harness-lähteitä ei synkata tästä tiedostosta.
- [x] **Kovakoodattu "C. Williams (QB) · OVR 88"** `CareerDashboardView`-combine-kortissa (shipatussa UI:ssa) → fiktiivinen nimi.

**Löydökset (AUKI — oma aalto):**
- [x] **P1 · Satunnaispolun nimipoolit ovat oikeita ihmisiä.** → KORJATTU review-aallossa (ks. ylin osio): kaikki kolme pooliparia fiktiivisiä, `draftclass` ajettu perään (32/32), ja gate E pitää ristitulon puhtaana jatkossa.
- [ ] **P3 · Bundle gate CI:hin**: `check_bundle.sh` on nyt käsin ajettava. Luonteva paikka on release-skripti tai pre-ship-tarkistuslista; se rakentaa Release-konfiguraation itse (~4 min) ja palauttaa exit 1 osumasta.
- [ ] **Huom (ei bugi):** template-rosterit alkavat 55–57 pelaajalla (53 + IR-huomiot). Ura alkaa `coachingChanges`-vaiheesta, joten cutdown leikkaa ne 53:een ennen ensimmäistä kautta — smoke mittaa 53/53 jokaisen kauden lopussa. Satunnaispolku alkaa 53:sta.

## 🧠 VAIHE 2 SHIPATTU: KEHITYS/HARJOITTELU-OVERHAUL + FI-POISTO — 2026-07-29

Realisaatiomalli toteutettu ja validoitu (suunnitelma `docs/PLAYER_DEVELOPMENT_OVERHAUL_PLAN.md`, data `docs/DEVELOPMENT_NFL_REFERENCE.md`). Kaikki portit vihreinä: career-harness 18/18 (44 800 uraa — hit ratet, trajektorit, peak-iät, R-jakauma), draftclass 32/32, familiarity, all, 3+5 kauden smoke (OVR-drift +1,07/5 kautta, loppu 0,00/kausi; **leaguePot plateautuu +0,14 → ratchet kuollut**), moraaliportit. Verify löysi 11 → 9 korjattu (washout-FA-bugi, workload-kerroin oikeisiin vammapolkuihin, scheme-decay-lattia 55, quitter-gatet, camp-raportti), 1 kohinaa, 1 defer (alla). fi-tuki poistettu kokonaan (387 käännöstä + knownRegions; katalogi-strippi byte-exact-roundtrip-validoitu). Committoimatta.

- [ ] **P1 · Laatupyramidin kalibrointiaalto** — ks. alempana säilytetty mittausanalyysi (2026-07-28-osion rivi "VAIHE 2 VERIFY-LÖYDÖS"): juurisyy LeagueGeneratorin absoluuttitaso (t=0-miss) + veteranPotential/developmentCeiling-yhdistelmä, EI kehityspino (career-harness konvergoi bandiin 30 kaudessa). Koskee intake-tasoa + ceiling-slopea + 18 career-asserttia yhtä aikaa → oma kalibrointiaalto. Luonteva hetki: vaihe 3:n realistisen aloitusliigan yhteydessä (LeagueGenerator korvautuu osin datavetoisella polulla joka kalibroidaan suoraan §8-bandeihin).
- [x] **P2 · Smoke-portin uusinta washout-fixin jälkeen: PASS** (2026-07-29, oma ajo): 3 kautta, drift +0,96 ≤ 1,5; moraali ka 72,9/p05 59; motivaatio 20/70/5/5; rosterit 53/53; pts 23,1–23,8; ei watchdogia. Watch-huomio: washout+eläköityminen poisti kaudella 3 yhteensä 519 pelaajaa (ka-OVR 70,8, ikä 25,9 — FA-poolin pohjan siivousta replacement-barin alta); pooli terve (181–412), mutta seuraa volyymiä laatupyramidin kalibrointiaallossa (mekanismit kytkeytyvät).
- [ ] **On-device-silmämäärä iPadillä (vaihe 1+2 yhdessä):** draft-luokka (PROD/LRN/CMP-sarakkeet, A-arvosanat, RB/S-kärki), PlayerDetailView (motivaatiobadge, Competitiveness, Coach's Projection), kehitysraportin uudet syyt (motivation/plateau/lateBloomer/schemeChange), scheme-vaihtouutinen. HUOM: sovellus kannattaa POISTAA simistä/iPadistä ennen asennusta (vanha fi.lproj jää muuten kummittelemaan).

## 🏈 DRAFT CLASS OVERHAUL SHIPATTU — seuranta & vaihe 2 — 2026-07-28

Koko draft-generointi uusittu NFL-realistiseksi (analyysi `docs/DRAFT_CLASS_OVERHAUL_PLAN.md`, data `docs/DRAFT_NFL_REFERENCE.md`, validointi `tools/balance-harness` `draftclass`-skenaario 31 asserttia / 200 luokkaa). Committoimatta — vaatii on-device-silmämäärän.

- [ ] **P1 · VAIHE 2: kehitys/harjoittelu-syväanalyysi** (käyttäjän seuraava pyyntö). Sisältö: taistelijamentaliteetti/competitiveness + vastoinkäymisistä motivoituminen (huono kausi → kovempi treeni), valmennuksen laadun syvempi rooli, `updatePotentialRealization` + `assessPotential` käyttöön (nyt dead code), oikea peliaikaosuus kehitykseen (nyt estimaatti vaikka `gamesPlayed/StartedThisSeason` trackataan), henkinen puoli NFL-vertailulla.
- [ ] **P1 · Liigan potentiaalitason ratchet pitkällä horisontilla**: intake-potentiaali ~79,5 (slot-korreloitu) > liigan lähtötaso → leaguePot nousee ~+0,9/kausi kohti intake-tasoa. 3 kauden gate PASS (Δ+1,00…+1,26 ≤ 1,5), mutta 5 kauden ajo vanhalla catch-up-säädöllä näytti Δ+2,4. Ratkaisu kuuluu vaihe 2:n kehitys/regressio-tasapainoon (intake-taso vs development vs regressio). Kalibrointiagentin top follow-up.
- [ ] **P1 · VAIHE 2 VERIFY-LÖYDÖS (vahvistettu mittauksella, EI korjattu tässä aallossa): laatupyramidi vs `DEVELOPMENT_NFL_REFERENCE.md` §8.** Mitattu `MultiSeasonSmokeTest` 3 kautta: baseline (t=0, pelkkä LeagueGenerator, ennen yhtään kehitysajoa) 90+ **3,5 %** [tavoite 1-2] · 80+ **37,7 %** [12-16] · 75+ **61,7 %** [30-40] · sub65 **9,8 %** [~25], keskiarvo 76,35. Kausi 3: 90+ 9,5 % · 80+ 45,6 % · 75+ 58,8 % · sub65 7,9 % · 33+ 5,1 % · ageMed 26, keskiarvo 77,38 (Δ+1,03 ≤ 1,5 PASS). **Kaksi erillistä syytä, kumpikaan ei ole vaihe 2:n catch-up-taulukko:** (a) §8:n bandit ovat ABSOLUUTTISIA OVR-rajoja liigalle jonka keskiarvo on ~72; shipattu LeagueGenerator tuottaa ~76,4 → miss on olemassa jo t=0:ssa; (b) huipun kiipeäminen (90+ 3,5→9,5 % kolmessa kaudessa) tulee siitä että `veteranPotential` (plan §2.7) takaa `pot ≥ overall` ja `developmentCeiling = 0,60·pot + 39` antaa ceilingin ~6-9 pistettä potentiaalin YLÄPUOLELLE koko liigan tyypillisellä välillä → jokainen generoitu veteraani aloittaa 6-9 OVR:n catch-up-varalla. **Vastanäyttö catch-up-taulukkoa vastaan:** sama kehityspino ajettuna 30 kautta pelkällä DraftClassBuilder-intakella (`./run.sh career`) päätyy 75+ **38,8 %** (bandissa) · 80+ 24,1 % · 90+ 4,1 % · 33+ 3,6 %, ka 73,71, drift +0,001/kausi — eli koodi konvergoi lähelle tavoitetta, lähtöpopulaatio ei ehdi kiertää 3-5 kaudessa. **Korjaus on kalibrointiaalto, ei triage-editti:** se koskee samanaikaisesti intake-potentiaalitasoa (drafted avgPot 81,7 @ avgOVR 59,2), `developmentCeiling`-slopea ja LeagueGeneratorin absoluuttista tasoa — ja jokainen näistä on suoraan sidottu 18/18 läpimenevään `career`-asserttiin (6.1 hit ratet, 6.2 elite-osuudet). Ks. myös rivin 10 skipattu löydös: sama `developmentCeiling`-slope.
- [ ] **P2 · Skipattu verify-löydös:** catch-up-fraktioiden puolitus (0.19→0.095 jne) hidastaa transientisti olemassa olevan saven yp0-4-pelaajia (joiden potentiaali generoitiin vanhalla mallilla). Poistuu ~4 offseasonissa; vaihtoehtona potentiaali-gated catch-up vaihe 2:ssa.
- [ ] **P2 · Skipattu verify-löydös:** plan §2/step-5 "starter-capable ceiling" -targetit (R1 90 / R3 55 / R7 25 %) eivät ole saavutettavissa nykyisellä talent-käyrällä (jokaisen draftatun ceiling ≥78 konstruktiona; mitattu 100/100/89,5 %). Vaatii design-päätöksen: käyrän häntä jyrkemmäksi slotin ~120 jälkeen TAI kynnys ylös TAI `developmentCeiling`-slope alas.
- [ ] **P3 · Pikkujutut:** `hometownState/City` dead fields (ei kirjoiteta koskaan); roster-listalta puuttuu Mental-analyysimoodi (LRN näkyy vain detaljissa); combine-drillien skip-mekaniikka ("DNP"); UserDefaults-prospektiarvosanat vuotavat careerien yli (`UserProspectGrade.swift:70-71`).
- [ ] **On-device-check iPadillä:** käynnissä olevassa pelissä migraatio purgettaa+regeneroi rikkinäisen luokan kun draft ei ole alkanut (scouting-progressi nollautuu siltä osin — luokka oli rikki). Silmämääräys: Prospects/Big Board/Combine PROD- ja LRN-sarakkeet, A-arvosanat kärjessä, RB/S-kärki R1-R2:ssa, K/P ≥ 3.

## ✅ PRE-SNAP-STANCE KORJATTU (regressio omasta snap-fixistä) — 2026-07-26 10:27

- **Oire:** pelaajat pystyssä pre-snap (ei aloitusasennossa); asento vain väläht?? snapissa. Pitäisi pysyä kunnes pallo lähtee C:ltä.
- **Juurisyy:** aiempi snap-romahdusfixini (setMoving: `leavingHeldPose ? fadeIn 0`) oli liian laaja — se osui SEKÄ snapiin (moving=true, nousu asennosta) ETTÄ idleen-menoon (moving=false, saapuessa muodostelmaan). Kun pelaaja saapui ja setMoving(false)→idle laukesi stance jo kiinnitettynä (mutta ei alkaneena beginTimen takia), idle **napsahti täydellä painolla stancen päälle** → pystyasento jäi päälle, stance näkyi vasta snapissa. Fix: fadeIn-0-vahti VAIN moving=true:hen (snap-nousu tarvitsee sen rest-pose-aukon välttämiseen; idle ei). Verifioitu simissä: OL kyykyssä pallon yli pre-snap, pysyy.
- Koppi (catch_a in-place) vahvistettu käyttäjän toimesta (2 drivea). Build 26.7. 10:27 + tuore save iPad Pro -simille.

## ✅✅ KOPPIKATOAMISEN AITO JUURISYY LÖYDETTY JA KORJATTU — catch_a:n root-offset — 2026-07-26 10:10

- **Käyttäjän ratkaiseva vihje:** vastaanottaja "menee katsomoon/lähemmäs 40:tä ja palaa" kopissa = POSITIOHYPPY, ei cull/sink. Ja container-POS oli puhdas → siirtymä on KLIPIN JUURILUUSSA, ei nodessa.
- **JUURISYY (Blenderissä todistettu):** catch_refresh-agentti retargetoi `PlayerClip_catch_a`:n **ILMAN --inplace**:a → sen `pose.bones["spine"].location` frame-0 = **(5.83, 4.68, 13.15)** eli juuri 14 yks sivussa + 4.7 ylös (näyttelijän maailmapaikka ZIG-takessa). reach() defaultaa catch_a:han → kickoff-vastaanottaja + syöttökopittajat soittaessaan sitä loikkaavat spinen tähän offsetiin → mesh ~13 jaardia sivuun/katsomoon, palaa kun klippi loppuu. **Vain catch_a rikki** (kaikki 10 muuta aallon klippiä tarkistettu, horiz~0). Alkoi aallosta koska catch_a tehtiin silloin — täsmää käyttäjän "alkoi animaatioparannuksista".
- **Miksi mittarini valehtelivat koko ajan:** container-positio (POS/teleport-tracer) ei näytä meshin luustolentoa; SINK näkisi pystyn muttei horisontaalia; VIS-cull-tracer oli sokea (±50 box). Ei mikään tracer mitannut oikeaa asiaa (meshin/luuston maailmapositiota).
- **KORJAUS:** rebase catch_a:n spine.location kaikki 3 akselia frame-0=0 (todellinen in-place, kuten run/catch_e). Verifioitu: offset 14.4→0, maadoitus 0.2 leijunta (mitätön). Shipattu Resources + build 26.7. 10:10 iPad Pro -simille + tuore save.
- **PIPELINE-OPPI (muistiin):** jokainen re-authored klippi TÄYTYY olla in-place ellei held ground-klippi; tarkista spine frame-0 offset ennen shippausta. Ks reference_clip_inplace_offset_pitfall.
- **JÄLJELLÄ:** käyttäjän varmistus (build 26.7. 10:10) — kickoff + heittopelit. Aiemmat revertit (play() delay) turhia mutta vaarattomia; setMoving fadeIn-0 + formationMove-fixit aitoja erillisiä korjauksia. Debug-tracerit jäävät buildiin.

## ⚠️ KATOAMINEN: MITTARIT OLIVAT SOKEITA KOPISSA → REVERT play()-VIIVÄSTYKSESTÄ — 2026-07-26 09:04

- **Käyttäjän avainvihje:** virheet alkoivat tämän päivän mocap-animaatioaallosta (committaamattomat muutokset vs 2f67eef); kaikki toimi ennen. Kickoffin vastaanottajan katoaminen kopissa on helpoin toistaa.
- **Miksi "Ennen 3, Jälkeen 0" oli väärin — mittareideni SOKEAT PISTEET kopissa:** (1) VIS-traceri testasi CONTAINER-nodea johon laitoin ±50 boxin (=aina "näkyvä") — SceneKit cullaa SKINNERIN luurangon boundsien perusteella → render voi cullata vaikka traceri sanoo näkyvä. (2) SINK-traceri oli gatettu `!isPosing`-ehdolla → kopin aikana (action-klippi soi) se EI lokittanut vajoamista. → mittarit sanoivat "puhdas" kun käyttäjä näki katoamisen. SINK-gate poistettu (näkee nyt posing-tilan aikaiset vajoamiset).
- **Juurisyyhypoteesi + fix:** play():n VIIVÄSTETTY klipin liittäminen (asyncAfter) oli tämän päivän muutos joka jättää action-avaimen TYHJÄKSI koko lennon ajan (kickoff landAfter ~2s); jos locossa on hetken aukko, luuranko putoaa lattia-rest-poseen → hahmo vajoaa turffin ALLE = "häviää ja palaa kopissa". REVERT toimivaan committiversioon: `beginTime`-liittäminen (klippi pysyy avaimella, loco pitää pystyssä kunnes beginTime). setMoving fadeIn-0-korjaus JÄÄ (erillinen, korjasi aidon snap-romahduksen).
- **EN saanut toistettua omalla similläni** (identtinen sim 049C, sama koodi) — 0 SINK/VIS jopa korjatuilla tracereilla. Syy siis revertin sulkemassa aukossa TAI ympäristössä (kaappaukseni missaavat lyhyen väläyksen). Build 26.7. 09:04 asennettu iPad Pro -simille + tuore save.
- **JÄLJELLÄ:** käyttäjä VARMISTAA valikon build-leiman (26.7. 09:04 tai uudempi) → testaa kickoff. Jos katoaa yhä, korjattu SINK-traceri nappaa sen käyttäjän lokiin (posing-lippu kertoo onko kopin aikana).

## ✅ KATOAMINEN NAULATTU KÄYTTÄJÄN SESSIODATALLA + KORJATTU LOPULLISESTI — 2026-07-26

- **Käyttäjän oma visdebug.log (kickoff + 3 pass):** kickoffissa 3 pelaajaa (node 13/0/14) VANISH **KESKELLÄ RUUTUA** (screen 916,1316 / 945,1296 / 376,1241) → aito frustum-cull, ei kameran ohipanorointi. Pass-plays gen 2-4: 0 keskiruudun vanishia (SINK-fix piti, syvin torso 0.22). TELE=0.
- **Miksi cullPad 2.5 ei riittänyt:** SceneKit cullaa SCNSkinnerin **luurangon bind-pose-boundsien** perusteella, ei mesh-noden boundingBoxin — 2.5m mesh-padi ei vaikuttanut cull-testiin. Keskiruudun pelaaja ei saa KOSKAAN culloutua → override ei purrut oikeaan nodeen.
- **Lopullinen fix:** ±50m boundingBox JOKAISEEN figuurin subtreen nodeen jota render-traversaali testaa — container + figure + skel.content (FootballFieldScene.makePlayerNode) JA skinnerNode + skeleton (SkeletalFigure.init). Cullaus käytännössä pois 22 low-poly-figuurilta = merkityksetön kustannus. **Verifioitu käyttäjän tismalleen samassa kickoff+pelivirrassa: keskiruudun VANISH 3 → 0 yli 11 pelin; lattiaromahdus 0; teleportti 0.** Ei visuaalista regressiota (varjot/mesh käyttävät oikeaa geometriaa).
- **Koko session katoamis-/romahdus-oireiden lopullinen tila (kaikki nolla mittarilla):** (1) snap-lyyhistys = setMoving fadeIn 0 held-posesta; (2) koppi pre-attach = play() viivästetty addAnimation; (3) frustum-cull = ±50m boundingBox; (4) formationMove-warppi = cancelPlay-poisto + etäisyysskaalaus; (5) kamera = broadcast-lentokehys. Tracerit (VIS/SINK/GROUND/TELEPORT → dynasty_visdebug.log) jäävät DEBUG-buildiin.
- iPad Pro -simi: korjattu build (26.7. 08:19) + tuore week-1-save + kello 10 s. JÄLJELLÄ: käyttäjän puhdas silmämäärä.

## ✅ "PELAAJAT MAAHAN SNAPISSA" + KOPPIKATOAMINEN — JUURISYY MITATTU JA KORJATTU (self-play-iterointi) — 2026-07-26 yö

- **Instrumentointi ratkaisi lopullisesti:** lisättiin SINK-tracer (spine_001-bonen world-Y render-silmukassa → Documents/dynasty_visdebug.log) GROUND+VIS+TELEPORT-tracereiden lisäksi. Ajettiin oma peli, mitattiin SINK snapin ympärillä.
- **JUURISYY:** `SkeletalFigure.setMoving(true)` snapissa poisti pidätetyn stance-klipin (blendOut 0.3) ja häivytti juoksuklipin sisään (fadeIn 0.32). Häivytysikkunassa oli AUKKO jossa kumpikaan klippi ei ollut täydellä painolla → SceneKit relaksoi luurangon rigin **rest-poseen, joka on LATTIATASOLLA (spine ≈ 0)** → pelaaja lyyhistyi vatsalleen snapissa ja nousi 0.6 s aikana. Mitattu: torso 0.01–0.06 m LOS:lla heti PLAY_STARTissa. Sama bind-pose-mekanismi kuin koppikatoamisessa (play() pre-attach) mutta stance→juoksu-siirtymässä.
- **Fix (3 iteraatiota SINK-mittarin ohjaamana):** stancesta/fallista poistuttaessa `setMoving` lisää juoksuklipin **fadeIn 0** (täysi paino heti) ja pudottaa poseen samalla hetkellä → rest-posea ei näy lainkaan. Loco↔loco (idle↔run) säilyttää pehmeän 0.32 s crossfaden. **Mitattu tulos: SINK 414 → 265 (fade 0.12) → ~0 lattiaromahdusta (fade 0.0)** yli 17 pelin. Jäljelle jää vain aidot taklauskasat (torso 0.09–0.14 alas kentällä, 3 pelaajaa yhtaikaa) ja puolustuslinjan matala lähtöasento (0.35–0.40, oikein).
- **Koppikatoaminen** jakaa saman juurisyyn (bind-pose-romahdus klippisiirtymissä) — play() pre-attach-fix (attach vasta alkuhetkellä, häivytys juoksuposesta) + tämä setMoving-fix kattavat molemmat oireet.
- **META-LÖYDÖS pysyi:** käyttäjä ajoi eilistä Xcode-buildia koko päivän (build-stamp alaviite lisätty valikkoon). Kaikki tracerit jäävät DEBUG-buildiin. Ympäristö: iPad Pro -sim (käyttäjän) sai korjatun buildin + tuoreen week-1-saven; playClock 10 s.
- **JÄLJELLÄ:** käyttäjän puhdas silmämäärä — erityisesti yksi OFFENSIIVINEN completion (QA-ajossa peli kääntyi puolustukseen eikä pakotettua completionia saatu, mutta objektiivimittari + jaettu juurisyy vahvistavat korjauksen).

## 🧨 KOKO PÄIVÄN "EI TOIMI" -MYSTEERIN JUURISYY: KÄYTTÄJÄ AJOI EILISTÄ BINÄÄRIÄ — 2026-07-25 klo 21:30

- **Todiste:** käyttäjän oman Xcode-DerivedDatan dynasty-binääri = **24.7. klo 18:16** — Xcode Run asensi sen simulaattoriin joka kerta → YKSIKÄÄN päivän ~10 korjauksesta ei ollut koskaan ajossa käyttäjän sessioissa (iPad Pro -kontin Documents tyhjä = QA-buildeilla ei pelattu; datakontti rotatoi Xcode-asennusten mukana). Kaikki päivän "ei toimi vieläkään" -raportit selittyvät tällä; QA:n instrumentoidut ajot (uusin koodi) olivat puhtaita.
- **Pysyvä korjaus sekaannukseen:** päävalikon alaviite näyttää nyt build-aikaleiman (binäärin mtime ajonaikaisesti): "v1.0 (1) · build 25.7. 21:27". Väärä buildi tunnistetaan yhdellä vilkaisulla.
- **Ympäristö siivottu:** Dynasty-R40-Smoke sammutettu (kahden identtisen sim-ikkunan sekaannus poissa); iPad Pro -simillä uusin build valikossa.
- Kaikki päivän korjaukset (koppikatoamisen pre-attach-fix, formationMove-warppi, kamera×2, kickoff/punt, animaatiot, cullPad) ovat buildissa 25.7. 21:27.

## 🎯 KOPPIKATOAMISEN JUURISYY NAULATTU KÄYTTÄJÄN SESSIODATALLA — 2026-07-25 yö (shipattu, committaamaton)

- **Näkyvyystallennin** (VIS APPEAR/VANISH ruutu+maailmakoordinaatein → Documents/dynasty_visdebug.log) ajettiin käyttäjän omassa sessiossa: palauttaja EI koskaan poistunut frustumista kopin ympärillä → ei cullausta → **luuranko lyyhistyy**. Mekanismi: `SkeletalFigure.play()` liitti klipin luurankoon HETI tulevalla `beginTime`llä — SceneKitin fadeInDuration alkaa LISÄYSHETKESTÄ, joten esiaktiivisessa ikkunassa luuranko blendautuu kohti bind-posea (T-pose content-offsetin takia puoliksi turffin sisässä) = pelaaja "katoaa". **Miksi vasta nyt:** vanhat beat/kesto-arvot pitivät startDelayn ~0.1 s:ssa (näkymätön); päivän uudet arvot (catch_a 0.85 s/beat 0.45, catch_e 1.4 s, kickoff-hang 2.4 s landAfter) kasvattivat ikkunan 1–2 sekuntiin → "vastaanottaja/palauttaja häviää hetkeksi ennen koppia".
- **Fix:** ei esiliitosta — viivästetty `addAnimation` vasta alkuhetkellä (DispatchQueue + `playToken`-vartija; token mitätöityy clearPose/fullReset-poluissa ettei kuollut koppi laukea seuraavassa pelissä).
- **HUOM: käyttäjä pelaa Dynasty-R40-Smoke-simillä** (ei iPad Pro:lla — lokit todistivat). Fiksattu build asennettu MOLEMMILLE simuille; molemmat päävalikossa.
- Tallentimet (VIS + TELEPORT + PLAY_START → visdebug.log) jäävät DEBUG-buildiin diagnoosia varten.

## 🔬 KOPPIHETKEN KATOAMISBUGI: SYVÄANALYYSI + FIX — 2026-07-25 yö (shipattu, committaamaton)

- **Oire:** pelaajat katoavat ruudusta juuri ennen koppia ja palaavat sen jälkeen (positiot jatkuvia — teleporttijäljitin hiljaa → renderöintibugi).
- **Syväanalyysi-workflow (repro + 2 koodilinssiä + adversariaalinen synteesi):** H2 (near-plane-leikkaus) KUMOTTU laskennallisesti (kamera ≥9 yd ylhäällä, pelaajat eivät koskaan ≤7 yd päässä near-planesta); H3 (aktiivinen piilotus) KUMOTTU tyhjentävästi (isHidden/opacity/LOD ei koske pelaajavartaloita missään). **H1 = ainoa mekanismi aidolle "kuvassa mutta ei piirretä" -katoamiselle:** SCNSkinner-meshit culletaan STAATTISELLA bind-pose-bounding-boxilla (repo: nolla boundingBox-overrideja, nolla frustum/culling-käsittelyä); in-place dive/catch-klipit vievät vartalon 1-2 m laatikon ulkopuolelle ja ground-clamp siirtää content.y:tä sen alla → frustumin reunalla koko mesh culletaan vaikka raaja on ruudussa; post-catch pull-back palauttaa. Tiukka uusi koppikehys työntää pelaajat juuri reunalle.
- **Fixit:** (1) `cullPad 2.5 m` boundingBox-override skinnerNodelle SkeletalFigure-initissä (no-regret; kustannus ~0); (2) lentokehyksen aim-bias 0.65→0.5 (marginaalipelaajat pysyvät kuvassa kopin läpi). Smoke-verifioitu: täydet muodostelmat renderöityvät läpi säkkien/FG:n/heittojen. Asennettu käyttäjän simille.
- Jos katoaminen jatkuu vielä: diskriminointitesti valmiina (cullPad=50 → jos katoaa yhä, kyse on puhtaasta kehystyksestä → FOV 52→58 + catchHold-trimmi).

## ✅ AITO TELEPORT LÖYDETTY JA KORJATTU: formationMove-warppi — 2026-07-25 myöhäisilta (shipattu, committaamaton)

- **Instrumentointi ratkaisi:** DEBUG-teleporttijäljitin render-silmukkaan (updateFootLocks → traceTeleports: >3 yd/frame → loki node+siirtymä+action-avaimet). Osumat nimesivät syyllisen suoraan: `keys=[formationMove+facing]`, 36 jaardia 0,15 s:ssa KESKEN pelin, pallo syvällä (= "teleportti ennen koppia").
- **Juurisyy 2-osainen:** (1) `cancelPlay` EI poistanut `formationMove`/`facing`-actioneita → pelien välinen lineup-siirto jäi henkiin snapin yli ja raahasi pelaajaa warp-vauhtia kunnes tämän oma play-liike korvasi sen — ja jäädytetyssä kickoff-lennossa/puntissa pelaaja ei SAA play-liikettä → raahaus koko lennon = "teleportti ennen koppia molemmissa". (2) Muodostelmasiirtojen kiinteä kesto (0.3–0.7 s) etäisyydestä riippumatta: possession-vaihdossa puolustus EI käy huddlessa → syncFieldToSituation veti sen 30+ jaardia 0,3 sekunnissa kentän poikki.
- **Fixit:** cancelPlay poistaa formationMove+facing; movePlayersToFormation + huddle etäisyysskaalaus (≤9 yd/s, katto 2.6 s), `scaleToDistance: false` aidoille teleport-synceille (pelin seed, replay-restage, kickoff-lineupin 0.1 s -stamppi).
- **Verifiointi:** identtinen save+auto-flow Smoke-simillä: ennen 11 osumaa / 7 peliä → **0 osumaa / 9 peliä** (sis. kickoff + possession-vaihdot). Korjattu build asennettu käyttäjän simille (appi oli dashboardilla). Jäljitin jää DEBUG-buildiin (hiljainen ellei teleportteja).
- Aiemmat "kamera + jaettu simulaattori" -löydökset olivat todellisia mutta OSASYITÄ — tämä oli se aito nodetason teleportti jonka käyttäjä näki puhtaassakin sessiossa.

## 🎬 ANIMAATIOAALTO: TACKLE/SACK/CATCH/JUKE MOCAP-ITEROINTI — 2026-07-25 (shipattu, committaamaton — vaatii on-device-silmämäärän)

- **Workflow-iterointi (25 agenttia: author → 2-linssinen adversariaalituomarointi → refine):** kaikki 7 klippiperhettä kandidaateiksi; 4 vaati refine-kierroksen. Lähteet fb21-mocap-pack (Rokoko-retarget Blender 4.2:lla) — Ochissa ei sack/juke-materiaalia.
- **Uudet klipit Resources:iin:** `sack_taken` (QB:n säkkiluhistuma paikaltaan, ei juoksuliukua; flush prone hold), `sack_make` (rusherin wrap-drive, hold:false — lähdemocapin takajalka ei koskaan maadoitu), `catch_e` (olan-yli-koppi punt-catch-mocapista → overShoulderReach käyttää), `juke_b`+`juke_c` (aidot vastakkaiskätiset zig-zag-leikkaukset; korvasivat Mixamo-väistön juke-poolissa).
- **Parannetut:** `tackle_a` (tiukka ikkuna [0.60,0.72]: gather→dive→land, beat 45 % keskellä; vanha = 60 % juoksua ja pike-jäätymä), `tackle_b` (koko takedown-rulla [0.548,1.0] + **arm-lift-korjaus**: riippuva käsi huijasi seat-metriikkaa ja vartalo leijui 0.3 yks turffin yllä — pelvis 0.59→0.28), `catch_a` (lähdevaihto zigzag-takeen: juoksukoppi + carry-jatko, ei enää käsivarsi-plateau'ta), `catch_b` (loppuhunch trimmattu [0.28,0.48]).
- **Swift-kytkennät:** SkeletalFigure: pool `sacked`/`sack`, juke-pool b+c, variantti-ensin actionTargetDuration + päivitetyt beat-fraktiot; FootballFieldScene: `PlayStep.fallStyles`, `FallStyle.sacked/.sackDrive`, overShoulderReach→catch_e; PlayChoreographer: sackSteps merkitsee QB `.sacked` + rusher `.sackDrive`.
- **Työkaluparannukset:** `strip_root.py --target Z` (ilmalähtöisten ikkunoiden maadoitus), `render_verify.py` end-pose-flat-metriikka/portti (`--end-flat-tol`) — nappasi tackle_b-leijunnan jonka molemmat visuaalituomarit missasivat.
- **Portit:** render_verify PASS kaikilla 4 maaklipillä (ground/descent/end-flat), sim-build vihreä. Fallbackit scratchpadissa (anim_v2/; tackle_a r8 = pidempi lead-in-variantti jos gather-crouch-siirtymä töksähtää laitteella).
- **JÄLJELLÄ:** käyttäjän silmämäärä coached-pelissä (erityisesti run→tackle_a-siirtymä ja sack-parin luenta) → commit.

## ⚠️ JUURISYYLÖYDÖS: JAETTU SIMULAATTORI — QA-AUTOMAATIO JA KÄYTTÄJÄN PELISESSIOT TÖRMÄSIVÄT (2026-07-25 ilta)

- **Aikajanatodiste (XcodeBuildMCP-runtimelokit):** käyttäjän klo 16.18–16.19 "massakaatuminen + WR ilmestyy" -screenshotit osuvat ikkunaan, jossa tutkintaworkflow'n repro-AGENTTI ajoi testisimiä (launch 16:09:58, puoliaika pelattu automaatiotapeilla 10 s kelloa vastaan, abandon ~16:58). Klo 17.24–17.25 "pallo turffilla + HUD jäässä 59 s" -sarjan KESKELLE (17:25:02 ja 17:25:11 väliin) osuu QA:n kylmä `launch-app`-resetti 17:25:04. Lisäksi playClockSetting flipattiin off→10 QA:n toimesta 17:25/17:38. → Päivän "toistuva poukkoilubugi" on suurelta osin kahden ohjaajan yhteentörmäystä samalla simillä, ei (pelkkä) pelikoodin vika.
- **Puhtaat yhden ohjaajan nauhoitteet** (kickoff, scrimmage, slant, Post 33 yd; 15+ peliä frame-stepattuna): liike jatkuvaa, ei nodehyppyjä, koppi kehyksessä. Aito ja korjattu visuaalivika = kameran pallonjahtipiiska (broadcast-lentokehys-fix).
- **Korjaava toimenpide:** QA-automaatio siirretty erilliselle simille **Dynasty-R40-Smoke (8C95404B-…)**, appi asennettu sinne; iPad Pro 13" -testisimiin ei enää kosketa automaatiolla kun käyttäjä pelaa. Testisim jätetty puhtaana (dashboard, kello 10 s, viikko pelattavissa).
- **JÄLJELLÄ:** käyttäjän PUHDAS verifiointiajo (kukaan muu ei aja simiä): jos teleport/massakaato/pallo-turffilla ilmenee vieläkin, screenshot + pelikellon aika → aito jahti jatkuu stall-vahdilla (locale-fix tehtävä: ps %cpu desimaalipilkku) omalla sim-kloonilla.

## 🔍 "TELEPORT"-TUTKINTA + BROADCAST-LENTOKEHYS — 2026-07-25 ilta (shipattu, committaamaton)

- **Tutkinta (workflow: 3 koodilinssiä + sim-repro 11 heittopelillä + adversariaalinen tuomari): pelaajat EIVÄT teleporttaa.** Frame-by-frame-repro: ei yhtään >5 yd nodehyppyä; kaikki liike jatkuvaa (run() = lineaarinen SCNAction.move). VAHVISTETTU pääsyy = follow-kamera jahtasi raakaa palloa per frame → syvä sivulle murtuva kaari panoroi ruudun niin lujaa, että tasaisesti juokseva vastaanottaja pyyhkäisee ruudun poikki / ulos ("teleportin" luku). Syvällä (~opp 15) näkynyt hahmo = ERI vastaanottaja täydellä spec-reitillään (vain kohde saa katkaistun reitin — koodifakta, PlayChoreographer L1037-1039); olan-yli-koppikäännös selittää "juoksee taaksepäin" -luvun kopissa.
- **Fix: broadcast-lentokehys.** runBallArc (apex>2 + live follow) asettaa `followFlightAimX/Until`; aim- ja chase-constraintit blendaavat lennon ajan 65 % laskeutumispisteeseen — kamera liukuu kiinniottoalueelle eikä piiskaa pallon perässä; kohdevastaanottaja pysyy kehyksessä. Verifioitu simissä (slant: vastaanottaja kehyksessä koko lennon, koppi keskialueella).
- **Kumotut hypoteesit kirjattu** (workflow-output): YAC-nytkähdys (sileä juoksu, vain 0.42 s odotus kopissa — pientä pacing-hiontaa varten), stale asyncAfter -reittijalat (latentti 0.15 s snap-rajalla — kovetus per-node-epochilla jos ilmenee), effectiveDuration vs startDelays -aukko (vain snap-askel), possession-flip-restamp (gated).
- **🐛 UUSI LÖYDÖS reprosta (oma buginsa, ei korjattu):** pallo-desync — yhdessä pelissä pallo jäi irralleen keskikentälle kantajan ollessa ~20 yd alempana. Vaatii oman repron (ballHandoffToken/detach-ajoitus?). Videot scratchpad teleport_repro/ + simqa/deeppass.mp4.
- **Massakaatuminen-screenshotit (16:18-16:19):** ei löytynyt mass-falls-mekanismia mistään koreografiapolusta (interception/fumble/TD tarkistettu); todennäköisin selitys = edellisen ison pelin pile+unstack-ikkuna luettuna "kesken pelin" (HUD flippaa ennen nousuja) TÄI sama kamera-artefakti. Seurataan lentokehys-fixin jälkeen — jos toistuu, oma tutkinta.

## 🎥 PRESENTAATIOAALTO 2: KAMERA + KICKOFF + PUNT — 2026-07-25 iltapäivä (shipattu, committaamaton)

- **Kamera-fix (käyttäjäpalaute "kääntyy ärsyttävän nopeasti keskelle taklauksessa"):** juurisyy follow-rigin aim-constraintissa — holdin `lateralFactor 0.25` VETI tähtäyksen kohti keskikenttää (0.25·ballX) heti taklauksessa. Nyt: hold jäädyttää sivuttaisseurannan paaluun (`lateralEase 0`), contact-holdit vapautuvat hitaasti (0.5 s lisäparkki + 1.0 s ramppi, rate-skaalattu; `followHoldReleasesSlowly`), ja between-plays-paluufokus 0.8→1.2 s. Verifioitu simissä: pile pysyy rajauksessa, hidas paluu.
- **Kickoff uusiksi (dynamic-kickoff-luenta):** coverage riviin vastaanottajan 40:lle + setup-zone 35/31 (5 jaardin rako), KUKAAN ei liiku potkun lennossa paitsi potkaisija+palauttaja → koppi → PURKAUS: 8 blokkiparia engageZ~28:ssa (blocks+blockStyles, ragged jitter), 2 vapaata middle-lane-juoksijaa = takaa-ajo, palauttajan weave-path (amplitudi skaalautuu matkaan) + ajastetut juke/spin/deadLeg-openField-liikkeet (uudet mocap-klipit), upback-escort, potkaisija = viimeinen mies. TD-variantissa takaa-ajo jää taakse. Touchback = jäädytetty lento + settle. **Verifioitu simissä videolta:** seinät jäässä → koppi → engagementit → flush-taklaus → "returned out to the 33".
- **Punt uusiksi:** gunnerit (roolit 7/8) karkaavat snapista, sisäkahdeksikko suojaa ensin (startDelay 0.55) ja irtoaa portaittain 10-16 yd taakse; palautusryhmän sisärusherit lukittuvat suojamuuriin (blocks: anchor/drive), 3 miestä peelaa walliin; palauttaja ottaa KORIKOPIN (catch_e `.overShoulder` — aiemmin ei koppianimaatiota lainkaan!), yksi juke-eskape → gunnerit käärivät (falls/wraps — aiemmin punt päättyi seisoskeluun ilman taklausta). Ei vielä katsottu livenä (vaatii 4. yrityksen pelissä).
- Sim-QA ajettu playbookilla: coached game avattu, kickoff + scrimmage-juoksu nauhoitettu ja ruudut arvioitu, peli hylätty puhtaasti (viikko pelattavissa). Videot scratchpadissa simqa/.
...
## 📊 KIERROS 5 VALMIS: VALIDOINTIKAMPANJA — VERDIKTI **CONCERNS** (2026-07-24, raportti docs/BALANCE_REPORT_2026-07.md)

- **Terve perusta (vahvistettu täyspeleissä):** ei dominanttia callia millään tierillä; positioarvokäyrät monotoniset (TE-kuolleisuus-väite kumottiin synteesin N=1500-uusinta-ajolla — TE elää, matala-arvoisin mutta monotoninen +11 win-swing); asymmetriset signatuurit uskottavat (shutdown-pari painaa syöttöjaardit); adaptaatio+heat terveet (heat = varianssia, ei ilmaista EV:tä).
- **🔴 P0-1: Täyspelien syöttö/pisteinflatio joka tierillä** — avg-avg: 33 pistettä / net-YPA 8.6 / 3rd-down 53 % (kaistat ~22 / 5.9-7.5 / 35-45). Syy = pelinvalinta täyspeleissä (play-tason kaistat pitivät!), ei attribuutit. Fix [L]: situationaalinen play-selection-retune → net-YPA ~7.2, re-run Sweep C.
- **🔴 P0-2: Talenttikäyrä liian jyrkkä ääripäissä** — eliitti vs heikko **100 % deterministinen** (+96.6 marginaali, max 136 pistettä!), good vs avg 98.6 %. Fix [L]: kompressio → 1 tier ≈ 70 %, 2 tieriä ≈ 85-90 %, ei koskaan deterministinen.
- **✅ P1 KORJATTU: pelikirja-cliff (verify PASS).** Todellinen juurisyy tarkempi kuin raportin väite: bust-TODENNÄKÖISYYS oli capattu molemmin puolin, mutta **LOPPUTULOS ei** — hyökkäysbusti = 0 jaardin incomplete (rajattu), puolustusbusti = taattu puhdas koppi ILMAN cappia joka ohitti INT/droppi-rullat (syvällä 20+ ilmajaardia → walk-in TD). Analyyttinen todistus: fam33-D syvä comp 0.501 = mitattu 50.3 %. **Fix:** `famDefBustYardCap` 18 yd + famCurve-kompressio (+0.03/−0.11 → +0.021/−0.02). Mitattu: fam100-vs-33 **74.1 %** (tavoite 70-80), fam66-vs-33 **63.7 %** (60-68), monotoninen ja loivasti konkaavi (cliff poissa); fam33-D-vuoto 516→444 syöttöjaardia (+59/peli isoloituna, oli +180); "100 vs 33 näkyy" säilyi (comp +10.3pt, +13.2 pistettä, 74 % voitot). Kaikki HEAD-kaistat pitävät; ei round-4-hunkkeja kosketttu.
- **🟡 Havainto (design-keskusteluun):** positioarvot OL ≥ QB (62.6 vs 53.6 win-swing) ja RB > WR (43.8 vs 35.4) — kääntää NFL-intuition; voi olla peliin sopiva valinta mutta kirjattu.

## 📊 KIERROS 5 (alkup. jono-merkintä)
...
- **Suunniteltu muoto:** (1) harness laajennetaan täysiin peleihin (GameSimulator; tier-rosterigeneraattori: eliitti 88-95 / hyvä 80-87 / keskitaso 70-79 / heikko 55-69 + epäsymmetriset: eliitti-O/heikko-D jne.); (2) rinnakkaiset sweepit: per-play-family × tier-matriisit; **per-pelipaikka-herkkyys** (yksi positioryhmä kerrallaan sweepattuna → position arvokäyrä: paljonko QB/OL/CB/RB-tier liikuttaa lopputulosta); täyspelikampanja N peliä/paritus → **lopputulosjakaumat + box-score-statsit vs NFL-kaistat** (pisteet ~17-27/joukkue, kokonaisjaardit 300-400, pass 200-250, rush 100-130, sackit 2-3/peli, INT ~1, ~63 playta/joukkue); hot/cold+mielentila-efektimittaus (form-herkät vs immuunit arkkityyppirosterit); pelikirjatuntemus 33% vs 100% head-to-head-peleinä; (3) synteesi balance-check-skillin formaatilla → `docs/BALANCE_REPORT_2026-07.md` + TODO-itemit poikkeamista.

## 🔥 KIERROS 4 VALMIS: TULI/KYLMÄ + MIELENTILAT — 2026-07-24 (2× PASS_WITH_NOTES, BUILD SUCCEEDED)

- **HeatState.swift (uusi):** jaettu per-pelaaja-forme molempiin engineihin — live syöttää per-taistelu (MatchupResolver-eventit), sim per-drive (PlayResult: iso peli/TD lämmittää, droppi/INT/fumble/sacki jäähdyttää; juoksusyöttö bidirektionaalinen ~0-keskiarvoon). Keskitetyt vakiot (winStep 0.34, bigStep 0.50, turnoverStep 0.55, decay 0.80/drive, hot-kynnys ±0.5) → drift-proof. Vanha flat ±2-nudge poistettu; efektit kierros 3:n komposiitteihin arkkityyppiskaalattuna: **sensitive 1.0 / neutral 0.60 / immuuni 0.0 (mitattu 0.61/0.0)**; kuuma +3.3-3.7pp comp / kylmä −3.9pp / juoksu ±0.2 ypc — cappien alla (mentalCompletionCap 0.10, mentalRunYardCap 0.60). Neutraaliparity: kaikki round-4-termit täsmälleen 0 @ heat=0 (byte-parity). Composure/clutch pakkotilanteisiin molemmissa engineissä; heat→morale ±2 olemassa olevaa polkua pitkin (ei skeemaa). SIM-ego heat-kanavan kautta.
- **UI:** GamePlanView "Mental Readiness" -paneeli (temperamenttichipit, morale-barit, "Get him the ball early" -vihjeet egotähdille; CareerShellView populoi avainpelaajista); vanhat form-ikonit/badget lukevat uutta HeatStatea.
- **Verifiointi repo-harnessilla** (SHA-manifestit riippumattomasti uudelleenlaskettu — stale-ansa rakenteellisesti poissa): kaikki ~18 HEAD-kaistaa pitävät; ainoa reunahuomio sackit 8.75% vs 8.7-katto = pre-existing, EI round-4:n aiheuttama (mental-termit eivät koske sack-polkua). Heat-trajektorit designin mukaiset (2 voittoa = +0.68 ohi hot-kynnyksen; decay ×0.8/drive; immuunit flat 0.0).

## 🔥 KIERROS 4 (alkup. jono-merkintä) — 2026-07-24

- **Käyttäjän lisäys kierroksen 3 päälle:** hot/cold-streak-konsepti (liekeissä oleva pelaaja ylisuorittaa hetken, kylmä alisuorittaa) + pelaajien mielentilat resoluutioon. **Scout ajossa** (read-only, rinnan kierroksen 3 kanssa): MentalAttributes-kentät, PersonalityArchetype-vaikutukset, team-morale/kemia → SimPlayer-polku, olemassa oleva momentum-termi, PlayResult-attribuutio (kuka droppasi/päästi sackin = heat-up/cool-down-hookit), UI-pinnat (hot/cold-badge pelaajachippeihin). **Workflow käynnistetään kun kierros 3 laskeutuu** (sama modifier-pino, clamp-järjestys ja grand capit — suunniteltava yhdessä 3:n kanssa laskeutuneen kehikon päälle). Rajoite: ei uutta Codablea → in-game hot/cold per-peli-muistissa; viikkotason forme vain jos olemassa oleva tilastomalli kantaa sen.

## 🧠 KIERROS 2: AI-VALMENTAJIEN PLAY-TASON OPPIMINEN + TALENTTIVETOINEN SYVÄ PALLO — 2026-07-24 (workflow ajossa)

- **Käyttäjän vaatimukset:** (1) AI-valmentajat oppivat vastustajan pelivalintoja pelin edetessä — SAMA peli ei saa toimia koko ajan (esim. juoksuspam → lisää miehiä boxiin); (2) PA-deep-korjaus talenttivetoisesti: syvän pelin pitää riippua QB:stä ja laitahyökkääjistä — huippu-QB + huippu-deep-threat → selvästi enemmän TD:itä kuin heikoilla.
- **Workflow (design ×2 rinnan → toteutus sarjassa → riippumaton tilasto + adversariaali):**
  - **PlayMemory** (laajentaa RunKeyStatea): kategoria-EWMAt (inside/edge/screen/short/mid/deep/PA per tilannebucket) + **täsmälleen-saman-callin toistomuisti** (nopeampi+kovempi rangaistus kuin kategoriatendenssi, decay vaihdon jälkeen), persona-kohtainen oppimisnopeus, vastapainot (antisipaatio avaa jotain muuta), read-linet ("BAL is sitting on the toss sweep"), sama komponentti coached+sim (AI-pelitkin diversifioituvat).
  - **Deep-talent**: liiga-keskiarvo kaistalle (syvä comp ~35-42% @70/70, keyattu PA-deep 25.5→9-12 EV) + talenttikäyrä QB-deep-skill × WR-deep-threat -kompositiolla vs DB — eliitti 90/90: comp ~50-55% + TD-rate ≥2× heikko; 55/55 ~22-28%; 90 QB + 60 WR väliin; keyatun boxin rangaistus skaalautuu samalla kompositiolla (eliittiduoa vastaan EI kannata keyata → emergentti valmennusrealismi).
  - **✅ VALMIS + 2× VERIFIOITU (PASS_WITH_NOTES ×2, BUILD SUCCEEDED, asennettu simiin).**
  - **PlayMemory** (AdaptiveOpponentAI:475+): kategoria-EWMAt (7 konseptia, down-bucketit, uniform-prior) + 4-leveä exact-call-ring + persona-timing (catPivot/catAlpha thresholdOffsetista). Mitattu: goRoute-spam 8.5→6.7→3.3 EV, insideRun-spam 4.7→3.7→2.4→2.2; **täysi palautuminen** 3 muun konseptin jälkeen (8.52 vs baseline 8.48); exact-spam (comp 62.5→40.1) rankaistaan kovemmin kuin kategoria (→50.4); **mixed 12-callin skripti −1.15%** (tavoite ≤5% = monipuolinen kutsuja ei kärsi). Threading: PlaySimulator identity-default-parametrit (0 = byte-identtinen), LiveGameEngine latchit + read-linet, sim-polku.
  - **Deep-talent** (PlaySimulator + AdaptiveOpponentAI): komposiitti QB(.55 accDeep/.45 arm) × WR(.50 speed/.30 RR/.20 spectacular) vs DB(.55 cov/.45 speed); deepBase 0.53, slopet 0.006/0.0095; paKey rebase 0.18→0.035, 8.0→2.0 × deepPunishScale. Mitattu tier-matriisi MONOTONINEN molemmilla akseleilla: comp 26.6% (55/55) → 38.6 (70/70) → 47.6 (85/85) → 53.9 (95/95); **TD%/syvä yritys: heikko 3.1 / avg 9.0 / eliitti 25.7 = 8.4×** (käyttäjän vaatimus "selvästi enemmän TD:itä" ✓); 95QB/55WR = 44.9% (välissä ✓); DB-akseli regressoi (95/95 vs DB90: 42.1%). **Keyattu PA-deep @70/70 = 11.25 EV (oli 25.5 — kuollut ✓)**; punish skaalautuu: heikko +1.7 / avg +2.7 / eliitti +5.0 (keyed EV 20.6 → eliittiduoa vastaan EI kannata pinota boxia = emergentti realismi).
  - **Riippumaton verifioija LÖYSI + KORJASI harness-defektin:** vanhan harnessin AdaptiveOpponentAI-extractissa oli stale-oletukset (paKey 0.18/8.0) jotka olisivat VALMISTANEET 25.5-hotspotin artefaktina — mittaukset ajettiin byte-verifioiduilla repo-arvoilla. Regressiovahti: kaikki eilisen kaistat pitävät (ypc 4.45, curve 62/56/38, YPA 6.42, INT 2.32%; sackit 8.7% = 0.2pt yli kaistan, note ei fail; all-run TD 2.7% full-enginellä).
  - ⏳ Pienet notet: blended sack ~8.7% (voisi trimmata deep-sackia); harness-extractin oletusarvot synkattava jatkossa repo-muutosten mukana (defekti dokumentoitu).

## ⚖️ PELIBALANSSI: JUOKSU vs HEITTO — 2026-07-23 (✅ VALMIS — ks. alla)

- **Käyttäjän raportti:** juoksupeli tuntuu selvästi heittoa tehokkaammalta — TD-drive pelkällä 2 juoksupelin vuorottelulla; puolustus ei reagoi vaikka hyökkäys juoksee joka pelin. VÄHINTÄÄN adaptaatio tarvitaan.
- **Workflow (diagnoosi+design ENNEN engine-muutoksia):** (1) analyyttinen EV-audit — kaikkien play callien odotusarvot/jakaumat/TO-riskit suoraan PlaySimulatorin kaavoista + NFL-baseline-vertailu (4.3-4.5 ypc, 6.5-7.5 net ypa, ~64% comp); (2) puolustus-AI-trace — miten AI valitsee callin coached+sim-peleissä, onko MITÄÄN tendenssiseurantaa, adaptaatiohookit; (3) empiirinen batch-simulaatio /tmp-harnessilla jos PlaySimulator on SwiftData-vapaa (≥2000 playta/call + degeneraatti all-run-skenaario); (4) design-synteesi balance-check-skillin formaatilla: per-peli EWMA-tendenssitracker → def-call-bias + rajattu in-play-modifier (box stack nerffaa juoksut MUTTA play-action-deep rankaisee = kivi-sakset-paperi, ei pelkkä nerf), sama mekanismi coached+sim, EI Codable-muutoksia, "Reads: BAL is keying the run" -UI-vihje. Tavoite: all-run → ~3 ypc ~6-8 peräkkäisen juoksun jälkeen.
- **✅ AUDIT VALMIS — CRITICAL:** empiirinen 5000 rolls/call vahvisti: juoksut 6.6-7.3 EV / 99-100% succ / p10=+4..+5 / 0% sack; syötöt slant **-0.5 EV** / ~10% succ / 24-28% sack. Juurisyyt: (1) juoksun kovakoodattu pohja `random(2.0...5.0)` = ei negatiivista häntää (NFL: 16-19% stuffattuja), (2) completion ~37% (NFL 62-66) + 3× sackit + 2× INT + depth-flat-bugi (syvä onnistuu kuin slantti), (3) **AdaptiveOpponentAI on olemassa ja tunnistaa tendenssin ja kutsuu bear frontin — mutta koko efekti = `runStop×6.0` pyöristettynä = tasan −1 jaardi** ("mekaanisesti inertti"); 2 eri kategorian juoksun vuorottelu alittaa 0.40-kynnyksen = käyttäjän exploit. Design: `/tmp/balance_design.md` (203 riviä; kerros A adaptaatio+RPS, kerros B NFL-kaistaviritys, prioriteettitaulukko, edit-site-lista).
- **✅ KOKO PAKETTI TOTEUTETTU + 2× VERIFIOITU (PASS_WITH_NOTES molemmat, BUILD SUCCEEDED, asennettu simiin).** Layer B (PlaySimulator 139 riviä + PlayCall 4): B1a-d juoksun negatiivinen häntä (visio/elus keskitetty @70, base 2.0...6.5, stuff-rulla 15-16%), B2 runStop-bite ×15 (bear ~2.1 yd), B3 sackit 24→6.7-8.1%, B4 completion 24→62% short, B5 depth-käyrä 62>56>48, B6 INT 2.14%, B7 trench-starterit. Layer A (AdaptiveOpponentAI+PlaySimulator+LiveGameEngine): RunKeyState EWMA (α=0.30, pivot 0.65 — dialattu harnessilla designin 0.60:sta; yardBite ×1.1, stuffBonus ×0.08), PA-punish (comp +0.18i, bigPlay ×8i, bite→0.9), Bear-front näkyviin kun keyattu, hint ~3-4 juoksun kohdalla. **Riippumaton tilastoverifiointi (harness byte-identtisillä lähteillä, 5000-8000 rolls):** blended 4.47 ypc / net YPA 6.94 / INT 2.14% / qbSneak+dive 8.7→5.1 (ei enää win-buttoneita; kaikkien callien succ% 41.5-75.0). **Degeneraatti all-run: TD% 35.8→6.6-7.4** (never-punt-config = käyttäjän raportti; 20-drive-testi: 1 TD / 19 downs-käännytystä), ypc ≤3.0 jo 4-6 juoksulla. Balanced 55/45 ei kärsi (Δ −0.22 ypc). Kooditarkastus: save-safe (ei Codablea), molemmat polut, ei main-thread-SwiftData:a, boundit OK.
- **⏳ SEURAAVA VIRITYSKIERROS (ei-blokkaava, verifioijien flagit):** (1) **keyattu PA-deep EV 25.5 (design-tavoite 9-11) — kuumin calli nyt**; itserajoittuva (6 stuffattua juoksua hintana, EWMA romahtaa yhdestä syötöstä) ja boom/bust (11% sack, 4% INT, p10 negatiivinen) eli EI vanhan juoksun kaltainen riskitön dominantti — mutta syvän pallon air-yards/completion (47.7% @ 20+ ilmajaardia) on seuraava säätökohde; (2) lyhyen syötön comp +3-4 pistettä → blended 58.4→60-66-kaistalle; (3) stuff-base ylös jos halutaan tiukka ≤0-stuff 14-19%.

## 🏃 SÄKKIHIIPPAILY — KORJATTU (2026-07-23 ilta, shed-then-burst + shallow-fix)

- **Juurisyy (mitattu):** rusherin koko LOS→säkki-matka oli YKSI vakiovauhtinen siirto koko ikkunan yli → 2.0-3.4 yd/s (≈40-55% DL:n huippuvauhdista), ja vauhtisynkattu askellus toisti sen kävelynä (k≈0.63-1.05 kun muut k≈2-2.8). Sama mekanismi FIX-2:n lavastetussa paineessa.
- **Fix (workflow, PASS_WITH_NOTES):** shed-then-burst — rusher GRINDAA blokissa engage-pisteessä (LOS-syvyys max(sackDepth-5.5, 1.4)) ja PURSKAHTAA viimeiset ~5.5 jaardia 0.8s:ssa (~7 yd/s, k≈2.2); bury-beat ennallaan. Sama profiili pressured-incompletionille (burst 4.4-5.1 yd/s, cap QB:n droppisyvyydestä). Mirror-turvallinen (lerp jo-peilatuista pisteistä).
- **Residuaali korjattu käsin (BUILD SUCCEEDED):** verifioija löysi että flat burstReach=5.5 jätti matalat säkit (sackYards 3-4 = 1/3 säkeistä, `random(3...8)`) hiipimään (2.3-3.4 yd/s). Fix: `burstDur = clamp(burstDist/7.0, 0.35, 0.8)` + säästö grind-vaiheeseen (`grindDur = rushTime + (0.8 - burstDur)`) → burst ~7 yd/s aina kun tilaa on, kokonaiskesto identtinen. PlayChoreographer:2454-2497.

## ✅ LOPPUVERIFIOINTI 2026-07-23 myöhäisilta — 12 PASS / 3 PARTIAL / 0 FAIL (frame-analyysi, uusi buildi)

Uusintatallennukset `/tmp/video_verify2_2026-07-23/` (kickoff 39 f + AITO Outside Run 28 f + Flood 32 f; **kortti-koordinaattioppi: Run-gridin keskipisteet idb-x = 112/314/516/718/920** — 446 osui Counteriin kahdesti). Verdiktit: kickoff-kaari+kamera PASS, elävä palautus taklauksineen PASS, QB-heitto pystyssä PASS, launch kädestä PASS, ball-in-hand PASS, mesh-vaihto PASS (pallo näkyy käsien välissä f006), mesh-luettavuus PASS (hold minimissä), ei-variksenpelätintä PASS, pass-mappaus PASS (paine näkyy: Cole Moore murtautuu ja heitto lähtee ohi avoimeen), offense-stancet PASS, outside-polku PASS, taklaus PASS.

**PARTIALIT + uudet residuaalit (seuraava korjauskierros):**
- **[MEDIUM] FIX-1-residuaali: kickoffin HUD-chipit vuotavat** — ylächipit "1st & 10 · OWN 22 · BUF ball" näkyvät jo f009 (ennen potkua) vaikka narraatio/score/playbook on gatettu oikein (f025); sama gating myös situation-chipeille+possession-pillille kickoffissa.
- **Kicker-approach PARTIAL:** pallo ei enää ilmesty tyhjästä, mutta luettavaa askel+potkuliikettä ei ole (polish, liittyy potkuclipin puutteeseen).
- **LB "kädet vapaana" PARTIAL/epäjohdonmukainen:** deep-pass-kulmasta kädet vapaana (PASS), run-kulmasta lukevat polvien PÄÄLLÄ — selvitä kulmailluusio vs. wiring; tarvittaessa HAND_FWD 0.15→~0.25 stanceLB:hen. CB:t PASS molemmissa, LB↔CB-siluettiero PASS.
- Pienet uudet: mesh-hold minimissä (0.5s → harkitse 0.75s); QB+rusher interpenetroituvat flushissa; kickoff-apexissa ~40% ruudusta mustaa taivasta (framing); kopin hetki ei näy (jatkuvuusaukko); syvän pallon kaari matala (spot-checkaa puhdas syvä koppi); coverage-jono yksirivinen (locomotion-rajoite).

## 🛡️ PUOLUSTAJIEN STANCET — 2026-07-23 ilta (✅ VALMIS: authoroitu + wirattu + verifioitu)

- **Käyttäjän havainto (referenssit image-cache/8.png OLB/MLB, 9.png CB):** puolustajien asennot huonot (DL ok). Juurisyy mappauksessa: CB:t käyttävät WR:n release-asentoa (stanceSplit), LB/S vanhaa kädet-polvilla-stance2:ta. **Authorointi ajossa:** `PlayerClip_stanceLB.usdc` (atleettinen two-point: polvet 40-50°, kädet valmiina reisien EDESSÄ ei polvilla, rinta ylhäällä — kuva 8) + `PlayerClip_stanceCB.usdc` (matala coil: polvet 55-70°, peppu alhaalla, kädet rentoina roikkuen, kuva 9); front-view-renderit pakollisia (pelaajakamera näkee puolustajat edestä). **Wiring JONOSSA top5-workflow'n jälkeen** (samat Swift-tiedostot): uudet caset .linebacker/.cornerback; roolit LB 4-6 → stanceLB, CB 7-8 → stanceCB, S 9-10 → stanceLB; DL ennallaan.

## 🎬 VIDEO-FRAME-QA — 2026-07-23 (kickoff+Counter+Flood, 1 frame/0.5s, 6 linssiä + adjudikoitu synteesi)

Videot+framet: `/tmp/video_analysis_2026-07-23/`. Fiksien verdiktit: **stancet LANDED** (ei T-poseja/ristijalkoja/uppoamisia), **pallo-pin LANDED** (pallo QB:llä dropbackissa, kaari lähtee kehosta, ei parkkia), **mesh-käsinvaihto OSITTAIN** (pallon jatkuvuus toimii; kahden miehen vaihto EI lueteltavissa — kamera leikkaa pois f007 ennen vaihtoa). HUOM: "juoksupeli" oli oikeasti COUNTER (tap osui väärään korttiin) — Outside Runin ulkomurto verifioimatta, uusinta-capture tarvitaan.

**Kriittiset:**
- **P1 — Throwaway animoituu tarkkana syöttönä peitetylle WR:lle** (pass f013-f016): "throws it away" -tulos, mutta pallo lentää 24 yd suoraan käsiin DB selässä, katoaa incompleteksi. Heitto pois = avoin ruoho/OOB. (choreography+engine-pres)
- **K1 — Touchback lavastetaan elävänä palautuksena** (kickoff f018-f022): palauttaja ottaa pallon maalialueella ja jää seisomaan, coverage tulvii kentälle EIKÄ kukaan taklaa; banneri sanoo Touchback. (choreography+engine-pres)
- **K2 — Kickoffin pallon lentorata inkoherentti** (f012-f019): pallo hyppii epämonotonisesti päästä päähän, kamera ei seuraa. (choreography+scene)

**Majorit:** X1 — tulos/HUD/narraatio spoilataan ENNEN animaatiota (systeeminen, kaikki 3 clippiä; kello jäässä); P2 — "under pressure" -narraatio puhtaassa taskussa (ei painetta ruudulla); R1 — mesh-vaihto ei näy (kamera leikkaa); G1 — QB:n aktiopoosit rikki: heiton windup = 90° etukumara + recoil, handoff-saatto = "variksenpelätin" (clipit, EI pin-vika).

**Minorit:** K3 potkaisijalla ei potkuanimaatiota; R2 backside-WR ei stalk-blokkaa; C1 kameraleikkaukset choppyja; Q1 capture-alku sisälsi 9 framea menuja (harness).

**TOP-5 seuraavat fixit (vaikutus/työ):** (1) [S-M] Gate tulos/HUD kunnes animaatio valmis — de-spoilaa kaiken; (2) [M] pass-tulos→animaatio-mappaus (throwaway ulos, pressure vain kun rusher voittaa blokin); (3) [M] touchback-koreografia + kickoff-kaari+kamera; (4) [S] vaihda 2 rikkinäistä QB-clippiä (21-pakissa 5 QB-heittoa valmiina!); (5) [S-M] mesh-vaihdon luettavuus (pidä zoom vaihdon yli, QB ojentaa näkyvästi).

Synteesi refutoi itse 5 väärää väitettä frame-uudelleenluvulla (mm. "kaksi palloa", "coverage-klimppi") — raportissa vain kestäneet löydökset.

## 🚀 PARANNUSSUUNNITELMAN TOTEUTUS — 2026-07-23 (3 agenttia, kaikki valmiit)

Suunnitelma: `docs/3D_PIPELINE_SKILLS_BASELINE.md`. Tulokset:
- **✅ #1 Sim-QA → XcodeBuildMCP (semanttinen automaatio TOIMII):** `snapshot-ui` paljastaa koko SwiftUI-AX-puun (dashboard 449 elementtiä; tap labelilla: "Coach the Game", pelikortit nimillä, SNAP/REVERSE). Koko coach-flow ajettu 100% semanttisesti, nolla koordinaattimatikkaa. **Uusi skilli `.claude/skills/dynasty-sim-qa/`** — jokainen resepti oikeasti ajettu; sudenkuopat: snapshot+tap SAMASSA shell-blokissa (TTL), label-match `head -1` (puu sisältää koko nav-stackin). idb ×0.688 = enää fallback. Muisti `project_sim_automation` päivitetty osoittamaan skilliin.
- **✅ #3 Render-harness:** uusi `tools/asset-pipeline/render_verify.py` — kontaktiarkki (sivu + 3/4, 5 framea) + koneluettava JSON-check-blokki + exit-koodit. Validoitu: run/tackle_a/stance3/fall_back PASS; tahallinen väärinlippaus FAIL+exit 1 (check laukeaa); dive `--fall` FAIL oikein (aito hyppy +0.84 → älä lippaa loikkia). Dokumentoitu `blender-game-asset-pipeline`-skillin referenssiin. USD-quirkit työkalun headerissa (armature-haku tyypillä, per-frame-kameraseuranta, turf-plane EI mukaan framing-bboxiin).
- **✅ #4 Joukkueväri-tint (worktreessä, EI vielä mergessä):** `SkeletalFigure.applyTeamColors` — `.surface`-shader-modifier UV-tile-maskilla (atlas = 8 tileä; tile 1 = iho = ainoa kova exclude; 2-5 → primary, 6-7 → secondary), luminanssi-säilyttävä recolor (kankaan varjostus + AO säilyvät), 0 lisäsamplea/0 lisäpasseja (TBDR-halpa), värit KVC:llä (`jerseyColor`/`pantsColor` SIMD3). Team-värit virtaavat jo valmiiksi (`makePlayerNode` → `Uniform` → `SkeletalFigure(jersey:pants:)`). BUILD SUCCEEDED + visuaalinen verifiointi offscreen-macOS-SceneKit-harnessilla (home punainen / away valkoinen-navy, iho koskematon). **Rajoite:** Ochi-atlas on ei-semanttinen (tile 5 = kypärä+reidet+sukat fuusiossa) → kypärä saa primary-värin, puhdas kypärä-exclude ei mahdollinen tällä assetilla. **Worktree:** `.claude/worktrees/agent-a6152ef59de58bb7a` (vain SkeletalFigure.swift, 95+/27- — FootballFieldScene koskematon → merge triviaali). HUOM: worktree-branch resetoitu feature-tippiin (9e9f4f2) koska EnterWorktree loi sen vanhasta commitista.
  - **Numero-ongelma (selvitys):** baked "10" on Ochi-PNG:issä (tile 4 rinta/selkä + tile 5 reisitagi), EI player_mesh_v2:n tuottama → poisto = suora kuvamuokkaus `uniform_0…5.png`:iin. **Ilmainen välivoitto:** luminanssi-tint demotoi "10":n matalakontrastiseksi haamuksi. **Suositus (M):** luu-sidottu SCNPlane-dekaali rinta+selkä (`spine_001`-lapsi, per-pelaaja-UIImage) + baked-numeron pyyhintä PNG:istä; billboard-kirkastus = halpa välivaihtoehto. Speksattu agentin raportissa, ei toteutettu.
- **⏳ SEURAAVAKSI (suunnitelmasta):** [M] Meshy-retexture-uniformit (vaatii MESHY_API_KEY), [M] numero-dekaali, [L] locomotion-blend-setti (odottaa committia + RealityKit-päätöstä). Tint-worktreen merge kun Plan A/B+stancet on committoitu.

## 🏈 PLAN A: KENTTÄ↔KORTTI-PEILAUS — sessio 2026-07-22 (myöhäisilta) — TOTEUTETTU + SIM-VERIFIOITU (EI committia)

Suunnitelma: `docs/PLAN_play_direction_and_reverse.md` (Plan A, Option B). Kaikki presentaatiota — engine/Codable koskematta.

- **✅ Merkki-testi AJETTU ENSIN (suunnitelman §Verification 1):** HOME-coachattu (BUF vs BAL), Toss Sweep → RB pyyhkäisi ruudun VASEMMALLE kun kortin nuoli osoitti OIKEALLE = peilikuva vahvistui livenä, ennusteen mukaisesti → **`lateralSign = -viewFacing` on oikea merkki** (ei `+viewFacing`). Todisteet: `~/.claude/jobs/9bca1305/tmp/signtest/` (r02 = "1ST & 10 · TOSS SWEEP" -banneri + pitch vasemmalle, r05 = kantti vasemmalla laidalla).
- **✅ Toteutus (`FootballFieldScene.swift`, kaikki 7 suppiloa + valinnainen):** (1) `lateralSign`-property + asetus `setViewFacing`issa, (2) `positionPlayers` home+away, (3) `movePlayersToFormation`, (4) `huddle` (siirto + `atan2`-turn-in peilatulla X:llä), (5) `execute(step:)` movet, (6) polut (`points.map`), (7) pallon `.arc`+`.slide` — sekä sideline-replay-kameran laita (`-27.5 * lateralSign`). EI peilausta `run(node:to:)`-tasolla (read-back-koordinaatit) eikä `moveBall`issa (aina x=0). Replay-polku kulkee samojen suppiloiden läpi → konsistentti. `animatePlay` = kuollut API (0 kutsujaa), ei kosketttu.
- **✅ Re-verifioitu fixin jälkeen (uusi buildi simissä):** sama HOME Toss Sweep → RB seisoo nyt QB:n OIKEALLA puolella esikatselussa (kortin mukaisesti) ja sweep + pitch menevät OIKEALLE = kenttä täsmää korttiin; 8 yd juoksu, kamera seuraa oikealle. Quick Out -muodostelma kortin mukainen, ei desync-merkkejä (OL/def linjassa). Edit 7 verifioitu toss-pitchillä (kaari seurasi RB:tä oikealle).
- **✅ Asymmetriset spot-checkit AJETTU (2026-07-23, agentti, HOME-peli):** Outside Run vasemmalle **PASS** (kenttä täsmää korttiin, kamera panoroi mukana), Flood oikealle **PASS** — pallon lentokaari oikean laidan resveille = **edit 7 vahvistettu syvällä syötöllä**. Counter/Flat/Corner UNCLEAR vain pelin luonteen takia (keskijuoksu / QB improvisoi ohi kortin primäärin / paineheitto pois) — EI koskaan mirror-vian takia; ei desynciä missään 5 pelissä. Call sheetillä ei Jet Sweep/WR Screen -pelejä → korvattu Outside Run + Flat. **Replay-sideline PASS** (Sideline/End zone/Skip -valikko; järkevä kiinteä matala kulma, ei outoa peilausta; paluu broadcast-framingiin vakaana). Peli hylättiin → viikko yhä pelattavissa. Screenshotit: `/tmp/plan_a_verify_2026-07-23/`.
- **⏳ AUKI — loppuverifioinnit:**
  - **Away-peli:** rakenteellisesti no-op (`lateralSign=+1` = identiteetti) — silmämäärä kun user coachaa away-viikon (save on home-viikko, ei automatisoitavissa nyt).
  - **Käyttäjän on-device-silmämäärä** ennen committia.
- **⚙️ HUOM SIM-TILA:** play clock kytketty POIS testisimissä (`xcrun simctl spawn <UDID> defaults write com.brewcrow.dynasty playClockSetting -string off`) — 10s ikkuna oli nopeampi kuin tap-automaatio. **Palauta** `-string 10` kun automaatiotestaus valmis (hyödyllinen pitää off myös Plan B -testeissä). Kesken jäänyt testipeli hylättiin (Abandon → viikko yhä pelaamaton).
- **✅ PLAN B TOTEUTETTU (2026-07-23, `BUILD SUCCEEDED`, EI committia):** `mirror: Float = 1` -kerroin RouteSpec (`resolve`: `canonicalStartX = startX * mirror` → sideSign pre-reflection-X:stä, lateraali `* sideSign * mirror`; `points`, `diagram(for:mirror:)`) → PlayChoreographer (`offensePositions` `clampX($0.x * mirror)`, Context/formation/preSnapStep/steps/specPath/fallbackPath) → PlayDiagramView (`mirrored`-param) → CoachedGameView (REVERSE-nappi snap-barissa AUDIBLE-tyylillä, näkyy kun offense+kortti valittu; reset `.onChange(selectedCall)`; `snapMirror` jäädytetään ennen selectionin tyhjennystä; vastustajan snapit mirror=1). Engine/enum/Codable/FootballFieldScene EI kosketttu → `mirror`/`sideSign`/`screenSign` pysyvät erillisinä (ei tuplanegatointia). **Verifioitu:** standalone-skripti `scratchpad/verify_mirror.swift` — mirror=1 no-op byte-identtinen (toss/insideRun/cross), mirror=-1 == peilaus x=0.5:n yli (dotit+reitit), centered-carrier-guard PASS (insideRun RB x=0 pysyy, reitin world-x flippaa). Ei test-targetia projektissa → skriptiverifiointi.
  - **✅ PLAN B LIVE-VERIFIOITU SIMISSÄ (2026-07-23, agentti, HOME-peli):** REVERSE-nappi näkyy AUDIBLE:n vieressä (inaktiivi = tumma pilleri kultatekstillä, aktiivi = kultatäyttö mustalla — selkeä kontrasti, ei layout-glitchejä). Kortin taide peilautuu (Toss Sweep nuoli oikea→vasen), kenttäpreview flippaa (slot/RB-offset peilautuu, OL/ulko-WR:t symmetriset paikallaan). Reversattu Toss Sweep juoksi VASEMMALLE (7 yd), normaali OIKEALLE (6 yd, 1st down) = puhdas A/B. Reset toimii kortinvaihdossa JA joka snapin jälkeen (jokainen uusi peli alkaa un-reversed — hyvä oletus). Flood-syöttö reversattuna: resiiverit floodasivat vasemmalle kortin mukaisesti; puolustus ei peilaudu (tunnettu scope-raja, ei bugi). Screenshotit: `/tmp/plan_b_verify_2026-07-23/`. Peli hylätty → viikko pelattavissa.
  - **⏳ AUKI (Plan B):** away-kompositio Plan A:n kanssa (kun user coachaa away-viikon) + käyttäjän on-device-silmämäärä. Scope-raja kirjattu: reversatussa syötössä CB:t kanonisella puolella → "generalizes to asymmetric passes later".

## 🏈 KÄSINVAIHTO (MESH POINT) + QB/RB-STANCET — 2026-07-23 iltapäivä (2 workflow'ta ajossa)

- **🐛 Käsinvaihto puuttuu juoksupeleistä (käyttäjän havainto, Desktop/Screenshot 2026-07-23 at 13.56.08.png):** Outside Runissa pallo "ilmestyy" RB:lle joka on jo kaukana QB:stä — ei visuaalista vaihtoa. Haluttu: RB juoksee mesh pointin kautta (QB:n vierestä), pallo vaihtuu siinä, sitten RB murtautuu ulos. **Workflow ajossa:** Trace (pallon siirtomekanismi + carrier-assignment) → Implement (mesh point VAIN PlayChoreographer/RouteSpec-tasolla; mirror-turvallinen johdanto alignment-koordinaateista; Draw'n viivästetty vaihto säilyy; toss/keeper/sneak ennallaan) → adversariaalinen Verify (mirror/lateralSign/timing/replay/AI/fumble-katko/build).
- **🐛 Pallo ei QB:n kädessä heitossa (käyttäjän havainto, Desktop/Screenshot 2026-07-23 at 14.02.14.png):** dropbackin+heiton aikana pallo makaa paikallaan turffilla snap-kohdan lähellä. Haluttu: pallo QB:n kädessä (dropback = rintaote OK, windup = oikea käsi `hand_r`) snapista release-hetkeen, ja heittokaari lähtee KÄDEN positiosta release-beatillä (sama takuu toss-pitchille). **Workflow ajossa:** Trace (pin-elinkaari: miksi #9-fixin käsi-pinnaus ohittuu pass-peleissä) → Implement (SkeletalFigure ensisijaisesti; release-synkka = heitto-oneshotin aloitus releaseFraction-offsetilla niin release osuu kaaren starttiin; kielletyt regionit = rinnakkaisten workflow'iden alueet) → adversariaalinen Verify (ei pinnitöntä frame-ikkunaa, read-back-koordinaattien ei-tuplapeilaus, koppi/tuck/fumble/replay, diff-hygienia).
- **🎯 QB+RB-stancet (käyttäjän referenssit image-cache/6.png shotgun, 7.png under center):** workflow ajossa — Scout (kuka jakaa mitkä clipit; UC/shotgun-pelit listaan) → 3 rinnakkaista Blender-authorointia (QB-gun: kädet valmiina edessä; QB-UC: lantiosarana + kädet yhdessä alas-eteen, pää ylös; RB: 2-point kämmenet polvilla) → Wire (vain jos uudet clip-nimet tarvitaan). Pole targetit pakollisina + pelikulma-renderit.

## 🏈 STANCE-POOSIT LUONNOLLISIKSI — 2026-07-23 (EI committia)

- **✅ stance3 + stance2 UUDELLEENAUTHOROITU + in-app-verifioitu.** Käyttäjän referenssit (NFL-broadcast + Chiefs-lähikuva) vs vanha "kerälle taittunut" poose (CROUCH=0.72, ~88° selkäcurl, pää alhaalla). Avainoivallus: `thigh_L/R` ovat spine-rootin lapsia → spine-rootin rotaatio = **lantiosarana** (IK-pinnatut jalat taivuttavat polvet uudelleen) → ylävartalo kallistuu suorana "lankkuna" ja lantio nousee ylös-taakse. Jalat+pää palautetaan seisonta-orientaatioon poseerauksen jälkeen (jalat lattiassa, katse eteen-ylös), koko käsivarsi+käsi IK:lla turffiin. **stance3:** PITCH_ROOT=1.25 rad, CROUCH=0.30, BENDS=[0.22,0.14,0.05] (suora selkä), FOOT_WIDE=0.09, STAGGER=0.10, oikea käsi maassa, vasen kyynärvarsi reidellä. **stance2:** PITCH_ROOT=0.55, CROUCH=0.18, kädet polvilla, pää ylhäällä. Reitti = käsinposeeraus Metarigille (ei mocapia: Blender 5.1.2 ilman Rokokoa; mixamo_retarget damppaa kädet = fataali käsi-maassa-poosille). Export identtisellä konventiolla (58 luuta byte-identtinen setti → transplant OK). Blender-verifiointi: before/after + **5 hengen rivitesti** (lukee aitona O-linjana), numeerinen maakontakti (jalat flat ±2cm, kämmenmesh 3.1cm turffiin). Renderit: `/tmp/stance_fix_2026-07-23/`. In-app-check näytti torson OK (etunoja, lantio ylhäällä, ei uppoamista) — `/tmp/stance_inapp_2026-07-23/04_presnap.png`. stanceSplit/UC/Upright tarkistettu renderillä → ei kerällä → ei kosketttu.
- **✅ REGRESSIO KORJATTU: stance3-jalat ristissä pelissä (käyttäjän havainto, image-cache/5.png).** Diagnoosi = (a) poose-geometria, EI transplant-vika: **polvi-IK:lla (`add_ik("shin_L/R", chain=2)`) ei ollut pole targetia** → kun nilkat levennettiin (0.28→0.46), solveri romahdutti polvet sisään (KNEE_SEP_X=0.045, oikea polvi kirjaimellisesti kehon toisella puolella). Byte-eksakti repro mk3.py:stä sulki transplant-hypoteesin. **Fix (`/tmp/stance_fix2_2026-07-23/mk_polefix.py`):** polvi-pole-targetit eteen-ulos (POLE_FWD=1.0, POLE_OUT=0.30, POLE_ANG=-90° — sweepattu {-135..180}, -90 ainoa symmetrinen). KNEE_SEP_X 0.045→0.498, jalat 1.32× hartialeveys, varpaat eteen (ei pigeon-toe), stagger 0.099. Ylävartalo byte-ennallaan (lantiosarana/selkä/pää/käsi-turffiin). Maakontakti kuten ennen (varpaat 1.8cm, kämmen kiinni). stance2 tarkistettu pelikulmasta → OK, ei kosketttu. **In-app-verifioitu pelikameran kulmasta** (leveä perusta, polvet ulos, ei ristiä): `/tmp/stance_inapp_2026-07-23/07_fixed_presnap.png` + lähikuva `08_ol_closeup.png`. Opit: (1) IK ilman polea = knock-knee-riski aina kun jalkoja siirretään, (2) verifiointirenderit AINA myös pelikameran kulmasta. ⏳ Käyttäjän on-device-silmämäärä.
...
**⏳ AUKI seuraavaksi:**
- **✅ Play-suunta peilikuvassa — RATKAISTU (ks. ylin osio).** Tämä diagnoosi oli VÄÄRÄ (resolve-lateraali/direction-hypoteesi): oikea juurisyy = world→screen-käsiheys (`viewFacing` kääntää L/R:n kamerassa, kortti kiinteä) ja se osuu HOME-coachattuun, ei awayhin. Korjattu `lateralSign = -viewFacing` -peilauksella scene-rajalla (`docs/PLAN_play_direction_and_reverse.md` Plan A).
- **Feature:** vastapuolen juoksut (sweep/outside kumpaankin suuntaan) + reverse-nappi. → Suunniteltu = Plan B samassa dokissa; seuraava toteutus.
...
- **✅ Taklaus: juoksu paikallaan pallon kohdalla ennen taklausta — KORJATTU.** Juurisyy (jäljitetty): default wrap-up-taklaus (`PlayChoreographer.tackleSteps` rivi ~1691) laukoi 0.36s "contact"-stepin ILMAN `falls`-listaa, sitten vasta "pile"-stepin joka kaataa kantajan. Kantajan ~0-matkan contact-siirto osui `distance>0.4`-skip-guardiin (`run()` rivi 1872) → `setMoving(false)` ei koskaan laukennut → juoksuklippi jäi pyörimään 0.36s kunnes pino laskeutui. **Fix:** siirretty kantajan `fall()` contact-steppiin (`falls:[carrier]`) ja poistettu hänen ~0-matkan siirtonsa sieltä → kantaja menee alas kontaktihetkellä kuten kaikki muut osumat. Pino (tackler+gang) laskeutuu beatin päästä. Muut taklausvariantit (big-hit/drag-down/diving/shoestring) kaativat jo kantajan ensimmäisessä stepissä = ei aukkoa.
- **🎯 "Kaikki katsovat vasemmalle" + "linjamiehillä ei ole kättä maassa" = SAMA JUURISYY (bug #7 vahvistui).** `applyStance` (rivi 960) poosaa lapsinodet `arm`/`armR`/`leg`/`legR` — joita on VAIN vanhalla proseduraalikitillä. Skinnattu skeletaalirigi käyttää oikeita luita (`upper_arm_R`, `thigh_L`, `spine`…) → limb-luuppi (rivi 1027) ei löydä mitään = **no-op**. Skeletaalipelaaja saa VAIN figuurin etukallistuksen (`pitch/sink`, rivi 1018) + jäätyneen idle-"Hold"-kyykyn (`idleFraction≈0.16`). → ei 3-point-asentoa, ei kättä maassa, kaikki identtisiä ja lievästi vinossa = "vasemmalle". **FEATURE: toteuta oikeat skeletaaliasennot.**
  - **Reitti A (proseduraali):** poosaa oikeat luut applyStancessa (spine eteen, `upper_arm_R`+`forearm_R` alas turffiin, `thigh/shin` porrastus) per rooli. Nopea; staattinen poose → proseduraali siedettävämpi kuin animaatiossa. Riski: käden osuminen tarkalleen maahan vaatii viritystä per rigi.
  - **Reitti B (klippi):** tallenna/authorroi 3-point-stance-clip (+ 2-point, under-center QB) rigille, soita held per rooli. Paras Madden-rima; vaatii uudet clipit (Blender-työ kuten tackle-clipit).
  - Muistin bug #7 (`project_session_state_2026_07_21`) merkitsi tämän "feature-kokoiseksi".
- **✅ TOTEUTETTU + VARMENNETTU SIMULAATTORISSA.** Authorroin 5 stance-clippiä Blenderissä (IK-poseeraus: jalat maassa, oikea käsi turffiin; bake IK→FK; export held-clipiksi; verifiointi renderillä). Clipit: `PlayerClip_stance3/stance2/stanceSplit/stanceUC/stanceUpright.usdc` Resourcesissa (synced folder → bundlaantuu autom.). Wiring: `SkeletalFigure.playStance()` + uusi `"stance"`-avain (isPosing=true → foot-lock pois; EI isGrounded → ei ground-clampia; `setMoving(true)` purkaa snapissa); `applyStance` ohittaa proseduraali-no-opin skeletaalille ja soittaa roolikohtaisen clipin. `applyStance`-stance→clip-map: threePoint→stance3, twoPoint→stance2, split→stanceSplit, underCenter→stanceUC, upright→stanceUpright. **In-app varmennettu:** OL 3-point käsi maassa, LB/S 2-point kädet polvilla, facing oikein (ei enää "vasemmalle"). Tuning-varaa: 3-point-poose melko syvä/kyyryssä (CROUCH=0.72) — voi keventää. Authorointiskripti: `scratchpad/mk.py` (agentin tekemä, 5 presettiä).

## 🏈 PINO-LEIJUNTA ("nousee ilmaan ja lentää kasassa") — sessio 2e (2026-07-22, `BUILD SUCCEEDED` + boots clean, EI committia)

- **✅ KORJATTU + TODISTETTU (numeerisesti + renderöidyllä kuvalla).** Juurisyy ei ollut se mitä hypoteesi (2d) arveli. Mitattu Blenderissä `spine.location[1]` (= pysty, +Y = UP; varmennettu world-Z-mittauksella): `tackle_a` nousee **monotonisesti 0→+1.67 eikä laskeudu koskaan** (viim. framessa alin luu +2.59 world = koko keho lentänyt ilmaan), `tackle_b` sama (+1.57→+1.30). **Ei "ylös+alas" — pelkkä ylösnousu, leikkeissä EI ole laskeutumista lainkaan.** `dive` sen sijaan on OIKEIN (hyppää +0.60 → laskeutuu −0.31 = maahan) → **divea EI kosketa** (per-frame-seat tappaisi hypyn).
- **Fix-mekanismi:** runtime-ground-clamp (`SkeletalFigure.updateFootLock`, rivi 451) on **lift-only** (`if lowest < -0.005 { content.position.y += -lowest }`) → se EI voi laskea leijuvaa kehoa, vain nostaa maahan uponnutta. Siksi pelkkä "cap ≤ base" ei riitä (jättäisi kropan leijumaan lonkkakorkeudelle). `strip_root.py`:hyn lisätty `--seat`-moodi: laskee root-Y:n per keyframe niin että **alin luun-pää (heads only, sama minkä runtime clampaa) istuu turffiin joka framessa** (target = seisonta-jalkakorkeus − 0.05 marginaali → clamp seatuu flush). Muunnos eksakti+lineaarinen (gain = worldZ/root_Y ≈ 1.35, mitattu probella). Tulos: alin luu vakio ~0.04 joka framessa, keho laskeutuu seisonta(spine 1.18)→makuu(0.46–0.93). Re-stripattu `tackle_a`+`tackle_b` (lähde: `git 8fae58d`, pre-strip). `dive` ennallaan.
- **Verifiointi:** Blender-render sivulta turffi-tasolla — BEFORE: keho lentää ulos ruudun yläreunasta (pelkkä nurmi näkyy); AFTER: taklaaja makaa nurmella (diving-sprawl), START seisoo jaloillaan. App buildaa + boottaa puhtaasti uusilla clipeillä (bundlen koot 267352/267357 = tuoreet, ei stale-trap). Muutokset: `tools/asset-pipeline/strip_root.py`, `PlayerClip_tackle_a/b.usdc`. Muisti: `project_tackle_rootmotion_2026_07_21`.
- **⏳ AUKI (pientä, ei raportoitu bugi):** (1) offensiivinen pylon-`dive` (`play(action:"dive",hold:true)`, rivi 2766) leijuu lievästi ~0.5 loppu-poosissa (dive-hyppy on aito → per-frame-seat ei käy; vaatisi vain lopun tail-seatin). Jo #10:ssä flagattu "slight float". (2) `tackle_a` END-poose = dynaaminen "jalka pystyssä" -sprawl held-tilassa — luonteva syöksytaklaukselle mutta jos held näyttää jäätyneeltä, leikkeen loppua voi trimmata. Molemmat = on-device-silmämäärän jälkeen päätettävää polishia.
- **⏳ VAATII käyttäjän on-device-silmämäärän** (juoksu+taklaus iPad Prolla): pino jää nyt nurmelle sim-kohtaan eikä lennä.
...
- **✅ RATKAISTU sessio 2e:ssä (yllä).** (Hypoteesi tässä oli osin väärä: leikkeissä ei ollut "ylös+alas" vaan pelkkä ylösnousu ilman laskeutumista; korjaus = per-frame ground-seat, ei pelkkä Y-cap.)
...
- [x] Root cause C: it never consulted the player's TRAITS at all. `CoachingEngine.schemeFit` (the real trait computation used by PlaySimulator / PickGradeCalculator / SigningInterestEngine) was not in the offseason path.
- [x] New canonical `CoachingEngine.rosterSchemeFit(player:offensiveScheme:defensiveScheme:)` — both halves measured as an EDGE against a neutral, which is what makes the distribution stationary: **traits** = his `schemeFit` for the installed system minus his mean across every system on his side (a great player fits everything; that is talent, not fit), **playbook** = familiarity vs `schemeFitFamiliarityPivot` 52. Constants: neutral 0.59, traitGain 1.77, famGain 0.25.
- [x] `VersatilityDevelopmentEngine.installBaseline` + `seedActiveSchemes` — a pro carries 12 + 0.18·learning + 0.06·coachability + 0.30·(deepest system he owns), capped at 50 (under the 55 `unusedSchemeFloor`) into a playbook nobody has taught him. Runs at camp (`applySchemeChanges`) and on every in-season practice week, so trades / street signings / promotions and carousel installs all get a real number instead of an absent-key 0. **Also fixed:** `decayUnusedSchemes` was missing its documented `value > floor` clause, so parking a half-learned system LIFTED it to 55.
- [x] `applySchemeChanges` now runs BEFORE `buildOffseasonInputs` (the fit is read off the dictionary, so the dictionary must describe the staff that will coach the upcoming season), and "what the club installs" resolves coordinator → head coach → assistant HC everywhere.
- [x] Harness: `career` now CALLS the shipped function (sliced by `sync_sources.sh`) instead of drawing a frozen `N(0.55, 0.18)`; it also runs the 17 weekly `learnScheme` reps the shipped `WeekAdvancer` runs and that the harness was missing. New `--- SCHEME FIT ---` block prints the distribution in the same shape `DevelopmentSourceDiag` prints it, plus the `updatePotentialRealization` bucket shares and `E[dPot]`.
- [x] Result: mean **0.597**, sd **0.176**, p10 0.35 / p50 0.57 / p90 0.83, drift over 22 measured seasons **+0.001**, 0.00 % of samples land exactly on 0.50. Pyramid unchanged vs baseline (mean 70.72 vs 70.65, sub65 24.86 vs 24.69, 80+ 16.27 vs 16.03).
- [x] Gates: `career` **30/30** (26 original + new 6.10a-d on the fit distribution) · `draftclass` 32/32 · `leaguegen` 10/10 · app build exit 0. Also unbroke the harness at this commit: `CollegeProspect.harness.swift` was missing the `hometownState`/`hometownCity` fields the last commit added to `DraftClassBuilder`, so `./run.sh` did not compile at all.
- NOTE: `career` 6.2b (R2 elite share, band lower edge 3.5 %) sits ON its bound both before and after this change (n = 6 400 R2 careers → SE 0.23 pp, observed 3.4-4.2 across runs). Pre-existing knife edge, not caused by #54 — worth widening the band or pooling more leagues.
- NOTE: a fresh generated league starts ~0.63 (the generator seeds the active scheme at 55-85) and settles to ~0.60 as the seeded veterans retire. That residual transient is a FAMILIARITY-model question (intake enters at 15-45 and `learnScheme` climbs ~12/season, so the league equilibrium is 54), not a scheme-fit-definition one.

## Audio: volyymisliderit + hiljaisemmat defaultit + menu/dashboard-musiikin restyle (#80, 2026-08-03)
- [x] **Kaksi erillistä volyymiä Settingsissä.** Sliderit olivat jo olemassa mutta nimettöminä (kaksi identtistä kaiutinikoni-riviä peräkkäin — kumpi on kumpi?). Nyt `volumeRow`-helper: otsikko + elävä prosenttilukema + slider. "Sound" → **"Game Sounds"**, koska pelkkä "Sound" luetaan koko sovelluksen mykistykseksi.
- [x] **Game Sounds = yksi master gain.** `AudioDirector.masterVolume` kattaa jo kaiken: jokainen `MatchSound`-one-shot (siis myös jokainen `CrowdReactor`-reaktio, joka menee `play(_:gainScale:preferTake:)`-kutsun kautta) ja yleisöpeti `applyCrowdState`issa. Varmistettu grepillä: `AVAudioPlayer`ia ei instansioida missään muualla kuin näissä kahdessa directorissa, joten sliderin ohi ei ole reittiä. Ei tarvinnut lisätä mitään.
- [x] **Hiljaisemmat fresh-install-defaultit** käyttäjän pyynnöstä: musiikki 0.50 → **0.35**, peliäänet 0.70 → **0.60**. Uusi `UI/Common/AudioSettings.swift` on ainoa paikka jossa luvut ovat — ennen sama literaali oli kolmessa paikassa (slider, director, reset-polku) ja olisi ajautunut erilleen huomaamatta.
- [x] **Vanhat asennukset eivät muutu.** Molemmat directorit lukevat `UserDefaults.object(forKey:)`in kautta ja coalescoivat vasta jos avainta ei ole koskaan kirjoitettu — `double(forKey:)` olisi palauttanut 0:n puuttuvalle avaimelle, mitä ei voi erottaa käyttäjästä joka veti liu'un nollaan. Nämä eivät myöskään ole `CareerScopedDefaults.keys`issa: laitteen äänenvoimakkuus ei ole tallennuksen ominaisuus.
- [x] **Menu-musiikin restyle** (käyttäjä halusi "Chariots of Fire" -tyylistä mahtipontisuutta): 4 uutta ACE-Step-kandidaattia, hidas 72-84 BPM, synteettinen johtomelodia orkesterin päällä, duuri, 94-107 s. **Kappaleen/artistin nimeä ei promptattu** — vain tyylipiirteet (tempo, sävellaji, soittimet, tunnelma), joten mallilta pyydettiin genreä eikä teosta.
- [x] **Dashboard-restyle** (tummempi, hitaampi, mellow): 5 kandidaattia, dark ambient -henkinen downtempo 60-75 BPM, niukka rytmi, lämpimät matalat syntikat, 156-190 s. Viisi 40-46 s ambient-**looppia** säilyivät koskemattomina — ne eivät olleet ongelma; ongelma olivat r2:n `bed_*`-raidat, jotka olivat samoja 165-175 s elokuvallisia cueita kuin menuteemat.
- [x] **Valinta mitattiin, ei arvattu.** Uusi `tools/audio/verify_music.py`: (a) decode-järkevyys pass/fail — dekoodautuuko m4a päästä päähän, onko kesto sama kuin masterilla, onko keskellä hiljainen aukko, onko toinen kanava kuollut, DC-offset, clippaus. **9/9 läpi.** (b) rooli-sopivuus rankkauksena — menuun myöhäinen huippu + oikea kaari, dashboardiin päinvastoin tasainen kaari + tumma spektrikeskiö. R2:n opetus huomioitu: siellä *tyyliä* rankattiin näillä ja meni väärin; nyt niitä käytetään vain koska r3:n brief itse ilmaisi rakennevaatimuksen mitattavin sanoin.
- [x] Shipattu menuun 2 (`menu_grand_r3_piano` 5.50, `_soar` 5.35), dashboardiin **3** eikä 4: neljäs (`_drift` 1.16) skoorasi huonommin kuin kolme neljästä korvattavasta r2-bedistä — malli ei totellut "dark, no build" -promptia. `_hollow` (0.46) epäonnistui samoin vielä pahemmin. Molemmat jäävät `gen_music/`iin swap-iniksi.
- [x] `ship_music.py` **karsii** nyt kohdekansion: ilman sitä korvattu raita jäi Resources/iin mukaan bundleen näkymättömänä. Retired: `music_menu_theme_c`, `music_dashboard_bed_d`. Budjetti 60 → 65 MB, toteuma **25 tiedostoa / 44.7 min / 54.8 MB** (r2 oli 27 / 59.7 MB).
- [x] `post_music.py` ottaa nyt stem-filtterin ja **mergeää** manifestiin — täysajo olisi laskenut 7 loopin saumahaun (54 renderöintiä kukin) uudelleen deterministisesti samaan tulokseen.
- [x] LICENSES.md: uusi "Music — round 3 (SHIPPED, current)" -osio (siemenet, promptit, kustannus $0.10, valintataulukko, korvattu-vs-retired, nykyinen ship-taulukko). R2:n taulukkoon superseded-huomautus. **Lisenssiasema parani:** kaikki 9 uutta on Apache-2.0 ACE-Stepiä, joten Stability AI Community License -velvoite koskee yhä vain 7:ää tiedostoa — nyt 25:stä eikä 27:stä.
- [x] `MusicDirector`: menu 2 raitaa, dashboard 5 looppia + 3 bediä. `levelTrim` ja `gapRange` koskemattomat.
- [x] Build: `** BUILD SUCCEEDED **` (Debug, generic iOS Simulator). App-bundlessa 25 music-tiedostoa, 0 retired-tiedostoa.
- NOTE → #74: 4 shippaamatonta r3-kandidaattia (`menu_grand_r3_anthem`, `_hymn`, `dashboard_dark_r3_drift`, `_hollow`) + 7 r1/r2-raitaa jotka menettivät paikkansa rooli-sopivuudella eivätkä laadulla (`orchestral_cinematic_take1/take2`, `menu_theme_r2_take1`, `_r2_strings`, `_r2_noble`, `dark_hybrid_r2_slow`, `_r2_atmos`) odottavat korvakuuntelua `tools/audio/gen_music/`ssa.

## Yhdistetyn diffin adversariaalinen review — FINAL FIX + GATE (#77/#78/#79 + audio, 2026-08-03)
Neljän rinnakkaisagentin (WARROOM / MARKET / ECONOMY / AUDIO) yhteisdiffistä löytyi 8 vahvistettua vikaa. Kaikki korjattu tässä aallossa.

- [x] **C1 — rookien `injuryWeeksOriginal` jäi nollaan.** `DraftEngine.carryPreDraftInjury` kirjoitti `isInjured` / `injuryWeeksRemaining` / `injuryType` / `rehabStatus` muttei nimittäjää, jonka jokainen lääkintänäkymä jakaa: "6 of 0 weeks remaining", rehab-palkki ikuisesti 0 %, `MedicalEngine`in setback-haara (`remaining < original`) ei voinut koskaan lauetta ja `WeekAdvancer`in `>= 4` -uutiskynnys ei ylittynyt. Nyt `player.injuryWeeksOriginal = carry.weeksOut` (ennuste, ei jäännös) — sama pari kuin `MedicalEngine.applyInjury`issa.
- [x] **C2 — "yksi availability-malli" oli yhä kolme.** `BigBoardView.availableAtPickProbability` eli edelleen ja ajoi Draft Day Predictorin + tier-rivien selviytyjälaskennan vanhalla kierrosämpärillä (0.95/0.75/0.40/0.15/0.05), joten Big Board sanoi 40 % ja Mock Draft ~44 % samasta miehestä samana päivänä. Runko delegoi nyt `DraftAvailability`iin (`consensusRank: marketRank(for:)`); kolme kutsupaikkaa ennallaan.
- [x] **C3 — "MY #N on se slotti jonka Big Board tulostaa" oli valhetta, ja se oli shipattua UI-tekstiä.** Numeroita oli neljä: war room ja pick sheet `UserDraftBoard.rankMap` (raaka talletettu järjestys), Big Boardin My Board indeksi `cachedCustomOrderedBoard`iin (= **aktiivinen filtteri** merkkitier-järjestykseen sortattuna), Mock Draft koko luokan yli ja Board Diff enumerate-indeksi. Kolme miestä `elite`-merkittynä sloteissa 4/19/41 tulostui Big Boardilla #1/#2/#3 ja war roomissa MY #4/#19/#41. Uusi `UserDraftBoard.slotMap(among:)` + `draftPopulation(_:)` (ilmoittautunut luokka, fallback koko luokkaan) on nyt **ainoa** lukija — Big Board, Mock Draft, war room ja pick sheet lukevat sen, ja Board Diff tulostaa saman numeron kuin rivi itse. `totalCount` My Boardilla on koko laudan koko, ei sektion.
- [x] **C4 — `.coachingChanges`-sykeviesti kertoi aina "0 underclassmen are still deciding".** `WeekAdvancer` ajaa `generateDeclarations`in samassa lohkossa ENNEN sykeviestiä, ja se jättää jokaisen alaluokkalaisen `.declared`/`.withdrawn`-tilaan → laskuri oli rakenteellisesti nolla. Teksti raportoi nyt lopputuloksen: montako on draftissa, montako heistä aikaisia ilmoittautujia, montako palasi kouluun.
- [x] **C5 — vanhat tallennukset piirsivät vetäytyneet alaluokkalaiset "LIKELY"-merkillä.** `generateDeclarations`in idempotenssivahti palauttaa tyhjän luokalle joka on jo käynyt ikkunan läpi, joten ennen tätä aaltoa tehty save ei koskaan saanut `declarationStatusRaw`ia → koko luokka dekoodautui `.undecided`ksi ja `declarationLikelihood` palautti arvon miehelle jota ei voi draftata. Uusi `ScoutingEngine.backfillDeclarationStatus(_:)` rekonstruoi tilan `isDeclaringForDraft`-lipusta — päätöstä ei koskaan arvota uudelleen, ja se ajaa vain luokalle jonka ikkuna on todistettavasti sulkeutunut (sama testi kuin generaattorin vahdissa). Kytketty `WeekAdvancer.healDeclarationStatus`in kautta `migrateLegacyDraftClassIfNeeded`iin (molemmat luokan palauttavat näytöt kulkevat sen läpi) + suoraan `.coachingChanges`-lohkoon. **Ei kutsuta `generateDeclarations`ista**: se jäsen viipaloidaan verbatimina balanssiharnessiin, ja viipaleen ulkopuolelle osoittava kutsu kaataisi `draftclass`-gaten käännöksen.
- [x] **C6 — war roomin prospektikortti avasi maksulliset, mutatoivat toiminnot kellon käydessä.** `.draft` on kaikkien kolmen kevätikkunan sisällä, joten "Invite for Workout" ajoi `conductPersonalWorkout`in (poltti slotin, kirjoitti mitat uusiksi, `save()` kesken draftin) ja "Send Scout" kirjasi raportin joka siirtää sumukaistaa miehestä, jonka `publicBoardRanks`in koordinaattori on jo cachettanut — mitään cachea ei invalidoida. Uusi `ProspectDetailView.isLiveDraftCard` sulkee evaluaation, haastattelun ja workoutin ja kertoo rivillä miksi; merkinnät ja muistiinpanot jäävät eläviksi (ilmaisia, käyttäjän omaa lautaa, ja koko kortin pointti kellossa). WARROOMin kaksi esityspaikkaa asettavat lipun.
- [x] **C7 — attribuutio: `UI/Career/TeamSelectionView.swift` `#Preview` (`avatarID`).** Muutos `"coach_m1"` → `"avatar_00000"` ratsasti audiodiffissä mainitsematta. Arvo on **oikein** (`coach_m1` on yksi 20 piirretystä muotokuvasta jotka avatar-remontti poisti; `UserPortraitView` vain *resolvaa* vanhan id:n vanhalle savelle, uusia ei pidä syntyä), joten se jää — mutta nyt kommentoituna ja tässä kirjattuna.
- [x] **C8 — eksplisiittinen päätös: neljä "Move to Round N" -kontekstivalikkonappia poistettiin.** Ne kirjoittivat `draftProjection`ia, joka on MEDIAN kierros: `applyProjectionDrift` takaa luokkatason multisetin säilymisen (jokainen nousu maksetaan laskulla), joten käsinkirjoitus oli parittomana koko draftin uudelleenviritys; `ProspectFog.consensusBand` / `DraftIntel.publicOVREstimate` / `consensusWindow` lukevat sen "mitä huone ajattelee" -arvona, joten STEAL/REACH-merkit vertasivat käyttäjän mielipidettä itseensä; ja `projectionAtGeneration` ankkuroi nyt markkinanuolen, joka olisi näyttänyt liikettä jota ei tapahtunut. **Korvaava kyky on jo olemassa** samassa valikossa: seitsemän tier-siirtoa (`manualTier`), Move Up / Move Down ja merkki + oma arvosana. Kyky poistettiin tarkoituksella, ei sivuvaikutuksena.
- [x] Lisäksi kolme low-severity -kohtaa: `ScoutingHubView.topScoutedRead` sai tie-breakin (kirjainarvosanan `rank` on karkea, ~30 miestä jakaa "B+" ja otsikko nimesi eri pelaajan joka uudelleenkäynnistyksellä); `SendScoutSheet.sendScout`in kommentti korjattiin vastaamaan sitä mitä vahti oikeasti tarkistaa; `DraftEngine.generateStaffRecommendations` ei enää lajittele `?? trueOverall`illa (kuollutta koodia, mutta se oli ainoa jäljellä oleva piilorating-vuoto joka odotti kytkemistä).

## Eläköitymisen realismiaalto — Luck / Donald / comeback (#84, 2026-08-03)

Kolme erikoistapausta eläköitymiseen **ilman että aggregaattitahti liikkuu**. Kalibrointi (a33+, ikäpyramidi, uran pituus) on `career`-harnessin porteissa, joten tapaukset EIVÄT ole ylimääräisiä heittoja vaan luokitteluja olemassa olevan hasardin sisällä (`PlayerRetirementEngine.classifySpecialCases`).

- [x] **LUCK (`.injuryToll`) — 1:1-vaihto, ei lisäys.** Ehdokas: uransa parhaissa vuosissa (ikä ≤ position `peakAgeRange.upperBound`, alle 33 → hasardi ~0), ≥4 vuotta ammattilaisena, ≥3 vakavaa vammaa (6+ vk), ≥30 viikkoa uran aikana sivussa, durability ≤58, motivaatio ei `.money`/`.fame` (ei-palkkasoturi). Portti: SplitMix64 (careerID, season) → 400/1000 kausista. Kun laukeaa, listalta **poistetaan täsmälleen yksi** tavallinen eläköityjä — pienimmän `retirementProbability`in mies (UUID tie-break), joka on samalla se "yhtä todennäköinen paikka" jota vaihto hakee (Luck-ehdokkaan oma hasardi on ~0). Molemmat puoliskot tai ei kumpaakaan → `|retirements|` on bitilleen sama kuin ilman koko ominaisuutta, joka kaudessa, ei vain odotusarvossa.
- [x] **DONALD (`.onTop`) — pelkkä uudelleennimeäminen.** Luetaan valmis lista: ikä ≥30, nykyinen OVR ≥90, palkintokaappi täynnä (≥1 mestaruus JA (HOF-ura TAI ≥4 elite-kautta)), per-pelaaja-portti 60 %, katto **2/kausi**. Jäsenyys ei muutu → aggregaattivaikutus tasan nolla. Mestaruudet johdetaan `SeasonSummary.championTeamID` × `PlayerSeasonHistory.teamID` -parista, ei uutta talletusta.
- [x] **COMEBACK — toteutuskelpoinen, koska `retire()` ei poista riviä** vaan merkitsee `isRetired`. Uudet kentät `Player.retirementSeason` / `retirementCaseRaw` (inline-default → kevyt migraatio). Ehdot: ≤2 kautta sitten eläköitynyt, uran huippu ≥88, ikä paluuhetkellä ≤38, EI Luck-tapaus (keho oli syy). Portti 450/1000 + haastajan (11+ voittoa) oltava olemassa roster- ja cap-tilaa myöten. Paluurating: shipattu `applyAgeRegression` kertaa poissaolovuodet (palvelusvuodet annetaan takaisin — sohvalla ei kerry kautta) + ruostesakko fysiikkaan/awarenessiin. 1 vuosi, 4,0 M$. **Käyttäjän joukkue ei ole kohde** — AI ei paina pelaajaa ja cap-hittiä hänen 53:een ilman kysymistä.
- [x] Uutiset/inbox: `RetirementCaseNewsFactory` (NewsGenerator.swift) + `InboxEngine.shockRetirementMessage` / `retiresOnTopMessage` / `comebackMessage`. Erikoistapaus KORVAA geneerisen seremoniaotsikon ja käyttäjän vakiojäähyväiskirjeen (legacy-piste jää molemmissa). Kaikki uusi teksti englanniksi.
- [x] Kaikki portit: `career` **30/30 PASS** kaksi kertaa peräkkäin muutosten kanssa. a33+ 2,92 % (baseline) → 2,91 % / 3,12 % (kaksi ajoa) — hajonta on ajokohtaista kohinaa, ei systemaattista ajautumista; budjetti oli ≤+1,5 pp. Uran pituus 5,61 → 5,60/5,61, blue chipit 24,3 → 24,2/24,6, drift muuttumaton. Harness ajaa erikoistapaukset **päällä** (`CareerScenario.harness.swift`), jotta väite mitataan eikä vain väitetä; mitattu Luck-tahti 0,133–0,150/kausi eli ~1 per 7 liigakautta (portin katto on 0,4/kausi).
- NOTE: harnessissa palkintokaappikartta on tyhjä (siellä ei ole mestaruushistoriaa), joten vain Luck-puolisko — se joka koskee jäsenyyttä — tulee mitatuksi. Donald on rakenteeltaan mahdoton siirtämään lukumäärää.

## Playtest-korjausaalto — käyttäjän 6 havaintoa yhdessä sessiossa (#106–#111, 2026-08-06)

Kaikki kuusi tap-verifioitu scratch-simulla (tuore ura, Fixed 2026 Dev / Titans) + adversariaalinen katselmus (0 HIGH, 5 MED confirmed → kaikki korjattu samassa aallossa).

- [x] **#106 Auto-Hire & coaching budget.** Repro EI vahvistanut "budjetti ei vähene" -perusväitettä (derivoitu aritmetiikka toimi), mutta kolme oikeaa vikaa löytyi ja korjattiin: dashboard-tile laski eri kaavalla kuin Staff-ruutu (vähensi scoutit+medicalin coaching-potista → $13.4M vs $18.2M samasta tilanteesta; nyt sama coaching-pot-kaava), slottilaskuri "22/21" (scout-slotteja oli koodattu 6, oikeasti `ScoutRole.allCases.count` = 8), ja auto-hiren allokaatio ei palannut jakamaan jäännöslompakkoa (AHC jäi palkkaamatta "no room in the budget" vaikka $18.2M jäljellä → toinen jakokierros fair-share-capilla). Lisäksi: tile-refresh `.onAppear`issa kevyellä `refreshStaffTile()`-polulla (koko `loadAllData` = kahden kauden pelit main threadilla pop-animaation aikana) ja advance-portti laskee nyt KAIKKIEN kolmen potin ylitykset (kaavan kavennus olisi muuten hiljaa leventänyt mitä viikko päästää läpi).
- [x] **#107 Combine Review -kuollut tila ennen combinea.** `prepCalendarRank == 0` -tilassa prosessipalkki piirsi stage 1:n kultaisena CURRENT + "Not read" + explainerin CTA "Open the Combine tab and read the numbers" tyhjän "0 of 0 prospects invited" -taulukon päälle. Nyt: locked voittaa currentin solussa (paitsi satisfied → tick säilyy), WAITING-chip + "runs automatically when the Combine phase begins" -rivi, "Opens at combine/pro days/draft week" -sublabelit, advance-bar piiloon (halpa `prepCalendarRank > 0` -portti, ei toista DraftPrepProgress-buildia per body pass), DraftPrepCardin lupaileva kopio odotusmuotoon ja sen "N to fix" -kohteet film-study-ikkunan taakse. Gating (`canAct`) koskematta — B3-luokka pysyy kiinni.
- [x] **#108 Auto-Set Priorities.** RosterEvaluationViewiin nappi joka täyttää kaikkien 9 ryhmän prioriteetit heuristiikalla (starter/depth-grade + `teamNeedDeficits`-sija + expiring-starterit + ikä vs peak, ST capattu mediumiin; kynnykset ≥3→high, 1-2→medium, ≤0→low), confirmation jos yliajaa käsin asetettuja, ei koske noteihin/ownAssessmenteihin, rivit saavat HIGH/MED/LOW-badget (kuollut `priorityColor` heräsi). Katselmuskorjaus: Big Boardin "You: Priority" -rivi näyttää vain high-ryhmät (top-3) — muuten yksi nappi olisi tehnyt rivistä "Priority QB, RB, WR, TE, OL, DL, LB, DB, ST".
- [x] **#109+#111 Listastandardi.** Combine-taulukko koko leveyteen molemmissa orientaatioissa Big Boardin omalla mekanismilla (nimi-floor 132pt + Spacer + trailing-ankkuroidut kiinteät sarakkeet → header/rivi-linjaus rakenteellinen kaikissa moodeissa), nimi katkeaa ENNEN chipejä (layoutPriority). Uusi jaettu `ProspectListControls` (positiochipit + Overview/Work-up/Physical/Mental/Position-moodit + jaetut solut) käytössä sekä Big Boardilla (pikseli-identtinen) että combine-listalla (5 moodia, drillit = home block). Sortti nollautuu rankiin kun moodinvaihto piilottaa sorttisarakkeen. Adoptiokandidaatit (InterviewSelectionView jolla on NELJÄS positiofiltteri, Top30Visits, ProDayTour, WorkoutsTab, MockDraft) → task #116.
- [x] **#110 + #114 Fog-vuodot.** Kortin alempi Position Skills -osio TULOSTI `truePositionAttributes`-totuusarvot (`isScouted`-portilla jonka `applyPreScoutedData` sytyttää top-250:lle ilman yhtään kaistaa) → poistettu; mental/position-gridit renderöivät nyt per-avain `ProspectFog.AttributeDisclosure`n kautta (lukitut solut + attribuutio + tyhjätilat; interview-avaimet `ScoutingEngine.interviewRevealedMentalKeys`istä, position-avaimet writerin mukaisesta kanonisesta taulusta — `SAc/DAc/TAK`-drift korjattu boardilta, combinesta ja compare-sheetistä, QB:n ikuiset "?"-sarakkeet paranivat). Big Boardin Physical-moodi tulosti `truePhysical`-raakanumerot (mikään instrumentti ei paljasta niitä) → fogatut combine-mittarit samoilla helpereillä kuin combine-taulukko; CMB-tintti drill-gradesta; `?? trueOverall` -latentit fallbackit pois; OVR/GRD-bändi molemmissa listoissa `ProspectFog.read`in kautta (sama leveys kuin kortilla). Personality-attribuutio: "League consensus" kun raporttia ei ole (pre-scout-freebie ei ole "Scouts' read").
- [ ] **Seuranta:** #112 (dashboardin 3× `.sheet(isPresented:)` samassa ketjussa + teamID-fetchit ilman careerID:tä), #115 (private workout ei kirjoita attribuuttibändejä — kallein instrumentti inertti; + `isScouted` epäturvallinen portti), #116 (kosmetiikkanippu + interview-pikatoiminto jaettuun kontekstivalikkoon + toolbar-adoptio loppulistoihin).

## Koko jonon rinnakkaisaalto — 6 agenttia + fleet-katselmus (2026-08-06 ilta, `dba2370`→`eec540c`)

Käyttäjän "tee kaikki jonossa olevat todot rinnakkain" → #112/#115/#116/#121/#122/#123 + balanssipaketti #88/#98/#99/#100/#101. Kolme committia: `dba2370` (#122 hinnoittelu), `648a335` (UI-slice), `eec540c` (balanssi+tradet+katselmuskorjaukset). Portit: career 36/36, leaguegen OK, draftclass OK, buildi vihreä. Fleet-katselmus: 19 löydöstä → 16 korjattu samassa aallossa (mm. KRIITTINEN kylmäkäynnistys tyhjällä draft-luokalla joka esti advancen; workoutin persoonaylikirjoitus; sweeten-2for1 position-tyhjennys).

- [x] **#121** Jaettu sarakejärjestelmä (ProspectColumns/ProspectRowIdentity) → Big Board + interview + film study, 5 moodia, portraitit, fogatut solut; board −511 riviä; 2 haastattelulistan vuotoa kiinni (raa'at 40yd/vertical-desimaalit, leventämätön OVR).
- [x] **#115** conductPersonalWorkout tiedostaa täyden raportin applyReportin läpi (8 mental + position skills, 0.9 conf); before→after liikkuu oikeasti; persoonasnapshot-suoja. Slice byte-identtinen. **Sivutuote: isScouted-auditti → #124 (36 UNSAFE-sitea, syncProspectGrades persistoi fabrikoidut bändit!).**
- [x] **#112** Dashboardin 4 sheet-modifieria → yksi .sheet(item:)-enum; Coach/Scout-fetcheihin careerID.
- [x] **#123** Hero = "Path to the Draft" (aidot prep-laskurit, vaihekone-CTA:t, ration loputtua Show Prospects + Advance samalla portilla kuin sidebar); pro day -lukot sanovat "opens after free agency" (yksi opensSentence-taulu); film-lopun advance hubin barille.
- [x] **#116** Mental-header-span, RPT=chargeable, tausta-param, onInterview-API (kytkentä #125), waitLabel-lyhenteet.
- [x] **#122** Film study: hinta+katto vain omista raporteista (Previous Staff ei laske), porras 20/35/55→15/25/40K.
- [x] **#98** K/P-hazard rakenteellisesti rikki (lievennys avautui vasta 39v) → isSpecialist 0.5× + wall+4 (elävät 41:een), washoutAge33 0.32→0.20. Career 36/36, a33 3.2%. **AUKI-osa: appin kausien 1-2 a33+-dippi (3.4→0.8→0.4%) EI poistunut — rig≠app-mekanismi, seuraa 8v-smokea.**
- [x] **#99** Varjopoolin juurisyy: nuori signaamaton ~0 kaikilla washout-portailla → washoutUnprovenFloor 0.35 + washoutSilentMarketFloor 0.22 (hazard 0.078→0.35). **Poolin KOKO vahvistamatta (smoke kesken; huom. refill-fallback generoi uusia pelaajia — katselmus F15).**
- [x] **#101** fifthYearOptionAppealFloor 74→80 (ovi B oli kuollutta koodia); rig 95.7→65.6% ≈ app 52-55%.
- [x] **#100+#88b** DealShape-repertuaari (needs swap, rental, buyer-avaus tarvepelaajalla, sweeten pick/2for1 depth-vahdittuna), 2 diiliä/klubi deadlinena, urgency 0.92: pelaaja mukana 0%→36%, deadline 6.75→13.0, bändit vihreät, roundup-kirje kertoo koko paketin.
- [x] **#88a** MLB bändissä (ei muutosta); **DT vahvistettu matalaksi mutta korotus kaataa 6.11e franchise-QB-bändin → #126 oma cap-tasapainotusaalto.**
- [ ] **Seuranta**: #97 (E:n perään jonotettu, LeagueGenerator-konfliktin takia), #124 (isScouted-siivous + save-korjaus), #125 (interview-kytkentä), #126 (DT), #127 (sopimusneuvottelujen cap-vuosi + Review Rosterin tuleva cap + kortin neuvottelunapit — käyttäjän illan viimeinen havaintosarja). Katselmuksen F14 (trade-passin PerfLog) ja F15 (#99 refill-mints) kirjattu tänne.

## Cap-vuosi + Class Depth + väristandardi + hub-IA -aalto (2026-08-06 yö, #127-#130 + #132/#136)

Neljä rinnakkaista Opus-agenttia → yhdistetty buildi vihreä ensiyrittämällä → adversariaalinen katselmus koko diffille (2 CRITICAL / 4 MAJOR / 10 MINOR, kaikki olennaiset korjattu F1-F13-nippuna) → tap-verifiointi scratch-simulla (Lions 2026 Review Roster -save).

- [x] **#127 Cap-vuosisemantiikka.** applyFranchiseTag ei enää koske annualSalarya/currentCapUsagea — tagi kirjautuu CommittedCapLedgerin uuteen forward-tauluun (seasonYear = sitomisvuosi, player-scoped) ja FreeAgencyEngine.settleFranchiseTags lunastaa sen rolloverissa ennen 5th-year-optioita. Remove Tag aito peruutus (+kellon palautus priorYears-stashilla). FranchiseTagView otsikoi seuraavan vuoden capin; komposeri näyttää katetut vuodet + aloitusvuoden projektion; kortilla Renegotiate/Extension ilman arviospoileria. Katselmuskorjaukset: tag-and-extend ei enää tuhoudu rolloverissa (rescind applyNegotiatedDealissa), Timeline offset-0 sisältää tagatun kuluvan palkan, careerID-skooppaus eksplisiittiseksi parametriksi, tagatut pois treidipoluista ("Franchise-tagged players can't be traded"), negatiivinen tila "Over by $X", 3% → capGrowthPerSeason. JÄÄNNÖS: applyNegotiatedDeal kirjaa yhä kuluvalle vuodelle (flat-salary-malli) — oma aalto.
- [x] **#128 Class Depth.** Uusi näkymä: declarations-header, 9 pelipaikkaryhmää, media-tier-palkit, tarve-chipit; Senior Bowl -taski laskeutuu tänne (ei tyhjälle combine-tabille); combinen pre-combine-tyhjätila rehelliseksi.
- [x] **#136 Class Depth ilman lukumääriä.** Käyttäjän havainto: "8 draftable" liian tarkka → kärkituomio ("Strong at the top" / "No top-end talent", liigaosuudella skaalattu, spesialistipoikkeus) + tier-legenda presence-only + a11y sanallisena. Syvyyspalkki säilyy mittarina.
- [x] **#129 Väristandardi + boardin percentiilit.** gradeColorForLetter (A+=eliteGreen, A=success, B=accentBlue, C=warning, D=alertOrange UUSI, F=danger); boardin Physical-moodi percentiiliväreillä + tier-labelit (Top 10% jne.), OVR+DRILL-sarakkeet, Physical auto-avautuu combine-kontekstissa. Katselmuskorjaus: percentiilirivin varaus yhtenäinen soluissa joilta puuttuu arvo.
- [x] **#130 Hub-IA.** ScoutingSurfaceSwitcher (Big Board / vaihe / Tools-menu, aktiivi 3 dimensiolla), prosessipalkki kompakti muualla kuin vaihepinnalla, Insights-osio per pinta (auki kerran per vaihe, muistaa valinnan; @AppStorage), positiochipit chromen pohjalle, laskeutumissääntö (Big Board ellei vaihe aidosti auki), ScoutingReferenceTabRow poistettu. Katselmuskorjaukset: combine-teaserin prefix ei pudota säilytettäviä faktoja, pinnanvaihdon 1-framen välähdys pois.
- [x] **#132 Cap Outlook ensimmäiseksi** myös aktiivivaiheessa (required-first-sort ohitetaan Review Rosterissa; gating-kopio ennallaan).
- [ ] **Seuranta:** QA-havainnot #133 (staff-laskurit 4 pinnalla eri luvut + budjetti elää relaunchissa), #134 (rank-flikkeri + Season Guide 1/7 vs 0/6), #135 (auto-hire palkkaa Conflict-fitit); katselmuksen kuolleet ledger-jäsenet + DraftPrepProcessBarin ternary-uupumus → #131-siivousaaltoon.

## Balanssi-megaaalto #97/#98/#99/#126 (2026-08-06 ilta)

Portit lopputilassa: career 36/36 · leaguegen 19/19 · draftclass 32/32, yhtään bändiä ei löysätty.

- [x] **#98 Veteraanichurn.** Juurisyy instrumentoituna: kolme ovea saturoituu ennen 33v (marketAppeal dockaa täydet 14 jo 30v → oma klubi ei voi re-signata alle OVR 86:tta; RosterValue.keepScore dockasi 3.2/v RAJATTA → 33v/88 hävisi rookielle cutdownissa molemmin puolin ovea; washout luki hiljaisuuden oikein). Korjaukset: agePenaltyCap 20 (puree vasta 31.25v), resignAIOwnCoreen erillinen veteraanibudjetti (2/klubi, 31v+, OVR≥76, successor-gap-testi), ChurnDiag a33-instrumentti. Smoke: a33+ s8 0.8%→2.4% (rig-ekvilibrium 3.3%), exitit retire 24 / washout 4 — mekanismi kääntyi kalibroidulle ovelle.
- [x] **#99 FA-varjopool.** Mintti: PracticeSquadEngine Pass 3 `while count < squadSize` generoi 2006 pelaajaa / 8 kautta (kahden vuoden diilit → purkautuvat FA:han); lisäksi metriikka laski 512 squad-miestä signaamattomiksi (teamID nil designilla). squadGenerationFloor 4 + shuffled klubijärjestys + rehellinen trulyUnsigned-metriikka. Mintti s4+ ≈ 0, aito pool tasaantuu ~207. JÄÄNNÖS → #144: squadit pyörivät 6-7/16 (rehellistä — ei 90-miehen leiriä josta täyttää), rakenteellinen camp-roster oma aaltonsa.
- [x] **#126 DT-kerroin 0.9→1.0 ilman kompensaatiota.** #88a:n 6.11e-kaatuma (0.746) ei toistu: nykypuulla 0.789-0.846 neljässä ajossa (floor 0.75) — oli budjettiahtauma-artefakti. Slack-kartta (CB/S/affordability) dokumentoitu vakiolla.
- [ ] **#97 AUKI, diagnoosi vaihdettu.** leaguePot 81.8 ≈ rigin konvergenssi 82.66 — headroom-transientti on käytetty, silti app 80+ 21-22% vs rig 16.1% SAMALLA potilla. Ero on realisaatiossa: `diag devsource` → offseasonDevelop +2.2 OVR/pelaaja/kausi (kaikki muut passit yht. +0.05). runwayCentre 0.55→0.40 kokeiltu: rig −2pp mutta 4 asserttia rikki (§6:n hit/elite-käyrät OVAT väite headroomista) → peruttu. Seuraava vipu: WeekAdvancerin processOffseason-syöte (opportunity/coaching/schemefit-kertymät), kirjattu smoken ANOMALY-riville.

## On-sim-verifiointien (#38/#43/#46) korjausaalto (2026-08-06 myöhäisyö)

Verifiointiagentti ajoi koko kaaren: draft (234 pickiä) → kausi → playoffit → HOF. Ydin toimii (trade-up, rookie-laskeutuminen, playoff-linjat kenttäpelaajille, aito HOF-résumé, #100:n treidimuodot vahvistettu pelissä 19 diilillä). 14 löydöstä → taskit #146-#156, joista 7 korjattu heti kahdella agentilla:

- [x] **#146 Pick-race (KRIITTINEN).** Kello juoksi confirm-alertin takana (8× nopeudella "19 s" = 2.4 s) → AI ehti kirjoittaa BPA:nsa ja käyttäjän vahvistus nieltiin `guard isUserOnClock`iin. Nyt: kello pysähtyy modaalien ajaksi (isClockHeld), selectProspect peruu clock-taskin ennen kirjoitusta + hylkää väärän slotin vahvistuksen, completePick idempotentti claimedPickNumbers-setillä (toinen kirjoittaja no-op, expiry ei tupla-advancea).
- [x] **#147 Hero-kortin oikeat NFL-nimet (HIGH).** "Williams · Daniels · Maye" oli neljä literaalia CareerDashboardissa — poistettu; hero lukee nyt aidon pick-tilan (WeekAdvancer.currentDraftPicks) ja targetit käyttäjän omasta boardista fogattuna, rehellisin fallbackein.
- [x] **#148 Draft-illan AI↔AI-treidit + litteä trade-up-hinta.** Sama juurisyy: ahne paketinrakennus avasi aina isoimmalla assetilla → 1.313× chart ylitti KAIKKIEN arkkityyppien katon (max 1.225) → 0 swapia 234 pickissä; ja käyttäjälle sama 800 pt -hinta joka kohteesta. Minimipakettihaku (≤3 assettia) → arkkityyppiparit 8/12→12/12, hinta seuraa chart-deltaa (385/352/312 pt kohteille #55/#58/#62). aiVsAiSwap katsoo nyt 6 putoajaa yhden sijaan.
- [x] **#153 Recap-modaalinippu.** Dismiss vaati 3-8 tapia = KOLME pinottua .sheet-modifieria no-op-settereillä (B1-luokan vika, 4. esiintymä!) → yksi .sheet(item:)-enum; kierroslista suodatettu kierrokseen; Pick 34/32 → aidot slotit; "#0 (-1 picks away)" → "No picks remaining"; REACH-beat mittaa nyt samaa julkista boardia jota se siteeraa (≥12 slottia).
- [x] **#149 QB:n playoff-heittostatsit nollina.** Juurisyy KAIKKIA kausia koskeva: accumulateStats kreditoi passingin/kickingin `offensePlayers.first { .QB }` -satunnaisfetchille kun muut perheet menivät pelin nimeämälle miehelle → uusi startingPlayer(at:) (sama paras-OVR-sääntö kuin PlaySimulator.findQB). Jäännös: historialliset savet pitävät väärän attribuution.
- [x] **#150 Neuvottelu-AI.** (a) Ratchet: vastatarjous hinnoiteltiin standalone-arvoilla mutta tuomittiin packageDecay+rosterSpotPenalty-tikkailla → myöntyvä paketti "ei riittänyt" ja hinta nousi (435→473). Nyt sama tikapuu molemmissa + standingCounter-takuu: GM:n omaa vaatimusta vastaava paketti hyväksytään aina. (b) Avoin thread: portti `lastActivityWeek < week` ohitti samana viikkona lähetetyn + hiljainen continue → `<=` ja hold-repliikit + expiry-varoitus. (c) Stance-flippaus: askNoise arvottiin joka viikko → avoin keskustelu hinnoitellaan avausviikolla.
- [x] **#151 Treidivalidointi.** Floor EI voinut purra: rosterBounds palautti 28 koko offseasonille → vaiheportainen (40 pro daysta eteenpäin) + uusi positional-integrity-sääntö molemmille klubeille (nollaa starter-positiota ei saa jättää) — se nappaa 16-for-1:n jota pääluku ei koskaan voi. Ylimaksun hylkäyskopio omaan overpayReason-haaraansa (4 persoonavarianttia) eikä enää kirjaa TradeTalk-strikea anteliaisuudesta.
- [ ] **Jonossa:** #152 (draft-vuosi off-by-one + urataulukon 2026-aukko), #154 (kopionippu: uutistemplaatit, PotW-skew, Wk18-batch, opponent-labelit, HC-arkisto, advance-kopio), #155 (DraftEnginen inertti konsensusbonus — lukee ROUNDIN pick-numerona! — + positioarvopaino; PerceptionScenario-portitettu balanssiaalto), #156 (finalizePostseasonHistory syntetisoi vastustajan tyhjän real-linen yli), #137/#138-#145.

## "Toteuta kaikki todot" -loppuaalto (2026-08-07 aamuyö, 3f07f63..ce5ad57)

Viisi rinnakkaista agenttia, viisi slicea, kaikki portit vihreät joka välissä.

- [x] **#137/#139/#142/#145** (`3f07f63`): boardin suositukset deterministisiksi (viiden tasapisteisen tarpeen dict-järjestys) + fogattu rankkaus; Insights-epäjohdonmukaisuus oli EDELLISEN URAN perintöä (globaalit avaimet → career-token); chipit boardin isännöimiksi; pikatoiminto 4 pinnalle jaetulla gatella; heal-polku portitettu + todistettu ettei herätä fabrikoituja bändejä.
- [x] **#140/#141/#143/#152/#154/#156** (`08df531`): Motivator-label; tagatut pois expiring-listoilta + valintablokki + Propose harmaaksi; poach-inboxviesti + medical-tenure; draft-vuoden off-by-one (inkrementti laukeaa 5 vaihetta draftin jälkeen → DraftYearLabel +1 KAIKKIIN printattuihin vuosiin, tallennetut koskematta); milestonet ylityshetkellä + templaattivarianssi UUID-hashilla + PotW-positiobias + GM+HC-arkisto; postBoxScoreGames-proveniens (aito 0-suoritus ei enää syntetisoidu yli).
- [x] **#144** (`b48372d`): squad-täyttö aidoista miehistä — accrued seasons (≤2 kautta 6+ ottelulla 53:lla; squad-vuosi ei kerrytä = liigan oikea sääntö), veteraanislotit keepScorella, round-robin. Mintti 9-87/kausi → 0 joka kausi, syvyys min6/max9. 12/16 aritmeettisesti mahdoton ennen #157:ää (yp0to3-ylitys ja squad-vaje = SAMA vipu).
...
Pushattu `775cae0` asti. **PÄIVITYS 2026-08-10: MEGARUN AJETTU — kohdat 1-4 ja 6 TEHTY. PÄIVITYS 2026-08-10/11: MEGARUN 2 AJETTU — ~~#97~~, #157 osittain, ~~#175~~, ~~#176~~, ~~#177~~, #167 aalto 1 TEHTY (committoimatta työpuussa, ks. Megarun 2 -osio alla). Jäljellä:**

**TILANNE 2026-08-14 (#199-siivousaalto): tämä jono on pääosin historiaa — älä lue sitä työlistana.**
Pushattu `3423d35` asti. Kohdat **3–19 (#178–#197) ovat KAIKKI TEHTY**, samoin #200–#208; ledgerit
ovat tämän osion alla: `## PÄIVÄ 2026-08-11 MAALISSA` (#178–#197 + päivien 2-3 rivit #200–#208).
Rivejä ei poistettu — speksit ovat yhä paras kuvaus siitä *mitä* tehtiin. Jokainen suljettu rivi on
merkitty alkuun `[TEHTY …]`.

**Aidosti auki tämän jonon riveistä:**
1. **#157-jäännös — [TEHTY 2026-08-14, aalto 1]** PS-diagnostiikka rakennettu shadow-mittauksena (CareerScenario.harness) → numerot EIVÄT oikeuta vipua (depth 11.2/16, 0 klubia genFloorin alla, signee-jakauma terve) → #157 SULJETTU mittaustuloksella.
2. **#167-LOW-jäännökset — [TEHTY 2026-08-14, aalto 1]** 6 audiorenamet + MusicDirector, publish/dev-jsonien SB-stringit fiktioiksi generaattoria myöten (TVAL-portti läpi), Cantonit pois, trademark_guard laajennettu.
3. **#198 — [TEHTY 2026-08-14, aalto 1]** (kohta 19a alla) — chatter-loki, round-rollover + RoundRecapSheet→DSResultSheet, board pressure, v3.3-jäännökset; kuvatuomari ACCEPT 10/10 molemmissa orientaatioissa.
4. **#199-kasa** — TYÖN ALLA 2026-08-14: camp-täyttö 80→87 + campShape 86→94 (`CampRosterEngine`),
   UDFA-kattoauditti (`UDFAMarketEngine.draftNightUserWindow`), FAWeeklyView'n pending-offers-palkki
   DS-tokeneille, design-token-baseline lukittu. Jäännökset alla omassa listassaan.
5. **#209 — [TEHTY 2026-08-14, aalto 2] Swift 6 -concurrency-varoitusvelka: 86 → 19 uniikkia varoitusta.**
   Alkuperäinen "~380-670" oli raakalukema; dedupattuna (`grep -E '^[^ ]+:[0-9]+:[0-9]+: warning:' <log> | sort -u | wc -l`
   — sama diagnoosi toistuu targeteittain) lähtötaso oli **86**. Loppubuild `scratchpad/w2-final.log`:
   BUILD SUCCEEDED, 0 virhettä, 772 SwiftCompile-entryä (täysi puhdas käännös, derived data poistettu ensin
   — osittainen build valehtelee varoitusluvun). **Kaikki kolme nimettyä luokkaa NOLLASSA:**
   - **(1) main-actor-isolated Codable-konformanssit 33 → 0** (pass 1): `Career.swift`:n 8 sisäkkäistä
     tilatyyppiä + `SeasonStatLine` / `FaceAssignmentRegistry` / attribuuttistructit `nonisolated`iksi.
     `@Model`-skeemaan ei koskettu, CodingKeyt ja kenttäjärjestys ennallaan → tavuyhteensopiva.
   - **(2) NSLock async-kontekstissa 2 → 0** (pass 2): vain `UI/Common/PersonFaceView.swift`:n
     `FaceImageCache`-ledger (koko repossa 4 lukkoa, muut 3 eivät varoittaneet). Lukkojen suoja-ala ennallaan.
   - **(3) main-actor-isolated static/init nonisolated-kontekstista 32 → 0** (pass 3): 29/32 oli sama
     kuvio — **metodiviittauksen konversio funktioarvoksi riisuu isolaation** (`sorted(by: Self.byID)`,
     `map(SimPlayer.init(from:))`, `filter(ProspectFog.hasOwnReport)`, `scheme.map(Playbook.mix(for:))` …).
     Korjaus = eta-ekspansio sulkeumaliteraaliksi (`sorted(by: { Self.byID($0, $1) })`); ei-Sendable
     sulkeumaliteraali perii kutsujan isolaation, joten semantiikka on identtinen eikä hyppyjä synny.
     Loput 3: `VisitTracker.defaultDurationHours` ja `FaceGeneratorConstants.facesFolder` `nonisolated`iksi
     (immutaabelit Sendable-vakiot), ja `enum LeagueTemplateLoader` `nonisolated`iksi — sen ainoa off-main-kutsuja
     (`TeamSelectionView`:n `Task.detached`) ajoi sen jo taustasäikeellä, joten annotaatio vastaa nyt totuutta.
   **Jäljellä 19 = pitkä häntä**, tarkoituksella koskematta (ei-concurrency, ei riskin arvoista): 8× käyttämätön
   `let`/`var`, 3× `'where' only applies to the second pattern match`, 2× `Text`-`+` deprecated iOS 26.0,
   `default will never be executed`, `withAnimation`-tulos käyttämättä, ViewBuilder + explicit `return`,
   preview-makron `entry1`, sekä yksi concurrency-jäännös: `UI/FreeAgency/FAWeeklyView.swift:2459`
   `'retroactive' attribute does not apply; 'FreeAgent' is declared in this module` (= Swift 6 -moodissa virhe,
   yhden attribuutin poisto, mutta oma pikkukorjauksensa). Uusia varoituksia ei syntynyt yhdelläkään passilla
   (`comm -13` vanha/uusi = tyhjä).
6. **5e** (kohta 5e alla) — käyttäjän silmätarkistus, ei koodityötä.
7. Käyttäjältä: #5 (iPad-katselmus), #74 (musiikit), IP-juristi, privacy policyn hostaus.
8. **UUSI #210 (käyttäjä 2026-08-14): Life Events -systeemi — DESIGN VALMIS, toteutus odottaa käyttäjän katselmusta.** Satunnaiset off-field-tapahtumat (pidätykset, aine/vedonlyöntikurinpito, avioliitot/perhe, non-football-vammat NFI-listalle, mielenterveys, mediadraama, positiiviset+milestonet) persoona/attribuuttigatein, kalenterisolmuista rullattuna, kestot + pelaajakortti-badget (SUSPENDED · N GM jne.) + kortin aikajanahistoria; uutiset valmentajalle, isot pop-upeina, 29+33 aitoa valmentajapäätöstä. **Design: docs/LIFE_EVENTS_PLAN.md · katalogi 347 tapahtumaa docs/life-events/catalog/*.json (K1 legal 121 · K2 personal 113 · K3 drama/positive 113) · tutkimus docs/life-events/research/ (4 raporttia, ~4100 riviä).** Arkkitehtuuri: 3 blob-saraketta Careeriin (EI V2-migraatiota), kuollut EventEngine absorboidaan, seeded RNG (careerID,season,salt), W0-esiehto = jaettu PlayerAvailability (~25 kutsupistettä; sama esiehto kuin #212:lla).
...
3. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **UUSI #178 (käyttäjän katselmushavainto 2026-08-11): Salary Cap -ruutu kahteen vuosiosioon.** Nykyruutu näyttää vain kuluvan vuoden yhteenvedon + 3 kauden outlook-palkit. Haluttu: OMA OSIO "This Year (kuluva liigavuosi)" (nykyinen Cap Summary + usage + dead money ryhmiteltynä) ja OMA OSIO "Next Year (N+1)": projisoitu cap (+7%/v — sama kaava kuin outlookissa), committed-summa (outlook laskee jo), projisoitu tila, tiedossa oleva siirtyvä dead money (restructure/cut-kiihdytysledgerit #102), ja linkitys expiring-listaan (mitkä erääntyvät ja net-vaikutus — kaava on jo expiring-footerissa). Cap Outlook -palkit jäävät alle pidemmäksi horisontiksi. Tiedosto: UI/Contracts/CapOverviewView.swift. TOTEUTUS ODOTTAA luettavuussweepin (wf_6444cdd8) valmistumista — sama tiedosto työn alla.
4. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **UUSI #179 (käyttäjän katselmushavainto 2026-08-11): Class Depthin "Depth by position" -kortin redesign — luettavuus + intuitiivisuus.** Ongelmat: segmenttipalkki ilman lukuja + selittämätön pystytikku; ~8pt piste-legenda toistuu joka kortissa; kolme signaalia ilman labeleita kilpailee ("Strong at the top" = kärjen laatu, DEEP-pilli = volyymi, NEED = oma tarve — näyttävät päällekkäisiltä tuomioilta); positiokohtainen declared-lukumäärä puuttuu. SPEKSI: (a) headeriin "POS · Full name · N declared" + oikealle YOUR NEED -pilli; (b) signaalit nimetään: "Top end: Strong · Class depth: Deep" labeloituina pareina; (c) palkki+legenda → väritetyt tier-chipit lukumäärä sisällä ("Blue chip 2 · Rd 1-2 4 · Rd 3-5 5 · Late+UDFA 3", ≥11pt) — data on jo palkkisegmenttien takana (ScoutBoardReads); ohut palkki saa jäädä chippien alle sekundaarina, legenda pois; liigakeskiarvotikku vain jos labeloitu, muuten pois; (d) top-3-rivit + fog-bändit ennallaan. Tiedostot: ClassDepthView.swift (+ScoutBoardReads jos laskenta tarvitsee count-APIn). ODOTTAA wf_6444cdd8-sweepin valmistumista (sama tiedosto työn alla) — toteutetaan #178:n kanssa samassa aallossa.
5. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **#180 (käyttäjä 2026-08-11): instrumenttipillirivi rikki — JUURISYY LÖYTYNYT, korjaussuunnitelma valmis.** #117-aikainen täysleveä HStack (5 pitkälabelista pilliä, ~600pt intrinsic) jäi W4a-detaljiuudistuksessa (717541f) DSDetailColumnsin ~305pt keskisarakkeeseen → SwiftUI puristaa jokaisen Textin alle sanaleveyden, kirjain/rivi. Rikki KAIKILLA iPad-leveyksillä. FIX: privaatti StatusPill-rivi (ProspectDetailView.swift:1283-1289, :2799-2809) → jaettu DSStateSlotRow/DSStatusPill lyhyin identein (FILM/MEET/PRO DAY/WORK/T30, ~250pt). Samalla: privaatti StatusPill-duplikaatti pois, 2 privaattia FlowLayout-kopiota (PositionVersatilityView:770, SquadDynamicsView:1128) → jaettu. **LAAJENNUS (käyttäjän lisähavainto 2026-08-11, Image #7/#8 — film study "ei näy kunnolla missään"):** pelkkä spent-status ei riitä — tehdystä instrumentista pitää nähdä MITÄ SE TUOTTI: (a) set-tilainen slotti on TAPATTAVA ja avaa tuloksensa (FILM → FilmStudyResultSheet [on jo olemassa #117:stä], MEET → interview-osioon, WORK → workout-tulokseen); (b) "N reports on file" -rivit nimeävät lähteensä lukumäärän sijaan ("Combine trip · Film study" — käyttää #184:n ownReport-erottelua); (c) slottiriville lyhyt yield-teksti set-tilassa (esim. FILM ✓ "tightened 4 bands"). Intuitiotavoite: yhdellä silmäyksellä näkee mitä on ostettu JA mitä sillä sai.
5b. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **UUSI #183 (HIGH — tutkinnan päälöydös): GradeRange.incorporate LEVENTÄÄ bändiä kaventamisen sijaan.** LetterGrade.swift:127-134: shift-suunnat käänteiset kommentteihinsa nähden (shifted(by:) positiivinen = parempi; ProspectFog.widened :742-746 tekee saman OIKEIN). Seuraus: TOINEN raportti tuottaa epätarkemman luvun kuin ensimmäinen (B/A+ → B-/A+ — käyttäjän screenshotin overall on täsmälleen tämän sormenjälki), kunnes kolmas romahduttaa yhteen kirjaimeen; myös haastattelu leventää combine-bändiä. Rikkoo tyypin oman lupauksen, UI-copyn ("Each one narrows his grade band") ja hintaportaan ($25K huonommasta luvusta). Korjaus + tarkista syncProspectGrades/honestUnbackedBand-vuorovaikutus (CareerShellView:2973-3013).
5c. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **UUSI #184: raporttilaskurin splitbrain + liput aukeavat liian aikaisin.** ProspectFog.mentalDisclosure/positionSkillDisclosure (:514-530), CollegeProspect.scoutReportCount (:479-481) ja hero-badge laskevat perityn "Previous Staff" -rivin mukaan (joka EI kirjoita yhtään bändiä) → "2 reports on file" / "Medium confidence 2/3 scouts" vaikka omia raportteja on 1; sama kortti näyttää 1/3 toisaalla (chargeableReports). Pahin: flagDisclosure (:615-627) avaa medical/character-kansion kokonaan kynnyksellä count≥2 → peritty ilmaisrivi + 1 combine riittää = liput auki yhden instrumentin liian aikaisin. Korjaus: kaikki → ProspectFog.ownReportCount (on jo olemassa, :124-128). Pikkujäännös samalla: kattoclampattu 3-leveä ensibändi (A-/A+) paljastaa hiljaa että scout näki A+:n — harkitse leveysvakiointia.
5d. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **UUSI #185: Personality-putki epärehellinen.** ScoutingEngine.applyReport:429-440 re-rollaa scoutedPersonalityn kiinteällä 70% kolikonheitolla kun MIKÄ TAHANSA raportti (myös peritty) sisältää personalityNotesin: (1) scout.personalityRead + character-fokus ohitetaan kokonaan — OIKEA logiikka on olemassa mutta kuolleessa koodissa (scoutProspect :136-172, 0 kutsujaa); (2) haastattelun lukema ylikirjoitetaan mutta label sanoo yhä "From your interview"; (3) workout-polku sai F7-suojauksen, pääpolku ei; (4) kortti näyttää ehdottoman arkkityypin ilman bändiä/konfidenssia vaikka arvo on 30% ajasta uniformisti väärä, ja FilmStudyResultSheet sanoo samasta raportista "Hard to get a clear read". FIX: PersonalitySource-jäljitys (.leagueConsensus/.report/.interview/.workout) joka kirjoittajaan, interview-suoja, scoutin attribuutti käyttöön, label lähteen mukaan.
5e. **[AUKI — käyttäjän silmä]** Avoin silmätarkistus käyttäjälle: Position Skills -rivin **ELU B/A** ei synny mistään koodipolusta (kaikki 144 incorporate-paria käyty) — BTK/RCV samalla rivillä ovat B-/A. Jos solu on oikeasti B/A eikä B-/A, se on tuntematon kirjoittaja (uusi bugi).
6. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **#181 (käyttäjä 2026-08-11): College Production — TUTKITTU (wf_3c694b60, 21 000 prospektin mittaus). Ydinkorrelaatio KUNNOSSA, esityskerros valehtelee sen ympärillä.** Tier korreloi gradeen aidosti (r=0.685, lisää tietoa bändin sisällä +0.24…+0.51) ja competition-diskontto toimii. BUGIT: (A HIGH) statsirivi skaalataan years/3-kertoimella mutta tier on years-riippumaton (corr years↔OVR = +0.001) → "Elite + 477 yds" EI ole glitch vaan 22.4% kaikista Elite-prospekteista; screenshotin rivi toistuu bitilleen ~neljänneksessä luokista (1100×1.30×1/3=477). (B/C HIGH) footnote on ehdoton literaali (ProspectDetailView:1911) JA väittää kausaliteettia jota ei ole — production EI vaikuta gradeen/kattoon (se johdetaan overallista, Step 7; osittaiskorrelaatio kattoon −0.019). (D) statsirivillä nolla per-prospekti-varianssia — 16 uniikkia stringiä/positio koko pelissä. (E) tier-kerroin skaalaa myös negatiiviset statsit — Elite-QB heittää ENITEN inttejä, Elite-OL päästää eniten säkkejä. (F) kausi/ura-semantiikka sekaisin → mahdottomia arvoja (57 OL-starttia, 38/49 FG/kausi; FG% ei kanna tier-infoa lainkaan). (G latentti) legacy-fallback vuotaa trueOverallia (generatorVersion<2). FIXPLAN (semantiikka ensin): tier = rate, years = volyymi, statsirivi = PARAS KAUSI → pudota years/3 kausipositioilta ("Elite 1/4" printtaa 1430 yds — yhden vuoden breakout, aito arkkityyppi), kumulatiiviset laskurit peleistä (OL min(4,years)×12), negatiiviset käänteisesti, ±8% deterministinen jitter prospektin id:stä, footnote ehdolliseksi ja kausaliteettiväite pois. **KÄYTTÄJÄPÄÄTÖS 2026-08-11: matala tuotanto SAA kätkeä helmen — mekanismi on OTOKSEN PUUTE, ei valehteleva signaali** ("yleensä se johtuu siitä että eivät pelanneet paljoa 2./3. stringinä"). SPEKSI — "usage-suppressed production": (a) pieni osuus luokasta (~4-6%) generoituu "buried on the depth chart" -polulla: rooli backup, snapit vähäiset (yearsStarted 0-1; salli 0/4), production-SCORE lasketaan usage-volyymista EIKÄ overallista → julkinen tier Below Avg / uusi "Limited sample" -tila korkeasta trueOverallista huolimatta; (b) usage-suppressed-joukon kykyjakauma SEKOITETTU: enemmistö aidosti keskinkertaisia (siksi istuivat), aito vähemmistö helmiä (istuivat eliittistartterin takana — narratiivikoukku talteen, esim. "sat behind a first-round pick"); (c) fog: julkinen board/mediaprojektio seuraa productionia → helmet projisoituvat myöhäisille kierroksille; scout-instrumentit (film/workout/interview) lukevat kyvyn normaalisti → löytäminen vaatii työtä; (d) UI: näille rehellinen kopio ("Career backup · 89 snaps · small sample"), EI laatutuomiota — Limited sample -chip eri värillä; statsirivi seuraa usagea; (e) balanssivahdit: osuus rajattu, draftclass-portin §7-bändit johdetaan uudelleen perusteineen (production↔overall-korrelaatio laskee tarkoituksella hieman), #155 AI-ankkuri saa seurata julkista boardia (helmet putoavat — se on pointti) mutta hyvän scouting-laadun AI-seura voi joskus napata; (f) toteutus samassa aallossa muiden #181-fixien kanssa (paras kausi -statsirivi, negatiivisten kääntö, jitter, footnote).
7. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **#182 (käyttäjä 2026-08-11): Big Boardin MEET-numerot (82/94) — SELVITETTY: TARKOITUKSELLINEN.** MEET-sarake = haastattelun tuottama TARKKA football-IQ-luku (BigBoardView ~1588-1605: "An exact football-IQ number, because that is what a meeting produces. A dash means you have not spent an interview slot on him"); sininen tarkka luku on tietoinen kontrasti scoutauksen kultaisiin fog-bändeihin — täsmällisyys on haastatteluslotin palkinto. Headerissa on ⓘ-tooltip. KÄYTTÄJÄPÄÄTÖS 2026-08-11: kirjainarvosana samaan tapaan kuin muut sarakkeet. TOTEUTUS: MEET-sarake → kirjain samalla asteikkomappauksella kuin muut (82→B, yksi kirjain koska lukema on eksakti, ei bändi); detaljikortin Interview-osio pitää muodon "B (82)" (tarkka luku säilyy palkintona siellä); sarakkeen ⓘ-tooltip päivitetään. Post-sweep-aaltoon.
8. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **UUSI #186 (käyttäjä 2026-08-11, HIGH): jatkosopimusneuvottelun cap-portti tarkastaa väärän vuoden + tagi-korvaus puuttuu.** Havainto (Goff, tagattu $41.9M/1v, "Talks ongoing"): tarjous jonka UI itse labeloi "Covers 2027–2030 — his current deal runs through 2026" estetään "Over over this year's cap by $6.1M — the new rate goes on the books as soon as he signs, not in 2027" — vaikka 2027:ssä on +$54.6M tilaa. KAKSI VIKAA: (a) portti vertaa kuluvan vuoden cappiin vaikka sopimus kohdistuu 2027+ ("uusi sopimus pitäisi olla vasten niitä cap-lukuja mille sopimus kohdistuu"); (b) tagatulla pelaajalla jatkosopimuksen pitää KORVATA tagi (tag-and-extend): tagin osuma pois kirjoista, uuden diilin vuosi-1 tilalle — nyt uusi rate tulisi tagin PÄÄLLE. SPEKSI: neuvotteluportti + applyNegotiatedDeal määrittävät kohdevuodet: tagattu → diili alkaa kuluvasta liigavuodesta tagin korvaten (portti: used − tagHit + year1Hit vs kuluva cap); ei-tagattu jolla vuosia jäljellä → diili alkaa nykysopimuksen jälkeen (portti: kohdevuoden committed + year1Hit vs projisoitu cap, sama +7%-kaava kuin outlook); footer-kopio kertoo kumpaa vuotta vasten tarkistetaan. **MOOTTORIOSA TEHTY (wf_04ef52a2): uusi Engine/Contract/DealTargetYear.swift** — yksi Plan-malli jota sekä portti että kirjaus lukevat (eivät voi eritä): kolme shapea (tagReplacement = tag-and-extend, tagin vuosi on diilin vuosi 1 eikä termi stackaa; deferredExtension = forward-kirjaus kohdevuodelle; immediate). Diagnoosin avainlöydöt: vanha portti oli JO algebrallisesti tag-netting (siksi literaali spec olisi toistanut saman eston — todellinen fix on ettei 2027-diiliä portata/kirjata 2026:lle lainkaan); tagilla on KAKSI tilaa (pre-rollover forward-rivi / post-rollover annualSalary) — molemmat hoidettu; committedStartYear tuplalaskubugi 4 pinnalla; AI budjetoi jo forward-vuotta (resignAIOwnCore) = epäsymmetria käyttäjää vastaan. Katselmus 8 löydöstä → 5 engine-fixiä tehty (space()-tuplalasku forward-coverage-mapilla, moni-forward-avaimet <uuid>#<season> taaksepäinyhteensopivasti, tagReplacement-saavutettavuus, deferred→Contract-rivi, AI-deferral-symmetria). **UI-JÄÄNNÖS FINAALIAALTOON (ilman näitä #186 ei näy käyttäjälle): (a) ContractNegotiationView'n portti → DealTargetYear.Plan (vanha oma aritmetiikka pois, estoviesti nimeää vuoden), (b) CapOverviewView'n projected-committed samaan kaavaan.** Harness-portit vihreät.
9. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **UUSI #187 (käyttäjä 2026-08-11): sarakelajittelu kaikkiin listoihin + FA-ruudun available cap näkyviin.** (a) W1-listastandardiin (DSListRow-headerrivi) tapattavat sort-headerit: tap sarake → desc, toinen tap → asc, chevron-indikaattori, oletussortti per ruutu, VAKAA id-tiebreak (#134a-oppi: ei-totaali komparaattori määrittelemättömän järjestyksen päällä = arpapeli). Adoptio: FA-signing-lista (TÄRKEIN, käyttäjän esimerkki: OVR/AGE/ASKS/YRS-sarakkeet), Roster, League Rosters + muut DSListRow-kuluttajat joilla ei ole omaa sorttia (Player Contracts -listalla ja scouting-listoilla on jo). (b) FA-signing: CAP ROOM/AVAILABLE on oikeassa yläkulmassa mutta ei pysy näkyvissä — sticky listan mukana + pending-tietoinen ("available after pending offers" kun tarjouksia pöydässä). UI-aaltoon.
10. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **UUSI #188 (käyttäjä 2026-08-11, HIGH): release ei toimi — tai toimii PUOLIKSI.** Käyttäjä vapautti pelaajan (yli capin, FIX IT -flow): cap ei päivittynyt, pelaaja ei poistunut listalta. JOHTOLANKA screenshotissa: "Active contracts $298.2M + Ledger variance −$2.9M = used cap $295.3M" ja Dead Money -kortti näyttää −$2.9M SAMALLA kun kopio sanoo "No dead money on the books" — negatiivinen ledger-poikkeama = kirjanpito sai hyvityksen mutta sopimus/pelaaja jäi. **TUTKITTU (wf_1fa45e87) — viisi bugia, moottori KUNNOSSA, save EI korruptoitunut:** (1) PÄÄBUGI: PlayerDetailView:599-605 Release-alertin closure on TYHJÄ ("// Release action handled by parent") eikä parent-callbackia ole olemassa (:374) — nappi engine-hinnoiteltuine dead-cap-kuvauksineen (:1992-2008) on täysi no-op; peräisin 3bc5354:stä, EI megarunista; sama kuollut-{}-luokka jonka "Set as Starter" -edeltäjä poistettiin. (2) CapOverviewView:100 päivittyy VAIN .taskissa @State-snapshotteihin — FIX IT on NavigationLink-PUSH → popissa ei reloadia → players-lista jää vanhaksi mutta team.currentCapUsage on live → dead = live − stale menee negatiiviseksi täsmälleen releasen capSavingsin verran = "Ledger variance −$2.9M" + vapautettu pelaaja yhä listalla (RosterCutView-luokan jäätynyt-snapshot-kuvio, esiintymä #2). (3) Dead Money -kortti (:392-416) printtaa negatiivisen arvon JA "No dead money" -kopion — ei dead<0-haaraa; summary-pilli käyttää deadLabel():ia, kortti ei. (4) CapComplianceView.releasePlayer:529-545 ilman eksplisiittistä savea (epäsymmetria applyRestructure/applyPayCutiin). (5) Vain RosterCutView kirjoittaa RosterCut-kuitteja → compliance/PlayerContractView-releaset eivät koskaan attribuoidu nimelle. Engine applyRelease (CapManagementEngine:442-508) on ATOMINEN JA OIKEIN. Huom: variancen voi tuottaa myös #87-dokumentoitu capHit-vs-annualSalary-lähde (:416). FIXIT INTEGRAATIOAALTOON: (a) PlayerDetail-nappi → applyRelease+save+dismiss (sama polku kuin PlayerContractView.cutPlayer), (b) CapOverview live-refresh (RosterCutView-fixin kuvio), (c) dead<0-haara rehellisellä kopiolla, (d) save-symmetria, (e) RosterCut-kuitti applyReleasen SISÄÄN → kaikki polut attribuoituvat.
11. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **#189 TUTKITTU (wf_d487d30b): recap-nollat = TUPLAPAINALLUS ylikirjoitti aidon kuitin — kiertueen data meni perille.** (a) CTA:lla ei ole one-shot-gatea: advanceSection gatettu vain canActilla, joka on tarkoituksella ikuisesti auki aiemmille stageille (DraftPrepProgress:255-257, film-study-fix) — väärä kysymys kertaluontoiselle suoritusnapille; hasRunTour (:117) on olemassa eikä sitä käytetä CTA:ssa. (b) Toinen painallus → needsRun false kaikilla 25 koululla → schools=[] → guard applied läpäisee silti (DraftClassMutator palauttaa true jos luokka on olemassa) → tourResult ylikirjoittuu nollilla ("no-op with a receipt" — F5-luokka jonka tiedoston oma header väittää mahdottomaksi). (c) LATENTTI: tourResult on @State switch-arm-näkymässä → recap katoaa tab-vaihdossa; käyttäjän omasta kiertueesta EI jää mitään pysyvää artefaktia (liigakiertue saa news/inbox-digestit, käyttäjän ei). Data OK: attendProDay kirjoitti raportit+proDayCompleted+fog→full DraftClassMutatorin kautta (persistoitu). FIXIT: CTA+skip hasRunTour-gaten taakse ("Circuit complete" -tila), advanceStage ei ylikirjoita ei-tyhjää kuittia, kuitti persistoidaan (CareerScopedDefaults/record) + inbox-digest käyttäjän kiertueesta, sub-label rehelliseksi.
12. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **#190 TUTKITTU (wf_d487d30b): lista tyhjä koska KAKSI oletussuodatinta — EI datavika (kandidaatteja ~286).** PÄÄSYY: WorkoutsTabView:84 `boardOnly = true` oletuksena → näyttää vain käsin Elite/Target-merkityt; merkitsemättömällä uralla 0 riviä (sisarnäkymät Interview/Top30 tekevät oikein). TOISSIJAINEN: :135 kunnioittaa jaettua positionFilter-bindingiä mutta hub EI piirrä chip-riviä workouts-tabille (ScoutingHubView:87) → Big Boardilla jätetty positiovalinta rajaa näkymättömästi. + 4 sivubugia: tyhjätila-kopio valehtelee ("Nobody left to bring in"), blockedReason testaa väärää joukkoa, hasRecommended-splitbrain (voi näyttää no-op-napin), kertaluontoinen .task-refresh. FIXIT: boardOnly=false oletukseksi (oikea kandidaattijoukko = declared ilman .personalWorkout-raporttia — :869 laskee jo), positiochipit workouts-tabille, rehelliset tyhjätilat, blockedReason filtered-joukosta.
13. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **UUSI #191 (käyttäjä 2026-08-11): prospektikortin Back vie aina Big Boardille — pitää viedä EDELLISEEN näkymään** (Class Depth / Mock Draft / workoutit / mistä tultiinkin). Navigaation alkuperä säilytettävä: kortti avataan eri pinnoilta, dismissin pitää palata avauspinnalle (NavigationLink-pino tai dismiss — ei kovakoodattua Big Board -reittiä).
14. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **UUSI #192 (käyttäjä 2026-08-11): Mark prospect -kontrollit headerin tyhjään kaistaan valmiina nappeina.** Kortin headerissa on tyhjä alue (käyttäjän Image #16 -rajaus); Elite/Target/Depth/Avoid-tier-napit sinne suoraan tapattavina + nykyinen mark-tila näkyviin. Kelluva alavasemman "Mark"-nappi POIS (redundantti); UNMARKED-alabanneri jää statukseksi mutta päivittyy; MY VERDICT -kortti vasemmassa sarakkeessa ohenee note-keskeiseksi (nappien duplikaatti pois). Sama yhden-kultafillin sääntö säilyy.
15. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **UUSI #193 (käyttäjä 2026-08-11): "Finalize Big Board" ei saa olla kuitattava taski.** Käyttäjän design-linjaus: boardin viimeistely on hyvä OLLA viimeinen steppi, mutta EI mitään "nyt tehty" -nappia — **draftiin siirtyminen ON finalisointi**. FIX: poista Finalize Big Board -taski pre-draft-checklististä (tai muuta passiiviseksi linkiksi boardille ilman kuittausta); "Advance to The Draft" toimii implisiittisenä lukkona. Samalla siivotaan duplikaatti: "Finalize Big Board" esiintyy sekä PRO DAYS & WORKOUTS -stepissä että THE DRAFT -stepin listassa. HUOM screenshotin varsinainen blokkeri oli "Get under the salary cap" (Required) — joka oli jumissa #188:n kuolleen release-napin takia; UI:n taittojärjestys sai sen näyttämään Big Board -blokilta → varmista että "Complete N required task(s)" -banneri nimeää blokkaavan taskin.
16. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **UUSI #194 (käyttäjä 2026-08-11): Draft-näkymän iso uudistus — "muokata PALJON".** Käyttäjän vaatimukset: (a) generoitu war room -taustakuva (alkuperäinen, EI NFL-materiaalia — bg_draft.jpg poistettiin juuri #167:ssä; faces-pipelinen Replicate-putki käytettävissä), (b) joukkueet oikeilla väreillään (LeagueTeamDatan värit on-the-clock-bandiin/pick-bandiin/tickeriin), (c) paremmat toiminnot ylös kontekstin mukaan (esim. "Trade for pick" kun EI ole oma pikki — trade-up-flow on jo engine-tasolla 9a53918:sta), (d) analyysi pelikehittäjän JA pelaajan näkökulmasta: koukuttavuus + kauneus. ENGINE KOSKEMATTA (DraftDayCoordinator/Mode-kone/AI-treidit) — presentaatio + action-wiring. Workflow ajossa (wf-194).
17. **[TEHTY — ledger: `## PÄIVÄ 2026-08-11 MAALISSA`]** **UUSI #195 (käyttäjä 2026-08-11): draft-kellon pause/resume + Skip to pick.** (a) BUGI: Pause → Resume nollaa pikkikellon alkuun — pitää jatkaa siitä mihin jäi (talleta remaining, älä restarttaa täydellä duraatiolla; huom 9a53918:n hold+atominen claim -mekanismi — älä riko sitä). (b) Alapalkkiin selkeä "Skip to my pick" -nappi joka hyppää odotuksen (pikakelaa AI-pikit omaan pikkiin asti): tarkista mitä nykyinen "My pick" -nappi OIKEASTI tekee (skippaus vai scrollaus?) — jos se jo skippaa, ongelma on labelissa/affordanssissa; joka tapauksessa lopputila = yksi selkeä skip-affordanssi. Toteutus #194:n perään (samat tiedostot työn alla).
...
**Megarun-katselmuksen LOW-rivit — kolme nimettyä, kaksi oli jo korjattu:**
- ~~staff-sheetin Continue-välähdys~~ **JO KORJATTU** ennen tätä aaltoa. `CoachingStaffView.swift:1444-1455`
  dokumentoi juurisyyn ja fixin: `sheetOutcome`-nillaus tapahtui samalla runloop-kierroksella kuin
  dismiss, joten `Group` vaihtoi kasvot kesken sulkeutumisanimaation; nyt `onContinue` vain sulkee ja
  `sheet(onDismiss:)` on ainoa paikka jossa outcome kuolee.
- ~~OwnerBriefingin ylimääräinen kultafilli~~ **JO KORJATTU** #177:ssä. `OwnerBriefing.swift`:ssä on
  **nolla** `accentGold`/`ctaPrimary`-osumaa; tiedosto kantaa nyt vain kommentit siitä miksi
  "View All" -pilli, otsikot, Win Now -CTA ja bonus-badge ovat sinisiä/vihreitä eivät kultaisia.
- **FAWeeklyView'n käsintehdyt rivit — YKSI OLI VIELÄ JÄLJELLÄ ja se korjattiin nyt.** #177 muunsi
  markkinalistan (`DSListRow` + `DSListHeaderRow`) ja bidding-updates-palkin, mutta **pending-offers-palkki
  jäi**: sen otsikko puhui `Font.caption`illa siinä missä sisarpalkki kolme riviä ylempänä puhuu
  tracked display -äänellä (= kolmas ääni §2.10:n kahden lisäksi), ja withdraw-kontrolli oli **11 pt
  glyyfi** — sama alle-targetin vika joka sisarpalkin "Dismiss"ista mitattiin 44 pt:ksi.
  `UI/FreeAgency/FAWeeklyView.swift:1067` (`pendingOffersBar`) ja `:1128` (`pendingOfferChip`):
  DSType/DSSpacing-tokenit, `44×44` mitattu target joka määrää chipin korkeuden, otsikko §2.9:n
  tracked `textSecondary`. Rahaluku jäi tarkoituksella plain `textTertiary`lle — `textTertiaryReadable`n
  oma doc kieltää sen: se on TUMMEMPI ja vain opacity-modifierista luopuvia paikkoja varten.
  Samalla tiedoston 5 viimeistä `.system(size: 11)`-glyyfiliteraalia → `DSType.text(DSType.Size.caption)`
  (identtinen ulostulo, nolla visuaalista muutosta). **FAWeeklyView'ssä ei ole enää yhtään
  `.system(size:)`-literaalia.**

**Camp-täytön viritys 80 → 87 (`Engine/Camp/CampRosterEngine.swift`):**
#208:n QA mittasi leiripoolin settlaavan tasan `campRosterTarget`iin eli 80:een, ja arvostelu osoitti
mihin se maksaa: `CutDay.cut90To75` — ainoa porras joka on *nimetty* luvun mukaan — pyysi viittä cuttia
viidentoista sijaan, eli cutdown-tarinan äänekkäin luku luki muodollisuutena. Nyt `campRosterTarget = 87`
(`:107`), headroom `offseasonRosterCeiling` 90:een kolme slottia: 1-for-1-vaihto on runkoneutraali,
2-for-1 tarvitsee yhden slotin, ja AI-seurat istuvat noissa ikkunoissa selvästi alle tavoitteen koska
PHASE A palvelee käyttäjän leirin samasta inventaarista ensin.
**Pakollinen atomipari:** `campShape` 86 → 94 (`:152`). Shape on per-positio-katto, joten
saavutettava määrä = Σ max(0, shape − nykyinen); 86:n summa jätti vain kuusi slottia 80:n yli, eli
tavoite 87 vanhalla shapella olisi jättänyt **jokaisen** seuran shape-lukkoon yhden miehen päähän
numerosta ennen kuin markkinaa edes kysytään. Kahdeksan lisäslottia menivät sinne missä leirikroppa on
uskottava (WR 10→11, TE 5→6, LG/RG 3→4, DE 7→8, DT 6→7, OLB 6→7, CB 9→10); QB/K/P/C koskemattomat.
**Verifiointi (staattinen aritmetiikka, ei ajoa):** kanonisesta 53-blueprintista saavutettava oli
vanhalla 33 (max 86, slack 6 yli 80:n) ja on nyt 41 (max 94, slack 7 yli 87:n) — sama suhde, ei
shape-lukkoa. Positiovahdit eivät heikentyneet: `campShape` on katto joka vain nousi, ja #208:n
release-vahti (`releaseBlockReason`, moottorissa) ei ole tässä tiedostossa. Ajonaikainen portti on
`FillSummary.userShortfall` SMOKE-diag-rivillä.

**UDFA-kattoauditti (`Engine/FreeAgency/UDFAMarketEngine.swift:233` + `:266`) — EI nostettu, ja tässä miksi:**
Tehtävän premissi ("interaktiivinen ≤10, bulk ≤4") on **vanhentunut** — ne olivat #208:aa edeltävät
luvut, ja molemmat polut poistettiin (ks. `DraftDayCoordinator.finishUDFASigning`in doc). Nykytila:
- AI-seuran katto = `clubSigningQuota` **6**, vahdittuna moottorissa sekä tarjous- että selvityshetkellä.
- Käyttäjä **ei koskaan tarjoa** tällä markkinalla: `postAIOffers` rakentaa listansa
  `teams.filter { $0.id != career.teamID }`, tarkoituksella — hänen UDFA-bisneksensä on draft-illassa.
- Draft-illan allekirjoitukset menevät `DraftEngine.convertUDFAToPlayer`in kautta eivätkä näy
  `UDFAMarketState.signings`issa, joten `clubSigningQuota` ei voi niitä nähdä eikä sitoa. Käyttäjän
  todellinen katto on paneelin luku **5** (`UI/Draft/DraftDayCoordinator.swift:183`).
- **Efektiivinen volyymi on käyttäjän puolella:** julistautunut luokka on ~285-350 miestä ja
  `generateDeclarations` tavoittelee 224 pikkiä → markkinan jäännös ~60-126. Käyttäjä poimii
  **ensimmäisenä, vastustamatta, koskemattoman poolin päältä**; 31 AI-seuraa jakaa lopun ≈ 2-4 kpl
  per seura kattoa 6 vasten. Nominaalinen katto on siis yhden miehen matalampi, efektiivinen ei ole.
Moottoriin lisättiin `draftNightUserWindow` (= `clubSigningQuota`, 6) auditin ja pariteettisäännön
kanssa doc-kommentissa, ja `clubSigningQuota`n vanha valheellinen "No privileged quota" -lause
korjattiin. Näkymän yhden rivin adoptio on jäännöslistalla (ks. AUKI NYT -osion #199-jäännökset).

**Design-token-baseline lukittu** (`tools/lint/design_tokens_baseline.json`): font 675 → **0**,
ratingfn 21 → **0**, radius 519 → **409**, spacing 2681 → **2367**, legibility-lattia 486 → **0**
alle-10pt-literaalia. Baseline päivitettiin `--update-baseline`illa tämän aallon jälkeen, joten
seuraava regressio näkyy diffissä eikä auditissa puolen vuoden päästä.

## PÄIVÄ 2026-08-11 MAALISSA — koko katselmuspäivän jono #178–#197 TEHTY (8 workflow'ta, ~130 agenttia; KOMMITTOIMATTA, 224 tiedostoa +11817/−7689 vs 5cc1c76)

**KORJAUS 2026-08-11 ilta: käyttäjä hylkäsi #194:n visuaalisen lopputuloksen** (screenshot-arvio: tausta näkymätön läpinäkymättömien paneelien takana, joukkuevärit eivät näy headerissa/bandissa, keskisarake tyhjiö, raili pistetaulukko) — toiminnot (kello/skip/pick boardilta) toimivat, mutta lavastus puuttui. **#194-v2 HYVÄKSYTTY kuvatuomarilla (4/5 PASS: tausta+värit+näyttämö+raili läpi, tähtihierarkia osittainen; 3 kierrosta, 2 verkkokatko-resumea; pikseliprobet todisteina). Portit vihreät. Tuomarin 7 jäännösfixiä siirtyi v3:een.** OPETUS: checkbox-sim-QA ≠ silmä-QA — "kauniiksi"-vaatimus tarvitsee kuva-arviota vasten iteroinnin. **V3 AJOSSA (wf_a0db5912): #202 (raili→drawer + kaksipalstainen täysboard scouting-koneistolla) + #203 (reveal-kortin tiedot + YOUR TARGET -beat) + tuomarin jäännökset (klippaus mid-animaatiossa, opaakki stage-kaivo, tinttinormalisointi lämpimille seuroille, feed-demotointi omalla kellolla, kuollut skip-nappi, eskalaatiotilan kuvakattavuus).**

**MUUTOIN TEHTY JA PORTIT VIHREÄT**: buildi OK · career 36/36 ×3 (6.2a 12.8-12.9%, 6.2b 4.2-4.7% — flaket kuolleet) · draftclass 39/39 (uudet §7.9-gem-assertit) · leaguegen 19/19 · lint paranee joka mittarilla (legibility: 0 alle-10pt-literaalia, oli 342/baseline-486) · trademark_guard puhdas. Finaalikatselmus: 36 löydöstä → 17 vahvistettu ja KORJATTU (mm. headerin kuollut Pick-CTA, auki jääneen kortin tuplapick-reikä, DealTargetYearin rollover-off-by-one, #186-sokeus kahdessa sisarprojektiossa, private workoutin väärä hasRunTour, perityn raportin combine-fidelity-vuoto, pause-saavutettavuus kellossa). Sim-QA (final-qa-scratch, 88 screenshotia, 0 kaatumista): cap-vuosiosiot PASS, release-ketju päästä päähän PASS, Class Depth/board/kortti PASS, **draft ALL PASS** (war room -tausta + värit, yksi board, rivi→täysi kortti, pause→resume jatkaa 26s→23s, Skip to my pick #3→#29, pick boardilta → Round Recap), FA-sortit+sticky cap PASS. AINOA silmätarkistamaton: workouts-tabin auki-tila (koodi-fix triviaali ja katselmoitu; sim-ajo ei ehtinyt tilaan — katso itse asennuksen jälkeen). Jäljellä: **#198** (P1-draft-backlog, yllä) ja **UUSI #199 finaalikatselmuksen LOW-kasa** (15 kpl, mm. orpo TradeOfferBanner + kuollut ProspectMarkPicker poistoon, trademark_guard kattaa 1/6 termiä → laajenna, TimelineTasksPanelin uudet raw-literaalit, CapOverview'n tupla-variance-varoitus, PRO DAY -slotti syttyy private workoutista, "4/3 reports" -badge). **Käyttäjältä: commit-sana, asennus, IP-juristi, privacy policyn hostaus.**
**#200 TEHTY (wf_00aa9395): Settings pelin sisällä** — gearshape ylänavigoinnin oikeaan klusteriin (inbox·kalenteri·gear·quit, 44pt, ei-kulta), SettingsView(context: .career) shellin OLEMASSA OLEVAN enum-sheet-slotin kautta (ei uutta .sheet-modifieria), career-kontekstissa piilotettu Replay Tutorial + Delete All Save Data. LIVE-äänet OLIVAT jo olemassa (MusicDirector/AudioDirector observoivat UserDefaults.didChangeNotificationia — mid-track-fade 0.4s toimii); lisätty vain idempotentti nudge. Silmätarkistamatta: gearin optinen linjaus + form-sheet iPadilla + portrait-leveys (~98pt slack). HUOM slot-kontrakti: Settingsin sulkeminen voi päästää jonotetun workout-promptin heti perään (designin mukaista).
**UUSI #202 (käyttäjä 2026-08-12, draft-huone v3): oikea raili alas napin taakse → Big Board kaksipalstaiseksi täysboardiksi.** Speksi: (a) oikean railin sisältö pois pystypalstasta: YOUR PICKS + CAPITAL → kompakti yhteenveto alapalkkiin (esim. "R3 · #83 · 3624 pts" -chip), täysi sisältö (capital-lista, SCOUT CHATTER, TRADE RADAR) napin takaa avautuvaan draweriin/sheetiin ("War Room" -nappi); (b) vapautunut tila Big Boardille → ~2 palstan leveys, ja siihen SCOUTING-BOARDIN KONEISTO UUDELLEENKÄYTTÄEN (ei kopiota): samat sarakkeet + linssivaihto (Overview/Work-up/Physical/Mental/Position — DSListColumn/ProspectListControls) + pikapositiosuodattimet (QB/RB/... -chipit) + draft-spesifit sarakkeet (BB-rank, availability, puhelin) päälle; (c) draft-toiminnot säilyvät riveissä (tap→täysi kortti, pick-nappi kellossa). ODOTTAA visuaaliluupin (wf_c87c01bd) valmistumista — samat tiedostot; toteutus omana kierroksena kuvatuomarilla.
**UUSI #203 (käyttäjä 2026-08-12, draft-huone v3): reveal-kortti kantaa liian vähän tietoa** — iso tyhjä alue nimen alla. LISÄTÄÄN (fog-kurilla — vain mitä käyttäjä tietää): (a) scouttauksen yleisarvosana (oma fogattu bändi kullalla jos oma raportti, muuten median harmaa konsensus); (b) KÄYTTÄJÄN OMA MARK isona jos on (Elite/Target/Depth/Avoid + note-indikaattori) — emotionaalinen beat: jos joku vie SINUN Target/Elite-merkityn, kortti huutaa sen ("YOUR TARGET — taken 5 before your turn"); (c) BB-rank vs pick -delta ("BB #14 · +8 value" — B SOLID -chipin rinnalle); (d) ikä · koulu · picking-teamin need-osuma. Toteutus #202:n kanssa samassa v3-kierroksessa (DraftTickerPanel reveal-kortti), kuvatuomarin läpi.
**UUSI #204 (käyttäjä 2026-08-12): UDFA-palkkaus/houkuttelu draftin jälkeen** — "sama ajatus kuin free agencyssä, mutta draftaamattomista pelaajista, scoutatuilla arvoilla". Auditoitava ensin: nykyisin bulk-UDFA-polku allekirjoittaa automaattisesti (DraftEngine.udfaContract 0.300%/3v, OTA-vaihe, refillUserRoster) — onko käyttäjäpintaa? Design: post-draft UDFA-ikkuna jossa käyttäjä kilpailee draftaamattomista fogatuilla arvoilla (kiinnostustasot kuten FA:ssa), AI kilpailee, EI tuplasignausta bulk-polun kanssa. Workflow ajossa (wf-offseason).
**UUSI #205 (käyttäjä 2026-08-12): offseason-rosterin realismi — tarkastus, suunnitelma ja toteutus.** Speksi: (a) rosterikoon kaari offseasonissa (90-man camp → karsinnat → 53) oikean maailman tapaan + cap-vaatimukset (top-51-sääntö tai perusteltu yksinkertaistus); (b) PRESEASON-PELIT: ei-coachattavia sim-pelejä, coach päättää pelaavatko startterit (pelikirjaoppi vs loukkaantumisriski); (c) joka pelistä RECAP-popup pelaajista + statseista (bubble-pelaajien esiinmarssit) jotta käyttäjä voi päättää ketkä pidetään; (d) omat kohdat vasempaan valikkoon rosterin trimmauksesta; (e) integraatio olemassa oleviin: RosterCutView-tikkaat (90→75→65→53), practice squad (#144/#157), camp battles, #102-cap-työtila. VAARAVYÖHYKKEET: SeasonPhase-rawValuet persistoituja (vain additiiviset caset), sopimuskello tikittää VAIN rolloverissa (#89), TaskGenerator-haamut (#134b), roster-floorit (9a53918). Workflow ajossa (wf-offseason).
**UUSI #206 (käyttäjä 2026-08-12, draft v3.3): landscape-header jättimäinen + koko sivun analyysikorjaus.** Kuvasta: (a) header/taustakerros vie landscapessa ~puolet ruudusta lähes tyhjänä ("On the clock" + YOUR PICK IN 2 + kello kelluvat tyhjiössä ennen pick-bandia) — pystymitoitus viritetty portraitille; landscapeen kompakti ~140pt kaista; (b) STEAL OF THE DRAFT -toast piirtyy Big Boardin kromin päälle (sama luokka kuin korjattu FANS-toast — bannerikerroksen ankkurointi); (c) koko sivun analyysipassi landscape-orientaatiossa + KUVAUSHARNESSIIN PYSYVÄSTI molemmat orientaatiot (kaikki tähänastinen tuomarointi oli portrait-only — aito kattavuusaukko). Toteutus v3.3:na HETI v3.2:n valmistuttua (sama tiedostoalue).
**#208 SULJETTU 2026-08-13 (3 fix-kierrosta + 2 re-QA:ta — QA kiersi ensimmäisen vahdin pelaajakortin polulla, joten vahti siirrettiin MOOTTORIIN):** jaettu releaseBlockReason-validaattori jota applyRelease itse kunnioittaa ja jonka kaikki 4 release-polkua näyttävät ennen committia — RE-VERIFIOITU täsmälleen kiertopolulla: QB 4→3→2 laillisesti OK, 2 QB:n kohdalla nappi estetty "CANNOT RELEASE — You must carry 2 QBs" (CHECK2-kuva); leiripoolin invite-aalto korjattu aitoon pelipolkuun: 51→56 (UDFA)→**80** campin alussa, Cut to 75 AITO required-porras "5 more to release" (CHECK1-kuva); Helped/Hurt elää (1/1 nimetyin perusteluin); badget + KR-osoitin OK. **JÄÄNNÖS #199-kasaan: camp-täyttö tuo +24 eikä +34 (pooli 80, tavoite ~90) — invite-määrän viritys; + UDFA-katto 5/5 tarkistettavaksi samalla.** **UUSI #209: buildin ~380-670 Swift 6 -concurrency-varoitusta (main-actor-isolated Codable-konformanssit, NSLock async-konteksteissa) = kova virhe jos kielimoodi nostetaan — oma velka-aalto (swift-concurrency-expert-skill).** Alkuperäiset löydökset: (a) KRIITTINEN: RosterCutView'lta puuttui positiovahti — kaikki 3 QB:tä (ml. ainoa startteri) sai valita releaseen ilman varoitusta; confirm-dialogi puhuu vain capista (RosterCutEvaluator.recommendCuts-vahti on olemassa muttei kytketty commit-polkuun); (b) leiripooli settlaa 64:ään (~90 sijasta) → Cut to 75/65 -portaat auto-ohittuvat "bankattuina"; (c) preseason-recapin "Helped 0 / Hurt 0" kaikissa 3 pelissä vaikka bubble-pelaajilla aitoja statseja (1/1 rec TD) = evaluointi inertti tai kynnys saavuttamaton; (d) kosmeettinen: starters-päätöksen badge-tekstit päällekkäin ("IRST-TEAM INS1O"); (e) UX: KR-startterin puute blokkasi advancen ilman osoitinta — blokkerin pitää nimetä puuttuva slotti. VERIFIOITU HYVÄKSI: UDFA-fog pitää, AUTO-PICK-label näkyy, waiver-claimit toimivat, cutdown-matematiikka oikein.
**UUSI #207 (v3.3-tutkinnan löydös, HIGH): kellon nollautuminen auto-pickaa käyttäjälle LIIGA-AI:N logiikalla TOSIARVOILLA.** tickClock(forUser:true) → DraftEngine.aiMakePick = need-painotettu BPA TRUE-ratingeilla, EI lue userBoardRanks/UserDraftBoardia, ei fog-sidottu → -46 REACH -kortit selittyvät. Skip on PUHDAS (pysähtyy aina, verifioitu :1654-1662). FIX: expiry-polku poimii KÄYTTÄJÄN boardin kärjestä (markit ensin, sitten board-rank, fallback fogattu konsensus-BPA) + pick merkitään AUTO-PICK-labelilla recapiin/tickeriin. Maaliviivakierroksessa (wf-final).
**#194-v3.3 HYVÄKSYTTY 5/5** (kompakti reveal-kortti LER 8-10%, landscape-header 151pt, toastit ankkuroitu, struck-badget himmeät, ei regressioita). Jäännösnotet (#199-kasaan): ~~boardHeaderClearance=196 kovakoodattu → mittaa~~ (KORJATTU maaliviivakierroksessa: `DraftBoardRowsTopKey` + `DraftRoomSpace.table` — LiveBigBoardPanel julkaisee rivialueensa mitatun minY:n, rail käyttää sen `overlayPreferenceValue`-kanavan kautta; 196 jää vain ensiframen fallbackiksi), ~~round-recapin Continue-nappi peittää viimeisen rivin (inset/fade)~~ (JO KORJATTU: `safeAreaInset(edge:.bottom)` tekee napista kromia ja insetoi scrollin sisällön sen korkeudella), landscape-toast kuvaamatta, landscape-kortti LER tasan 15% (kynnyksellä), portrait-kortin pieni yläoikea void.

**#207 KORJATTU (maaliviivakierros).** `tickClock(forUser:true)`-expiry ei enää kutsu `DraftEngine.aiMakePickiä` (= liiga-AI, `trueOverall`/`truePotential` `AIDraftPerceptionin` läpi) vaan uutta `UserDraftBoard.autoPick(among:boardRanks:)`ia: (1) markit ensin, `elite` ennen `targetia`, sisällä oma board-järjestys; (2) `prospectCustomBoard`-rank kaikille joille käyttäjä antoi paikan; (3) fallback fogattu `mediaConsensusOrder`; `avoid` on veto joka murtuu vasta kun poolissa ei ole muita. Ei lue yhtään piilonumeroa (ei myöskään `scoutedOverallia` — board-järjestys sisältää sen jo). `claimedPickNumbers`-atomisuus (#146) ja `ownerOverride`-mekanismi ennallaan. **AUTO-PICK-label**: `PickResult.ownerOverride` → `PickResult.isAutoPick` + uusi `DraftDayCoordinator.autoPickNumbers` (ticker-rivit rakennetaan `DraftPickistä`, jolla ei ole tietoa tekijästä, eikä SwiftData-mallia levennetä yhden illan faktalle) → näkyy reveal-kortin punaisena bannerina ("AUTO-PICK — CLOCK EXPIRED" + provenienssirivi), ticker-historian `AUTO`-merkkinä ja round-recapin oman pick-rivin alla; kaikki kolme VoiceOverissa. Samalla kaksi paljastunutta seurannaisbugia: auto-pick näytti `YOUR ELITE — GONE` -snipe-bannerin ja laukaisi `targetSnipedBeat`-stingin käyttäjän OMASTA pickistä.
**UUSI #201 (löytyi #200-työssä, pre-existing): (a) "Replay Tutorial" on kuollut nappi** — pendingTutorialReplay-lippua ei lue MIKÄÄN (MainMenuView ajaa tutoriaalin vain activeSheet-polulla; doc-kommentti valehtelee) → 3 riviä MainMenuView'hun (lue lippu onAppearissa → activeSheet = .tutorial → nollaa). **(b) "Delete All Save Data" VALEHTELEE**: performReset() pyyhkii vain UserDefaultsit — SwiftData-urat säilyvät, vaikka dialogi+footer+success-alert lupaavat "every career removed". Joko poista myös SwiftData-store (CareerScope.cascadeDelete kaikille urille) tai kirjoita kopio totuudenmukaiseksi. Liittyy #171-migraatiokuriin.

## MEGARUN 2 2026-08-10/11 — #97+#157 + #167 aalto 1 + #175/#176/#177 (workflow wf_18d86e28, 44 agenttia; committoitu fa61979, 87 tiedostoa +1735/−768)

- **#97 SULJETTU — juurisyy oli RIGISSÄ, ei apissa.** CareerScenario.harness arpoi valmentaja-attribuutit N(58,15):stä, mutta shipattu LeagueGenerator.makeCoach pakottaa playerDevelopmentin U(70,85):een kaikilla 7 positiocoach-roolilla + strength coachilla → E[hierarchicalDevelopmentBonus] 1.1646 (app) vs 1.0594 (rig) = **+9.9% kehitysvolyymiä jota §6/§8-kalibrointi ei koskaan nähnyt**. Korjattu rigin peili verolliseksi (shippedCoachAttrs koko 12-slotin goodCount-valinnalla, myös karusellin re-rollit) → rig 80+ 16.22→18.66 = appin 20.8 sisällä 0.14 OVR:llä. ATOMIPARI: CoachingEngine.developmentBonusPivot 50→60 (nimetty vakio, sync_sources kantaa) → appin dev-volyymi −6.4%, **6.2a 10.35→12.6-13.0 (~7σ irti lattiasta, flake kuollut)** + löytyi ja kuoli AIEMMIN HUOMAAMATON 6.2b-flake, blue chipit ensi kertaa §8-bändissä 25-35, 6.10e johdettu uudelleen (ei levennetty). Katselmus pakotti vielä: koordinaattorin schemeExpertise-peili (N(58,15)→U(75,95)) + karusellipremissin korjaus + pivot-kommentin johto.
- **Facility-tuplakerroin REFUTOITU mittauksella** (kaksi sitea = disjoinktit additiiviset kanavat, f·(A+B), eksponentti 1 — mitattu 1.53 vs ennuste 1.50) — vain harhaanjohtava kommentti korjattu. Coach-dev-ratchet mitoitettu MC:llä (400k): attributeCeiling raijaa, ei muutosta.
- **#175**: TEAM-kortti → League Standings (sim-verifioitu). **#176**: 9 todistetusti kuollutta UI-tiedostoa poistettu re-verifioinnin jälkeen (FreeAgencyView 1297r, EventAlertView, CoachRelationshipView, LombardiTrophyView, PositionGradeMiniGrid, SkeletonView, FAStorylineInboxView, FAStorylineToast, VisitTimelineView). **#177**: kaikki 5 LOW korjattu.
- **#167 aalto 1**: 32 fiktiivistä nimeä + NFLTeamData→LeagueTeamData-rename, toimittajat/mediatalot fiktiivisiksi, liigatermisweep (Championship/All-Star Game/The Showcase), ~20 bg_*-imagesetiä pois + DS-gradienttitaustat, TARGETED_DEVICE_FAMILY 2, build-stamp DEBUG-gaten taakse, PrivacyInfo.xcprivacy, ITSAppUsesNonExemptEncryption, docs/PRIVACY_POLICY.md, uusi tools/lint/trademark_guard.sh. Sim-verifioitu: 32 fiktiivistä nimeä listassa, ei yhtään NFL/Super Bowl -osumaa käydyillä ruuduilla.
- **Katselmus**: 4 linssiä → 27 löydöstä → 12 vahvistettu ja korjattu (mm. PlayoffResult "SUPER BOWL CHAMPS" + "SB L", median "SUPER BOWL?!"-reaktio, hostile-reporter-mekaniikka kuoli outlet-renamessa → henkiin, Shanahan-skeemanimi, Canton-metonyymi, balanssipeilin 3 aukkoa).
- **Portit**: BUILD OK · career 36/36 ×3 (6.2a 12.92/12.62/12.98) · draftclass OK · leaguegen 19/19 (uusi ekvilibrium: mean 71.08, 80+ 17.30%, 75+ 35.25%) · lint paranee joka mittarilla (ratingfn 0).

## MEGARUN 2026-08-10 — koko jono balanssiaaltoon asti (workflow wf_110c624a, 68 agenttia; committoitu+pushattu 5cc1c76, 70 tiedostoa +16193/−9817)

- **#105 W1-W5 KOKONAAN**: W1 listastandardi (DSListRow 3 tiheyttä + DSStatusPill + DSEmptyState; Big Board, Roster, Standings, Schedule, League Rosters, FA-rebuild) · W2 hub+shell (yksi hub-layout, ShellDestination-auditti, bookmark-strip 7) · W3 flow't (FA 5 steppiä + cutit 3 stagea DSSlatBand/DSActionBar/DSResultSheet; war roomin DraftPickBand + 5 overlayta → standardisetti [DramaOverlayView/ReactionToast/TradeBeatBanner poistettu]; neuvottelucattien merge → NegotiationChat-chassis; §2.8-sweep + viikkotikkaat + MainMenu 350ms-hack pois) · W4 detaljinäkymät yhdeksi layoutiksi + **ratingfn 21→0** · W5 press×2- ja owner×2-merget + #166-scroll + staff/camp/inbox + modaalit DSResultSheetille + call sheet + entry-ruudut + kulta-sweep.
- **#168 EI OLLUT ELÄVÄ VIKA**: kaikki 4 treidipolkua kulkevat TradeEngine.movePlayerin kautta (teamID-kirjoitus), lukupuoli on teamID-pohjainen (Team.currentRoster), Team.players on dokumentoidusti write-only. TRADE_OVERHAUL_PLAN.md §7.5:n huoli on VANHENTUNUT. Latentti: Team.players jää pysyvästi vääräksi relaatioksi — kovennus (poisto/uudelleennimeäminen) kuuluu #171-migraatioaaltoon. pressureWeek-jäännös oli MYÖS jo korjattu (91f62a5).
- **#169 PREMISSI VÄÄRÄ**: kuolleita statsinäkymiä ei ole (kaikki konstruoidaan livenä); TODO.md:24:n PlayerStatsView-datawiring on jo korjattu. Sivutuotteena löytyi AITO kuollut UI-kasa → #176.
- **#170** Settings: 6 kuollutta kontrollia + valheellinen Difficulty-footer pois. **#172** MetricKit: Diagnostics/CrashReportStore.swift (cap 20, thread-safe). **#171** docs/SWIFTDATA_MIGRATION_PLAN.md (711 r, file:line-ankkuroitu). **#173** playoffsHeroCard oikeasta bracket-datasta + Vegas line pois; Cap Scenario rehelliseksi (valhe-checkmark pois).
- **Katselmus**: 7 linssiä → 31 löydöstä → 20 verifioitu → **17 vahvistettu ja korjattu**, mm. RosterCutView'n jäätynyt roster-snapshot (cutdown-tikkaat eivät edenneet KOSKAAN sessiossa — 4 linssiä löysi saman), CareerShellView'n holdout-result valehteli forced-tradesta, handleSheetDismiss ajoi väärän haaran kalenterin stagettua seuraavan sheetin, OTAs-workout-prompti tuhoutui holdout-dialogiin, scout-palkkauksen $0.0M, Big Boardin RPT-slot laski perityn raportin omaksi.
- **Portit**: BUILD SUCCEEDED · career 36/36 (6.2a-flake ks. yllä) · draftclass OK · leaguegen 19/19 · lint: font 675→533, radius 519→425, spacing 2681→2394, ratingfn 21→0, ei regressiota.
- **Scratch-sim-QA** (oma sim, luotu+poistettu): 8 screenshotia, 0 kaatumista, 0 tyhjää ruutua; uusi ura + onboarding + hub + roster + detail + board + FA + settings käveltiin läpi. Löydös → #175. Patch-snapshotit per aalto scratchpadissa (w105-*.patch).

**#167 Legal-aalto 1 — PARKITUS KUMOTTU käyttäjän päätöksellä myöhemmin 2026-08-10**: käyttäjä käski toteuttaa heti; työ käynnistetty rinnakkaisajossa #97/#157/#175-177:n kanssa. Sisältö ja todisteet: docs/APP_STORE_COMPLIANCE_AUDIT.md (32 nimeä NFLTeamData:an publish-jsonista + rename LeagueTeamData, 8 toimittajaa pois, bg_*-taustat + 16 imagesetiä pois, ~200 stringiä SeasonPhase-suppilon kautta, TARGETED_DEVICE_FAMILY 2, PrivacyInfo.xcprivacy, ITSAppUsesNonExemptEncryption, privacy policy -dokumentti). IP-juristin liikkeellelähtö on edelleen käyttäjän oma askel (lead time!).

Käyttäjältä: W0+v4-silmäys, ~~Big Boardin Recommendations-lohkojen taittopäätös~~ (päätetty: OMA Scout Notes -välilehti + Insights kiinni oletuksena — toteutettu, taitto verifioimatta), ~~#167-ajoitus~~ (päätetty: parkissa peliin-valmis-hetkeen), #74 musiikit, #5 (iPad-katselmus).

## Ilta-aalto 2026-08-07 — committoitu `91f62a5` (kirjoitettu HEAD b694011:ssä, nippu landattu myöhemmin samana iltana)

Kuusi rinnakkaista agenttia. `BUILD SUCCEEDED` koko kompositiolla. Committoitu `91f62a5` — laitteella verifioimatta (ks. VERIFIOIMATTA alla).
Numerovaraus: `#166` oli jo varattu (press-scroll), joten tämän aallon UI-työ on **`#174`** koodikommenteissa.

- [ ] **#174 Insights kiinni oletuksena + Scout Notes -välilehti.** (a) Vanha sääntö oli "auki kerran per pinta per vaihe": hubissa on 11 pintaa, joten YKSI kalenterin edistys aseisti ne kaikki uudelleen → yhdeltä advancelta yksitoista ruutua joissa proosalohko peittää taulukon jonka takia tultiin. Nyt: kiinni kunnes käyttäjä avaa, sitten pinnan oma muisti. `DraftPrepProcessBar.swift:537` (`resolvedExpansion` puhdas luku, default `false`), `:617` (`resolveDefault` ei enää kirjoita — uudelleenaseistus oli sen ainoa kirjoittaja; `onChange(of: phaseToken)` poistettu). (b) Avain oli `scoutInsightsOpen_*` = paljas `UserDefaults`, ja phaseToken oli se joka PEITTI vuodon; uudelleenaseistuksen poistuttua scoping piti tehdä oikeasti → `ScoutingHubView.swift:725` `insightsSurfaceKey` = "<career>-<tab>". Vanhat avaimet jäävät inerteiksi (nimi vaihtui) — haluttua, koska oletuskin vaihtui. (c) `ScoutingTab.scoutNotes` + `warRoomTabs` 6-tabiksi: `Big Board · Class Depth · Scout Notes · Draft Order · Mock Draft · Scout Team`.
- [ ] **Scout Notes -irrotus.** `BigBoardView.swift` **−548 riviä**: `recommendationsSection` ja `depthAnalysisSection` pois taulukon päältä. Ne EIVÄT olleet itsenäisiä — lukivat boardin per-dataversio-välimuistia jota myös jokaisen rivin NEED-chip lukee → uusi **`ScoutBoardReads.swift`** (401 r) = yksi kävely josta molemmat lukevat (sama kaava kuin `DraftPrepProgress`/`DraftPrepStageCell`), **`ScoutNotesView.swift`** (436 r) piirtää. Sumu- ja determinismitakuut (`:820-843`, `:854-880`, `:922-938`) siirretty mukana.
- [ ] **#158 + #133 + #135 → uusi `Domain/Staff/StaffLedger.swift`** (231 r). #158 juurisyy: KAKSI predikaattia yhdelle kysymykselle, ja Season Guiden `onAdvancePhase` kutsuu `performShellAdvance()`ia SUORAAN ohi dashboardin — eikä siinä ollut staff-porttia lainkaan, joten kultainen "Advance" saattoi ajaa ylikulutetulla potilla / vajaalla koordinaattorilla, ja estotilassa luki "0 required tasks remaining" kuolleen napin päällä. #133 juurisyy: paikat laskettiin `StaffSlots`illa mutta **palkat summattiin per RIVI** → GM+HC-savessa hairahtanut head-coach-rivi veloitettiin potista jolla ei ole sitä paikkaa → "23/23 filled" ja "used $x" kuvasivat eri populaatiota, ja luku liikkui relaunchissa; lisäksi dashboard luki omistajan `team`-`@State`stä jonka latausjärjestys vaihteli per käynnistys (nil → koko budjettirivi katosi, samalla kun Staff-ruutu keksi `$20.0M`). #135 juurisyy: Conflict oli −9 OVR **sakko**, ei este → ~11 OVR parempi konfliktikandidaatti voitti tuolin jonka seuraava ruutu merkitsee ✗:llä. **Varaus:** relaunch-oiretta ei saatu toistettua laitteella, vain paikannettua kaksi koodipolkua — jos oire oli kolmas kuvio, se on yhä auki.
- [ ] **#134 + #131.** MOLEMMAT olivat jo puoliksi korjattuja (`8ba35f1`) — TODO-rivi on vanhempi kuin korjaus; tänään korjattiin jäännös. #134a: `PlayerDetailView.leagueRanking` teki `sorted { $0.overall > $1.overall }` `@Query`-tuloksille **ilman sort descriptoria** → ei-totaali komparaattori määrittelemättömän hakujärjestyksen päällä. 500 permutaation harness: **20/32 pelaajan näytetty sija liikkui**, id-tiebreakin jälkeen 0/32. `comparablePlayers` pahempi (±3 OVR -ikkuna → melkein kaikki tasan). #134b: `TaskGenerator`in read-only `"─ Offseason ─"` -bannerí toimitetaan pre-`.done`; vasen paneeli jätti sen pois, Season Guide laski sen JA laski sen valmiiksi → haamu-1 osoittajassa. Kaksi kopiota "laske taskit" → yksi (`TimelineTasksPanel.taskProgress`). #131: 7 kuollutta jäsentä, −37 riviä. **Kieltäytyi oikein** poistamasta `LegacyTracker.pressPromises` / `PressPromise.isDelivered`: ei lukijoita, mutta ovat `Codable`n talletettuja kenttiä `Career.legacy`ssä → savenmuotomuutos, kuuluu #171:een. Löysi ~191 lukijatonta jäsentä puun laajuisesti eikä pitänyt sitä poistojonona (protokollakonformanssit, property wrapper -API).
- [ ] **#36 trade-myönnytysten pehmennys.** Ei ollut puuttuva pehmennys vaan **rajaton** pehmennys: `concessionTarget(view:round:)` on ollut olemassa `5af7a6a`sta (BACKLOG:23 merkitsi jo `[x]`, vain plan-doc sanoi auki). Mitattu ennen: **aggressiivinen GM antoi takaisin 100 % preemiostaan kierrokseen 10 mennessä**, balanced 98 % → saman paketin uudelleenlähetys oli ilmaista vipua; `maxRounds` (2-4) luettiin VAIN insult-polulla vaikka `sendOffer` kasvattaa kierrosta joka lähetyksellä → kierros 20 kahden kierroksen miestä vastaan oli laillinen ja kannatti. Nyt per-arkkityyppi `concessionCap` (oldSchool 0.35 → aggressive 0.65) + `deadlinePressure` (vk 6-9) + `needPressure`; max 35-65 % levossa, 44-83 % täydessä paineessa, **ei koskaan 100 %**. `counterLead` sanoo nyt "hold where they are — this is their number" kun GM on lakannut liikkumasta (aiemmin UI opetti jatkamaan Counter-nappia). #150:n no-ratchet säilyy.

**VERIFIOIMATTA (tärkein ensin):**
- [ ] **Portrait-taitto.** 7 slattia + 6 tabia 1032 pt:llä. Aritmetiikka sanoo että mahtuu ilman varaa (~113 pt/slat vs 112 pt lattia; 6 tabia ≈ 160 pt, "CLASS DEPTH" ≈ 110). Landscape väljä. **Ei koskaan piirretty.**
- [ ] `ScoutNotesView` ei ole renderöitynyt kertaakaan laitteella: tyhjä tila, scrollaus hub-kromin alla, lukevatko lohkot oikein `List`in ulkopuolella.
- [ ] Insights-kiinni-oletus rakennettu muttei nähty ajossa.
- [ ] #38/#46 (trade-onsim) ei yritetty — vaativat simuslotin.
- [ ] **Jäännös jota ei otettu:** `WeekAdvancer.swift:5890` välittää yhä oletus-`pressureWeek`in → AI:n aloittama paluu deadline-viikolla hinnoittelee kiireensä threadin AVAUSviikosta. Yksi argumentti.
