...
- [ ] **A · Trade-järjestelmän selvitys + uudistus.** Selvitys ensin: miten AI tekee tradeja NYT ja onko se realistista (arvostuslogiikka, frekvenssi, ajoitus) → sitten uudistus: (1) trade-neuvottelu-UI samaan tapaan kuin pelaajasopimusneuvottelut, käytössä kaikkialla — myös draftin aikana (pick-tradet draft-huoneessa); (2) draft-pickien ja/tai pelaajien yhdistelmät samassa diilissä + arvokartta; (3) AI tarjoaa tradeja aktiivisesti myös käyttäjälle; (4) offseason-tradet + kauden trade deadline realistiseksi — kuumat joukkueet (playoff-jahti) ETSIVÄT vahvistusta, huonot joukkueet MYYVÄT ja rebuildaavat (fire sale). Aloita selvitysraportilla ennen koodia.
...
**2 · PlayerSeasonHistoryn oikeat statsirivit** (pieni-keskikoko, iso näkyvä hyöty)
- keyStat1-3 on tyhjä placeholder; generoi kausistatsit (OVR+positio+rooli → pelin statsimalli) → urasivut, HOF-perustelut, vertailut. Template-liigalla OVR-käyräurat jo pohjana.
