...
VERBATIM_SOURCES=(
  ...
  # The cutdown-day sort key, whole. 99 lines, one dependency (`Player`), and it
  # is read by BOTH sides of the roster door in the app (`trimAIRosters` decides
  # who is cut with it, `refillAIRosters` who is signed back) and by the whole
  # practice-squad eligibility rule (`PracticeSquadEngine.isSquadEligible`'s
  # veteran-slot branch is a keepScore test). Task #98's `agePenaltyCap` was
  # transcribed into `CareerScenario.keepScore` by hand once; verbatim is how
  # the next constant that moves here reaches the rig without a second edit.
  "Engine/Simulation/RosterValue.swift"
...
# Practice squad (task #157 diagnostic): the eligibility rule, the service
# window, the signing key and the engine's OWN FillSummary diagnostic line.
PRACTICESQUAD_SOURCE="$ENGINE/Engine/Contract/PracticeSquadEngine.swift"
CAMPROSTER_SOURCE="$ENGINE/Engine/Camp/CampRosterEngine.swift"
MEDICAL_SOURCE="$ENGINE/Engine/Medical/MedicalEngine.swift"
...
# Task #157: the practice-squad fill ranks candidates on the club's THINNEST
# position groups, and `topTeamNeeds` is the function `fillSquads` calls for it.
# Without it the rig's shadow fill would have to invent its own notion of "thin".
grep -q 'static func topTeamNeeds(roster' "$DEVSLICE" \
  || die "DraftEngine slice lost topTeamNeeds (practice-squad fill reads it)."
{
  echo "// GENERATED by sync_sources.sh — DO NOT EDIT."
...
# --- 8i) PracticeSquadExtract.swift ------------------------------------------
# Task #157: the practice-squad RULES, so the `career` scenario can measure the
# squad path it never had any exposure to.
#
# What comes across: the eligibility gate (`isSquadEligible`), the service-window
# test (`needsVeteranSlot`), the cutdown-day signing key (`squadSigningScore`)
# and the engine's OWN `FillSummary` — the last one deliberately, because its
# `diagnosticLine` is the line `fillSquads` prints in the app, so the rig and a
# sim run report the same fields in the same order and are directly comparable.
#
# What is left behind: `fillSquads` itself, `stash`, the weekly poach pass and
# the accrued-seasons fetch — every one of them is bound to `Career`,
# `ModelContext`, `InboxEngine` or `LeagueGenerator`'s insert path. The rig
# supplies the allocation loop as scaffolding (the `career` scenario's own note
# on what it owns) and calls the three shipped predicates above for every
# decision that has a rule behind it.
echo "==> regenerating PracticeSquadExtract.swift from repo (awk keep-list slice)"
PRACTICESQUAD_OUT="$SRC_OUT/PracticeSquadExtract.swift"
cat > "$DEVANCHORS" <<'EOF'
struct FillSummary \{
static func isSquadEligible\(
static func needsVeteranSlot\(
static func squadSigningScore\(
EOF
keeplist_slice "$PRACTICESQUAD_SOURCE" "$DEVANCHORS" "$DEVSLICE"
verbatim_guard "$PRACTICESQUAD_SOURCE" "$DEVSLICE"
# The four gates the diagnostic is ABOUT. Each of these is a number the rig
# reports a distribution against, so a slice that lost one would print a
# measurement of a rule the app does not have.
PRACTICESQUAD_CONSTS="$(grep -E '^[[:space:]]*static let (squadSize|squadGenerationFloor|activeRosterCeiling|veteranSlots|youngPlayerYearsPro|maxPerPosition|maxQuarterbacks|startingCalibreOverall|accruedSeasonGames|accruedSeasonsLimit|ownCutSigningBonus|thinPositionSigningBonus)(: [A-Za-z<>]+)? =' "$PRACTICESQUAD_SOURCE")"
for k in squadSize squadGenerationFloor activeRosterCeiling veteranSlots youngPlayerYearsPro \
         maxPerPosition maxQuarterbacks startingCalibreOverall accruedSeasonGames \
         accruedSeasonsLimit ownCutSigningBonus thinPositionSigningBonus; do
  printf '%s\n' "$PRACTICESQUAD_CONSTS" | grep -qE "static let $k(:| =)" \
    || die "PracticeSquadEngine constant $k not found in the repo file."
done
grep -q 'return player.overall < startingCalibreOverall' "$DEVSLICE" \
  || die "PracticeSquad slice lost the unrestricted-seat OVR gate."
grep -q 'RosterValue.keepScore(player) < Double(startingCalibreOverall)' "$DEVSLICE" \
  || die "PracticeSquad slice lost the veteran-slot keepScore gate."
grep -q 'accruedSeasons <= accruedSeasonsLimit' "$DEVSLICE" \
  || die "PracticeSquad slice lost the accrued-seasons window."
grep -q 'var diagnosticLine: String' "$DEVSLICE" \
  || die "PracticeSquad slice lost FillSummary.diagnosticLine (the rig prints the engine's own line)."
{
  echo "// GENERATED by sync_sources.sh — DO NOT EDIT."
  echo "// Source: dynasty/dynasty/Engine/Contract/PracticeSquadEngine.swift (sha $(sha "$PRACTICESQUAD_SOURCE"))"
  echo "// Transform: awk KEEP-LIST slice of the squad ELIGIBILITY + signing key +"
  echo "//            FillSummary. fillSquads/stash/poaching (Career, ModelContext,"
  echo "//            InboxEngine) are deliberately left behind."
  echo ""
  echo "import Foundation"
  echo ""
  echo "enum PracticeSquadEngine {"
  echo "$PRACTICESQUAD_CONSTS"
  echo ""
  cat "$DEVSLICE"
  echo "}"
} > "$PRACTICESQUAD_OUT"
printf 'EXTRACT    %s  build/src/PracticeSquadExtract.swift  <=  %s  dynasty/dynasty/Engine/Contract/PracticeSquadEngine.swift  (keep-list slice)\n' \
  "$(sha "$PRACTICESQUAD_OUT")" "$(sha "$PRACTICESQUAD_SOURCE")" >> "$MANIFEST"
echo "    PracticeSquadExtract.swift"

# --- 8j) CampRosterConstantsExtract.swift -------------------------------------
# The camp roster's two numbers, by grep — the rig reports its own camp size
# against them (task #157: the 80/90-man camp path had ZERO rig exposure). The
# rest of CampRosterEngine is @MainActor + SwiftData and stays in the app.
echo "==> regenerating CampRosterConstantsExtract.swift from repo (grep of 2 constants)"
CAMPROSTER_OUT="$SRC_OUT/CampRosterConstantsExtract.swift"
CAMPROSTER_CONSTS="$(grep -E '^[[:space:]]*static let (campRosterTarget|campContractYears) =' "$CAMPROSTER_SOURCE")"
for k in campRosterTarget campContractYears; do
  printf '%s\n' "$CAMPROSTER_CONSTS" | grep -qE "static let $k =" \
    || die "CampRosterEngine constant $k not found in the repo file."
done
CAMPROSTER_CEILING="$(grep -E '^[[:space:]]*static let offseasonRosterCeiling =' "$TRADEVALUE_SOURCE")"
[ -n "$CAMPROSTER_CEILING" ] \
  || die "TradeValueEngine.offseasonRosterCeiling not found (CampRosterEngine.rosterCeiling forwards to it)."
{
  echo "// GENERATED by sync_sources.sh — DO NOT EDIT."
  echo "// Source: dynasty/dynasty/Engine/Camp/CampRosterEngine.swift (sha $(sha "$CAMPROSTER_SOURCE"))"
  echo "//         dynasty/dynasty/Engine/Contract/TradeValueEngine.swift (offseasonRosterCeiling)"
  echo "// Transform: grep of the offseason roster SIZE constants — the ladder the"
  echo "//            career rig measures its own camp against."
  echo ""
  echo "import Foundation"
  echo ""
  echo "enum CampRosterEngine {"
  echo "$CAMPROSTER_CONSTS"
  echo "}"
  echo ""
  echo "// The 90 lives in TradeValueEngine so the trade market and the camp fill"
  echo "// cannot disagree about what a legal offseason roster is; GMPersonaExtract"
  echo "// already owns the enum, so it arrives here as an extension."
  echo "extension TradeValueEngine {"
  echo "$CAMPROSTER_CEILING"
  echo "}"
} > "$CAMPROSTER_OUT"
printf 'EXTRACT    %s  build/src/CampRosterConstantsExtract.swift  <=  %s  dynasty/dynasty/Engine/Camp/CampRosterEngine.swift  (grep of 2 constants + TradeValueEngine ceiling)\n' \
  "$(sha "$CAMPROSTER_OUT")" "$(sha "$CAMPROSTER_SOURCE")" >> "$MANIFEST"
echo "    CampRosterConstantsExtract.swift"

# --- 8k) MedicalEngineExtract.swift ------------------------------------------
# `MedicalEngine.dressed` — the one function the shipped GameSimulator calls to
# decide who suits up. It is four lines and it is the whole of D4-A's "injured
# players stop dressing" fix, so the rig has to carry the REAL one: a stub that
...
# Signature-tolerant on PURPOSE: `forTeam` gained a defaulted `declaredIdentity:`
# when D7/F-56 let the user declare a franchise identity, and an exact-signature
# guard turned a compatible change into a harness outage. What this assert is for
# is proving the slice CARRIED the function, not freezing its parameter list.
grep -q 'static func forTeam(id: UUID' "$DEVSLICE" || die "GMPersona slice lost forTeam."
{
  echo "// GENERATED by sync_sources.sh — DO NOT EDIT."
